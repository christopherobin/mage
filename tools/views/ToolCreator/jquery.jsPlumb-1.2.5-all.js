...
(function() {
	...
	var jsPlumbInstance = function() {
			...
			if (draggable) {
				if (jsPlumb.CurrentLibrary.isDragSupported(element) && !jsPlumb.CurrentLibrary.isAlreadyDraggable(element)) {
					...
					options[stopEvent] = _wrap(options[stopEvent], function() {
						...
						_currentInstance.fireUpdate("jsPlumb_dragged", {
							element: element
						});
					});
		...
		this.detach = function(source, target) {
			...
			// 18/05/2011 - Commented out fireDetachEvent since it's already being fired when detachFrom is called, resulting in the detachConnection even being thrown twice
