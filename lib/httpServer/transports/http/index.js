...
function requestHandler(req, res) {
	...
	// track the response for data gathering purposes

	trackHttpResponse(req, res, path, isQuiet);

	// if no handler was found for this route, return: 404 Not found
	// the one exception is the asterisk which is a request for the entire server (typical for OPTIONS)
	...
	// allow clients to query our capabilities
	// also used as an opportunity to respond to asterisk requests with 200 OK

	if (req.method === 'OPTIONS' || isAsterisk) {
