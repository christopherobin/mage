...
var logger, expose, server, wsServer, cors;

var isListening = false;

var longRoutes = [];
var longThreshold = 500;
...
function sendDecompressedHttpResponse(res, statusCode, headers, body) {
	...
	zlib.gunzip(body, function (error, uncompressed) {
		...
		logger.debug('Decompressed GZIP encoded content for client without support.');
	...
	return function (req, res, path, urlInfo) {
		fn(req, path, urlInfo.query, function (httpCode, out, headers) {
			...
			if (httpCode >= 500 && httpCode <= 599) {
				logger.error.data(req).log('Internal server error (' + httpCode + ') during HTTP request:', path);
			} else {
...
function deliverFolder(absPath, relPath, res, cb) {
	fs.readdir(absPath, function (error, files) {
		...
		html.push('<html>');
		html.push('<head>');
		html.push('  <title>' + relPath + '</title>');
		html.push('  <style type="text/css">');
		html.push('    BODY { font: 14px Arial, sans-serif; }');
		html.push('  </style>');
		html.push('</head>');
		html.push('<body>');
		html.push('  <h1>Contents of ' + relPath + '</h1>');

		html.push('  <a href="../">..</a><br>');

		var count = files.length;

		if (count === 0) {
			html.push('Folder is empty');
		} else {
			files.sort();

			for (var i = 0; i < count; i += 1) {
				var fileName = escapeHtml(files[i]);

				html.push('  <a href="./' + fileName + '">' + fileName + '</a><br>');
			}
		}
...
exports.serveFolder = function (route, folderPath, options, onFinish) {
	...
	exports.addRoute(regexp, function (req, res, reqPath) {
		...
		if (absPath.indexOf(folderPath + '/') === -1 && absPath !== folderPath) {
			res.writeHead(404);
			res.end();
			return onFinish();
		}
		...
		fs.stat(absPath, function (error, stats) {
			...
			if (stats.isFile()) {
				...
				deliverFile(absPath, res, headers, onFinish);
				return;
			}

			// show the directory contents

			if (stats.isDirectory() && options.autoIndex) {
				...
				deliverFolder(absPath, reqPath, res, onFinish);
				return;
			}
...
function requestHandler(req, res) {
	...
	// time the execution, so we can warn about slow operations
	...
	var isLong = isLongRoute(req.url);
