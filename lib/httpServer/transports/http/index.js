	...
	return function (req, res, path, urlInfo) {
		fn(req, path, urlInfo.query, function (httpCode, out, headers) {
			...
			if (httpCode >= 500 && httpCode <= 599) {
				logger.critical.data(req).log('Internal server error (' + httpCode + ') during HTTP request:', path);
			} else {
...
exports.setFavicon = function (buffer, mimetype) {
	...
	exports.addRoute(FAVICON_ROUTE, function (req, path, params, cb) {
...
exports.enableDefaultFavicon = function () {
	// Here is a hack to make sure we don't overwrite existing favicon routes because MAGE calls
	// enableDefaultFavicon on startClientHost which happens after setup when people normally set
	// their favicon.

	if (routers.http.get(FAVICON_ROUTE)) {
		return;
	}
...
function requestHandler(req, res) {
	...
	// We expect msg stream to stay open for a long time, so no need to warn on those requests.
	var msgStreamRegEx = /^\/msgstream/;

	function complainAboutLongRequest() {
		var duration = Date.now() - reqStart;
		if (duration > 500 && !msgStreamRegEx.test(req.url)) {
