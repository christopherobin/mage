...
function followRoute(route, req, res, path, urlInfo) {
	// time the execution
	...
	res.on('finish', function () {
		...
		if (durationSec > longThreshold * 1000 && isLongRoute(req.url)) {
			logger.warning(req.url, 'completed in', durationSec, 'sec');
		}

		exports.emit('response-finish', String(route.matcher), duration);
	});
...
function requestHandler(req, res) {
	...
	// follow route from an external function so its closure remains as small as possible

	if (!isQuiet) {
		logger.verbose('Following HTTP route', route.matcher);
	}

	followRoute(route, req, res, path, urlInfo);
}
