...
HttpServer.prototype.setupCommandSystem = function (config) {
	...
	function onCommandResponse(transportError, responses) {
		...
		if (transportError) {
			// "auth": the user should login again
		...
		for (var i = 0; i < responses.length; i += 1) {
			...
			if (errorCode) {
				// look for system wide errors that need to be emitted on the io.error namespace

				var m = errorCode.match(/^io\.error\.(.+)$/);
				if (m && m[1]) {
					// system-wide error at the single command level

					errorCode = m[1];

					that.eventManager.emitEvent('io.error.' + errorCode, {
						reason: errorCode
					});
				}
			}
