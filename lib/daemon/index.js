...
function start(app, cb) {
	...
	if (app.runstate === STATUS_RUNNING) {
		err(('Already running (pid: ' + app.pid + '), aborting...').yellow.bold);
		return cb(EXIT_FAILED);
	}

	// Check if process disappeared
	if (app.runstate === STATUS_DISAPPEARED) {
		err(('Process disappeared (pid: ' + app.pid + '), aborting...').red.bold);
		return cb(EXIT_FAILED);
	}

	var appName = process.argv[0];

	// copy the args, but remove the "start" command
