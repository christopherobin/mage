...
function SampleTaker(sample) {
	this.min = sample;
	this.max = sample;
	this.sigma = new StandardDeviation(sample);
...
SampleTaker.prototype.addSample = function (sample) {
	...
	this.sigma.addMeasurement(sample);
...
Panopticon.prototype.workerSetup = function () {
	this.on('sample', function (data) {
		process.send({ event: 'workerSample', sample: data });
	});
};
...
Panopticon.prototype.sample = function (id, sample) {
	this.timeUp(); // Check if this sample should be in a new set.
