 ...
 * @param {number} interval
 */
function genericSetup(thisObj, startTime, interval) {
	...
	thisObj.endTime = now + offset + (offset < 0) ? this.interval : 0;
...
function Panopticon(startTime, interval) {
	...
	genericSetup(this, startTime, interval);
 ...
 * @param {String[]} path Addresses the data object, with each element down a level from the one before it.
 * @param {String} id A key to assign data to within the address defined by path.
 * @param {Number} n Set or increment the addressed data by n.
 */
