...
function WorkerData(startTime, interval) {
	...
	// Generate an endTime, at which we deliver the sample
	var now = Date.now();

	var offset = (startTime - now) % interval;

	// We need offset to be positive. As startTime can be before now, we need to bump the resulting
	// offset by one interval in that case.
	if (offset < 0) {
		offset += interval;
	}

	this.endTime = now + offset;
...
WorkerData.prototype.resetData = function () {
	// the data property holds all the samples
...
WorkerData.prototype.timeUp = function () {
	...
	// Recreate the timer if it's not running
...
exports.setup = function (cfg) {
	...
	var beginReportingIn = cfg.startTime + (cfg.interval / 2)  - Date.now();
