...
function sendQueue(client) {
	...
	// generate a basePath and ensure that it starts and ends with a slash (eg: "/foo/", "/foo/bar/" or "/")
	...
	var req = http.request(reqOptions, function (res) {
		...
		res.on('close', function () {
			client.logger.error.data({ request: reqOptions, data: data }).log('Disconnected while receiving response from peer', client.peer);
		...
		if (res.statusCode !== 200) {
			client.logger.error.data({ request: reqOptions, data: data }).log('Got an unexpected status from peer', client.peer + ':', res.statusCode);
	...
	req.on('error', function (e) {
		client.busy = false;
		client.logger.error.data({ request: reqOptions, data: data }).log('Got a transport error from peer', client.peer + ':', e.message);
		client.emit('transportError', queue);
	});
