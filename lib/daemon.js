...
var EXIT_TIMEOUT = 2; // No response from Mithril
...
function daemonRunningStatus(pid) {
	// if no PID is given, load the PID from the PID file

	if (!pid) {
		pid = getFilePid(DAEMON_LOCK_FILE);
...
function start(daemonPid, cb) {
	if (daemonPid) {
		err(('Already running (pid: ' + daemonPid + '), aborting...').yellow.bold);
		return cb(EXIT_FAILED);
	}

	var appName = process.argv[0];
	var args = detectArgs(process.argv, true);
...
function stop(daemonStatus, daemonPid, cb) {
	if (daemonStatus !== STATUS_RUNNING || !daemonPid) {
		err('Process is not running.'.yellow.bold);
		destroyFilePid(DAEMON_LOCK_FILE);
		return cb(EXIT_FAILED);
	}

	out(('Stopping... (pid: ' + daemonPid + ')').green.bold);

	sendSignal(daemonPid, CMD_QUIT, null, function (exitCode) {
...
function restart(daemonStatus, daemonPid, cb) {
	stop(daemonStatus, daemonPid, function (exitCode) {
...
function reload(daemonStatus, daemonPid, cb) {
	if (daemonStatus !== STATUS_RUNNING || !daemonPid) {
	...
	out(('Reloading... (pid: ' + daemonPid + ')').green.bold);

	sendSignal(daemonPid, CMD_RELOAD, null, cb);
}


function status(daemonStatus, daemonPid, cb) {
	switch (daemonStatus) {
	case STATUS_RUNNING:
		out(('Status: Running (pid: ' + daemonPid + ')').green.bold);
		break;
	case STATUS_NOTRUNNING:
		err('Status: Not running'.red.bold);
		break;
	default:
		err(('Status: Undefined (pid: ' + daemonPid + ')').red.bold);
		break;
	}
...
function runCommand(command, cb) {
	...
	case 'restart':
		restart(daemonStatus, daemonPid, cb);
		break;
	...
	case 'status':
		status(daemonStatus, daemonPid, cb);
		break;
