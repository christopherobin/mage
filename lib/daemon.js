...
var waitForConfirmation = function (cb, time) {
    ...
    var cleanListeners = function () {
        process.removeListener('SIGCHLD', sigChildHandler);
	...
	// Daemon is done
    var sigChildHandler = function sigChildHandler() {
    ...
    var sigContHandler = function sigContHandler() {
		clearTimeout(timeout);
		timeout = timeoutFactory();
	};

    var sigUsr2Handler = function sigUsr2Handler() {
		clearTimeout(timeout);
        cleanListeners();
		cb(true);
	};

	process.once('SIGCHLD', sigChildHandler);
	process.once('SIGCONT', sigContHandler);
	process.once('SIGUSR2', sigUsr2Handler);
};
