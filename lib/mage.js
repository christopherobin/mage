...
/**
 * With useModule you can tell mage which bundled modules you want it to use. Pass in an array of
 * module names, or module names as arguments to this method.
 *
 * @return {Object} The mage itself is returned to allow chaining.
 */

Mage.prototype.useModule = function () {
	var moduleList = Array.isArray(arguments[0]) ? arguments[0] : arguments;
...
/**
 * With useModule you can tell mage which game modules you want it to use. Pass an array of game
 * module paths, or the paths as arguments to this method.
 *
 * @return {Object} The mage itself is returned to allow chaining.
 */

Mage.prototype.addModule = function () {
	var moduleList = Array.isArray(arguments[0]) ? arguments[0] : arguments;

	for (var i = 0; i < moduleList.length; i++) {
		// Resolve the path to a full path relative to rootPath.
		var modPath = path.resolve(rootPath, moduleList[i]);

		// Determine the name using the last path chunk.
		var name = modPath.split(path.sep).pop();
		var ext = path.extname(name);

		// If the module is naked, we may need to remove the extension for the name.
		if (ext !== '') {
...
/**
 * Sets up mage and its modules. Listen for the `'setupComplete'` event, or pass in a callback.
 *
