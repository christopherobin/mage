...
Mage.prototype.setupArchivist = function (cb) {
	...
	try {
		topics = require(path.join(rootPath, 'lib/archivist'));
	} catch (e) {
		return cb(new Error('Could not find topic APIs. You must have a module set up at lib/archivist to use the archivist module.'));
	}
