...
Mage.prototype.setupArchivist = function (cb) {
	...
	var apisPath = path.join(rootPath, 'archivist');

	this.core.helpers.readDirectory(apisPath, /\.js$/, function (error, dir) {
		if (error) {
			return cb(error);
		}

		var targets = dir.files.concat(dir.directories);

		targets.forEach(function (target) {
			var vaultName, apiPath, api;

			try {
				// the vaultName is filename without extension

				vaultName = target.replace(/\.[a-zA-Z0-9]+$/, '');
				apiPath = path.join(apisPath, target);

				logger.debug('Configuring topics API for vault', vaultName, 'from path', apiPath);

				// require the topics api and register it

				api = require(apiPath);

				archivist.registerTopicsApi(vaultName, api);
			} catch (error) {
		...
		archivist.createVaults(cfg.vaults, cb);
	});
/*
	var cfg = {
	...
	archivist.registerReadOrder(['file']);
	archivist.registerWriteOrder(['file', 'mage-client']);

	archivist.registerTopicsApi('file', {
		testTopic: {}
	});

	archivist.registerTopicsApi('mage-client', {
		testTopic: {
			shard: function (topic, vars) {
				return true; //vars.actorId;
			}
		}
	});

	archivist.createVaults(cfg, cb);
*/
};
