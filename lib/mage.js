...
/**
 * Add a search path for modules. Add the most important first.
 *
 * @param {String} modPath A path to a directory with game modules.
 */
	...
	for (var i = 0; i < arguments.length; i++) {
		...
		for (var j = 0; j < this._modulesList.length; j++) {
			if (name === this._modulesList[j][0]) {
				this.fatalError('Attempted to add a second module with name:', name);
			}
		}
