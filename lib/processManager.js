...
function createNewWorker() {
	...
	logger.mithril_debug('[ProcessManager] Forked worker', worker.process.pid);
}
...
if (cluster.isMaster) {
	...
	process.once('SIGINT', function () {
		logger.mithril_debug('Caught SIGINT.');
		mithril.quit(true);
	});

	process.once('SIGTERM', function () {
		logger.mithril_debug('Caught SIGTERM.');
		mithril.quit(true);
	});
} else {
	process.on('message', function (msg) {
		if (msg === 'shutdown') {
			logger.mithril_debug('[ProcessManager] Worker received shutdown request.');
...
exports.quit = function (graceful, returnCode) {
	...
	logger.mithril_debug(logPrefix, 'is about to shut down (graceful: ' + (graceful ? 'yes' : 'no') + ').');
	...
	if (exports.isMaster) {
		...
		logger.mithril_debug('[ProcessManager] Asking all workers to shutdown.');
		...
		cluster.disconnect(function () {
			logger.mithril_debug('[ProcessManager] Workers disconnected.');
		});
...
exports.start = function () {
	...
	try {
		...
		mithril.core.logger.mithril_debug('Unable to get process info from package.json.');
	}
