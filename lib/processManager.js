...
function createNewWorker() {
	var worker = cluster.fork({ MASTER_PID: process.pid, CLUSTER_START_TIME: exports.startTime });
...
if (cluster.isMaster) {
	...
	cluster.on('fork', function (worker) {
		startupTimeouts[worker.id] = setTimeout(function () {
			logger.error('Worker', worker.process.pid, 'is taking too long to respond. Restarting in', WORKER_STARTUP_TIMEOUT, 'msec.');
	...
	cluster.on('exit', function (worker, code, signal) {
		...
		if (mithril.getRunState() !== 'quitting') {
			if (exitCode) {
				logger.error('Worker', worker.process.pid, 'just died (' + signal + ', exit code: ' + exitCode + '). Restarting now...');
				createNewWorker();
			} else {
