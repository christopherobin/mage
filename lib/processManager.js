...
function WorkerManager() {
	...
	this.max = require('os').cpus().length;
...
WorkerManager.prototype.createWorker = function (cb) {
	...
	success = function (address) {
		...
		worker._mithrilManagedExit = false;
	...
	failure = function () {
		...
		var error = new Error('Worker ' + worker.process.pid + ' exitted prematurely.');
		logger.alert(error);
	...
	worker = cluster.fork();
	worker._mithrilManagedExit = true;
...
function waitForMithrilStarted(cb) {
	function runStateCheck(runState) {
		// when running, emit that we've started

		if (runState === 'running') {
			mithril.removeListener('runState', runStateCheck);
			processManager.emit('started');
		}
	}

	mithril.on('runState', runStateCheck);
	runStateCheck(mithril.getRunState());
...
processManager.start = function (cb) {
	...
	if (processManager.isMaster) {
		...
		// set the number of workers and spawn them

		workerManager.setMax(numberOfWorkers, function () {
...
processManager.quit = function (graceful, cb) {
	...
	if (workerManager) {
		...
		// nothing special to do here for workers, it's the mithril object that does the final
		// process.exit()
