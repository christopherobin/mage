var mithril = require('../mithril'),
    logger = mithril.core.logger.context('membase'),
...
MembaseStore.prototype.autoTransaction = function () {
...
MembaseStore.prototype.getMany = function (keys, required, errorCode, cb) {
	if (keys.length === 0) {
		logger.verbose('getMany (no keys given)');
		return cb(null, {});
	}
	...
	client.getMulti(keys, function (error, values) {
		logger.verbose('getMany', keys.join(' '));
...
MembaseStore.prototype.setMany = function (valueMap, ttl) {
	...
	for (var key in valueMap) {
		...
		logger.verbose('set(Many)', key, '->', value);
