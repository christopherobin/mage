...
exports.close = function (cb) {
	...
	mithril.core.logger.info('Gracefully closing connections to Membase');
...
function getClient(config) {
	if (!client) {
		...
		client.on('failure', function (details) {
			mithril.core.logger.error('Membase server went down', JSON.stringify(details));
		});

		client.on('reconnecting', function (details) {
			mithril.core.logger.error('Reconnecting to Membase server', JSON.stringify(details));
		});

		client.on('reconnected', function (details) {
			mithril.core.logger.error('Reconnected to Membase server', JSON.stringify(details));
		});

		client.on('issue', function (details) {
			mithril.core.logger.error('Issue occured on Membase server', JSON.stringify(details));
		});

		client.on('remove', function (details) {
			mithril.core.logger.error('Membase server removed from cluster', JSON.stringify(details));
		});
	}
...
MembaseStore.prototype.commit = function (cb) {
	mithril.core.logger.debug('KV: Commit');
	this.connect().commit(cb);
};


MembaseStore.prototype.rollBack = function (cb) {
	mithril.core.logger.debug('KV: Rollback');
...
MembaseStore.prototype.getMany = function (keys, required, errorCode, cb) {
	if (keys.length === 0) {
		mithril.core.logger.debug('KV: getMany (no keys given)');
		return cb(null, {});
	}
