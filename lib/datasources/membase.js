...
exports.close = function (cb) {
	...
	logger.notice('Gracefully closing connections to Membase');
...
function getClient(config) {
	if (!client) {
		...
		client.on('failure', function (details) {
			logger.emergency('Membase server went down', JSON.stringify(details));
		});

		client.on('reconnecting', function (details) {
			logger.alert('Reconnecting to Membase server', JSON.stringify(details));
		});

		client.on('reconnected', function (details) {
			logger.notice('Reconnected to Membase server', JSON.stringify(details));
		});

		client.on('issue', function (details) {
			logger.alert('Issue occured on Membase server', JSON.stringify(details));
		});

		client.on('remove', function (details) {
			logger.emergency('Membase server removed from cluster', JSON.stringify(details));
		});
	}
...
MembaseStore.prototype.getMany = function (keys, required, errorCode, cb) {
	if (keys.length === 0) {
		logger.verbose('KV: getMany (no keys given)');
		return cb(null, {});
	}
	...
	client.getMulti(keys, function (error, values) {
		logger.verbose('KV: getMany', keys.join(' '));
...
MembaseStore.prototype.set = function (key, value, ttl) {
	...
	logger.verbose('KV: set', key, '->', value);
...
MembaseStore.prototype.setMany = function (valueMap, ttl) {
	...
	for (var key in valueMap) {
		...
		logger.verbose('KV: set(Many)', key, '->', value);
...
MembaseStore.prototype.incr = function (key, size, cb) {
	var client = this.connect();
	var state = this.state;
...
MembaseStore.prototype.decr = function (key, size, cb) {
	var client = this.connect();
	var state = this.state;
...
MembaseStore.prototype.delMany = function (keys) {
	...
	for (var i = 0, len = keys.length; i < len; i++) {
		...
		logger.verbose('KV: del(Many)', key);
