...
function getClient(config) {
	if (!client) {
		...
		client.on('failure', function (details) {
			mithril.core.logger.error('Membase server', details.server, 'went down due to:', details.messages);
		});

		client.on('reconnecting', function (details) {
			mithril.core.logger.error('Total downtime caused by Membase server', details.server, 'is', details.totalDownTime + 'ms');
		});

		client.on('reconnected', function (details) {
			mithril.core.logger.error('Reconnected to Membase server', details.server);
		});

		client.on('issue', function (details) {
			mithril.core.logger.error('Issue occured on Membase server', details.server);
		});

		client.on('remove', function (details) {
			mithril.core.logger.error('Membase server', details.server, 'removed from cluster');
		});
	}
