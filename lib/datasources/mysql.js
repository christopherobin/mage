var mithril = require('../mithril'),
    mysql = require('mysql'),
	logger = mithril.core.logger.context('mysql');
...
function sampleSqlError(error) {
	if (!error.hasOwnProperty('number')) {
		exports.emit('sqlMiscError');
		return;
	}

	switch (error.number) {
	case 1159:
		exports.emit('sqlReadTimeOut');
		return;
	case 1161:
		exports.emit('sqlWriteTimeOut');
		return;
	case 1205:
		exports.emit('sqlLockWaitTimeOut');
		return;
	default:
		exports.emit('sqlMiscError');
		return;
...
function query(conn, sql, params, cb) {
	...
	conn.query(sql, params, function (err, info) {
		var durationRaw = process.hrtime(begin);
		var duration = durationRaw[0] * 1e3 + durationRaw[1] / 1e6;

		exports.emit('mysqlQueryDuration', duration); // Send the data as nanoseconds.
