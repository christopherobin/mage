var mage = require('../mage'),
    mysql = require('mysql'),
    logger = mage.core.logger.context('ds-mysql');
...
// We need to have a reverse error code lookup for the sampler.
var sqlErrorCodes = {};

for (var key in mysql) {
	if (mysql.hasOwnProperty(key) && key.split('_')[0] === 'ERROR') {
		sqlErrorCodes[mysql[key]] = key;
	}
}
...
MySqlDatabase.prototype._connect = function (config, cb) {
	...
	logger.verbose('Connecting to', options);
...
// Wraps a query so we can fire emissions when queries open, and also so that we can measure the time a query takes.
function query(conn, sql, params, cb) {
	exports.emit('mysqlQuery');

	var begin = process.hrtime();

	conn.query(sql, params, function (err, info) {
		exports.emit('mysqlQueryDuration', process.hrtime(begin)); // Send the data as length 2 array.
...
MySqlDatabase.prototype.source = function (readonly, cb) {
	...
	if ((readonly && this.transactionRules.read) || (!readonly && this.transactionRules.write)) {
		...
		this.rw(function (error, conn) {
			...
			logger.verbose('Transaction start');

			query(conn, 'START TRANSACTION', [], function (error) {
				if (error) {
					return cb(error);
				}
...
MySqlDatabase.prototype.close = function () {
	if (this.connRO && this.connRO !== this.connRW) {
		logger.verbose('Closing read-only database connection.');
	...
	if (this.connRW) {
		logger.verbose('Closing read/write database connection.');
...
MySqlDatabase.prototype.commit = function (cb) {
	...
	query(this.connTransaction, 'COMMIT', [], function () {
...
MySqlDatabase.prototype.rollBack = function (cb) {
	...
	query(this.connTransaction, 'ROLLBACK', [], function () {
...
MySqlDatabase.prototype.getOne = function (sql, params, required, errorCode, cb) {
	...
	this.source(true, function (error, db) {
		...
		query(db, sql, params, function (error, results) {
			logger.verbose('getOne', sql, 'using', params);

			if (error) {
				state.error(errorCode, { sql: sql, params: params, error: error }, cb);
				return;
			}
			...
			if (required && len !== 1) {
				state.error(errorCode, { sql: sql, params: params, error: 'expected exactly 1 record, but received ' + len }, cb);
				return;
			}
...
MySqlDatabase.prototype.getMany = function (sql, params, errorCode, cb) {
	...
	this.source(true, function (error, db) {
		...
		query(db, sql, params, function (error, results) {
			logger.verbose('getMany', sql, 'using', params);

			if (error) {
				exports.emit(error, sqlErrorCodes[error.number] || 'MISC_ERROR', error.number);
...
MySqlDatabase.prototype.getMapped = function (sql, params, map, errorCode, cb) {
	...
	this.source(true, function (error, db) {
		...
		query(db, sql, params, function (error, results) {
			logger.verbose('getMapped', sql, 'using', params);

			if (error) {
				exports.emit('mysqlError', error);
				cb(error);
				_this.state.error(errorCode, { sql: sql, params: params, error: error }, cb);
				return;
			}
			...
			for (var i = 0, len = results.length; i < len; i++) {
				...
				if (map.value) {
					if (map.type) {
						out[row[map.key]] = mage.core.PropertyMap.unserialize(row[map.type], row[map.value]);
					} else {
...
MySqlDatabase.prototype.exec = function (sql, params, errorCode, cb) {
	...
	this.source(false, function (error, db) {
		...
		query(db, sql, p, function (error, info) {
			logger.verbose('exec', sql, 'using', params);
