 ...
 * @param {Function} callback  A callback that must be called to close the state.
 */
...
CommandCenter.prototype.processHeaders = function (headers, server, callback) {
	...
	processBatchHeader(state, {
		...
		// Attach the session to the server to be reused later
		server.mageSession = state.session;

		function cb() {
			server.mageSession = null;
			state.close();
		}
