...
CommandCenter.prototype.expose = function () {
	var maxAccess = this.app.access;
	var modNames = mage.listModules();

	var count = 0;

	logger.verbose('Exposing commands.');
	...
	for (var i = 0; i < modNames.length; i++) {
		...
		var files = fs.readdirSync(modPath + '/' + commandsDirName);
		...
		for (var j = 0; j < files.length; j++) {
			...
			if (mage.core.access.compare(cmdAccess, maxAccess) < 0) {
				// this command is not allowed to be exposed on this app
				logger.verbose('Not allowed to expose command', cmdAccess, 'on app:', this.app.name, '(skipping)');
				continue;
			}
