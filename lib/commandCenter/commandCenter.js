...
CommandCenter.prototype.executeCommand = function (cmd, session, cb) {
	...
	if (!state.canAccess(cmdInfo.access)) {
		...
		// we don't have the auto close cb set yet, let's do it manually
		// TODO: change that once PR#267 is in
		return state.error('auth', new Error(errorText), function () {
			// just close the state and send the response
			...
			async.mapSeries(
				...
				function (error, results) {
					...
					postProcessCommandResponse(results, function (options, content) {
						...
						var msg = cached ? 'Executed and cached user commands' : 'Executed user commands (could not cache)';
