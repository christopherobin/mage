...
CommandCenter.prototype.assertValidAccessLevel = function (level, command) {
...
CommandCenter.prototype.executeCommand = function (cmd, session, cb) {
	...
	try {
		this.assertValidAccessLevel((session && session.meta) ? session.meta.access : 'anonymous', cmd);
	} catch (err) {
		logger.error(err);
