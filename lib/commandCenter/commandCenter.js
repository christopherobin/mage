...
CommandCenter.prototype.assertValidAccessLevel = function (level, commands) {
	for (var i = 0, len = commands.length; i < len; i++) {
		var command = commands[i];

		if (!command || !command.name) {
			throw new Error('Cannot access nameless command: ' + JSON.stringify(command));
		}

		var cmdInfo = this.commands[command.name];
		if (!cmdInfo) {
			throw new Error('Command "' + command.name + '" is not exposed on app "' + this.app.name + '"');
		}

		if (mage.core.access.compare(level, cmdInfo.access) < 0) {
			throw new Error('User command access level not satisfied for command "' + command.name + '" on app "' + this.app.name + '"');
		}
	}
};
	...
	state.archivist.createVault('client', 'client', { state: state }, function () {
		...
		paramList.push(function () {
			...
			state.close(function (closeError, response) {
				...
				logger.info('User command', cmd.name, 'execution took', 1000 * duration, 'msec.');
...
function resolveFilesInParams(params, files) {
	...
	for (var key in params) {
		...
		if (files.hasOwnProperty(param)) {
			params[key] = files[param];
			continue;
		}

		// if this argument is a filelist, turn its value into an array of filedata

		var m = params[key].match(/^__filelist:(.+)$/);
		if (m) {
			var value = [];

			var fileIds = m[1].split(',');
			for (var i = 0; i < fileIds.length; i++) {
				var fileId = fileIds[i];

				if (files.hasOwnProperty(fileId)) {
					value.push(files[fileId]);
				}
			}
...
CommandCenter.prototype.executeCommands = function (commandNames, headerData, paramsData, files, queryId, transportInfo, transportCb) {
	...
	processCommandHeader(state, headerData, paramsData, function (error, tags) {
		...
		try {
			that.assertValidAccessLevel((session && session.meta) ? session.meta.access : 'anonymous', commands);
		} catch (err) {
			logger.error(err);
