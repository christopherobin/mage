...
var COMPRESSION_THRESHOLD = 4096;
...
exports.passDependencies = function (mageInstance, mageLogger, stateConstructor) {
	mage = mageInstance;
	logger = mageLogger;
...
function getMemcachedClient() {
	...
	if (mcClient === false) {
		logger.verbose('Response cache disabled (missing configuration)');
		return null;
	}

	var config = mage.core.config.get('data.kvcache');
...
function createResponseCacheKeys(session) {
	if (!session) {
		logger.verbose('No session, so could not create response cache keys');
	...
	if (!sessionKey) {
		logger.verbose('No registered session key, so could not create response cache keys');
...
CommandCenter.prototype.expose = function (requirements, commandList) {
	var count = 0, cmds, cmdName, cmd, modPath, cmdPath, mod, execPath;

	logger.verbose
		.data('requirements', requirements)
		.data('commands', commandList)
		.log('Exposing commands.');

	for (var gameModule in commandList) {
		cmds = commandList[gameModule];

		for (var i = 0, len = cmds.length; i < len; i++) {
			cmdName = cmds[i];

			cmd = this.commands[gameModule + cmdPathSplitter + cmdName];
			if (cmd) {
			...
			if (!mage[gameModule]) {
				throw new Error('Game module "' + gameModule + '" not found.');
			}

			modPath = mage.getModulePath(gameModule);
			if (!modPath) {
				throw new Error('Could not resolve path of module "' + gameModule + '".');
			}

			cmdPath = modPath + '/' + commandsDirName + '/' + cmdName;

			logger.verbose('Exposing command "' + gameModule + '.' + cmdName + '" at "' + cmdPath + '"');

			mod = require(cmdPath);  // require() may throw, which we like in this case

			if (!mod.params) {
				throw new Error('Command', cmdPath, 'has no configured params.');
			}

			execPath = gameModule + cmdPathSplitter + cmdName;
	...
	logger.notice(count, 'commands exposed.');
};
...
CommandCenter.prototype._tryLoadCachedResponse = function (session, queryId, cb) {
	...
	logger.verbose('Trying to load response cache');
	...
	if (!this.responseCacheTTL || this.responseCacheTTL < 1) {
		logger.verbose('Response cache disabled (non-positive TTL set up)');
		return cb();
	}

	var client = getMemcachedClient();
	if (!client) {
		logger.verbose('No memcached client available');
		return cb();
	}
	...
	client.get(keys.info, function (error, info) {
		if (error) {
			logger.verbose('Error while requesting response cache meta-info:', error);
			return cb();
		}

		if (typeof info !== 'string') {
			logger.verbose('No response cache meta-info found:', keys.info);
			return cb();
		}
		...
		client.get(keys.data, function (error, response) {
			...
			if (typeof response !== 'string') {
				logger.verbose('Response cache was desired, but no data found:', keys.data);
				return cb();
			}
			...
			logger.verbose('Loaded command response from cache.');
...
CommandCenter.prototype._cacheResponse = function (session, queryId, options, response, cb) {
	...
	client.set(keys.data, response, ttl, function (error) {
		...
		logger.verbose('Caching command response successful:', keys.data);
		...
		client.set(keys.info, info, ttl, function (error) {
			...
			logger.verbose('Caching command response meta-info successful:', keys.info);

			logger.time('Cached command response in', Date.now() - startTime, 'msec.');
	...
	processCommandHeader(state, headerData, paramsData, function (error, tags) {
		...
		if (failure) {
			logger.error
				...
				.log('User command request requirements failed');
			...
			async.mapSeries(
				...
				function (error, results) {
					if (error) {
						...
						cb(null, { httpStatusCode: 500 });   // 500: Internal service error
					} else {
						// turn results array in a reportable response (possibly gzipped)
						...
						postProcessCommandResponse(results, function (options, content) {
							...
							that._cacheResponse(session, queryId, options, content, function () {
								...
								logger.time('User command batch', commandNames, 'execution and caching took', Date.now() - startTime, 'msec.');
