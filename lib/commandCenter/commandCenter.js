...
CommandCenter.prototype.setupUserCommand = function (modName, cmdFile) {
	...
	if (cmd.acl) {
		errorString = 'The acl configuration of user command ' + execPath + ' must be an array.';
		assert(Array.isArray(cmd.acl), errorString);
	}
 ...
 * @param {Object[]} batch.header         Array of header objects that will be passed to message hooks one by one.
 ...
 * @param {string[]} requiredHooks        A list of required hooks. Without all of these, an error will be returned.
 * @param {Function} cb                   Callback on completion.
 */

function processBatchHeader(state, batch, requiredHooks, cb) {
	if (requiredHooks.length > 0) {
		for (var i = 0; i < requiredHooks.length; i += 1) {
			var found = false;

			for (var j = 0; !found && j < batch.header.length; j += 1) {
				if (batch.header[j].name === requiredHooks[i]) {
					found = true;
				}
			}

			if (!found) {
				return state.error(null, 'Missing hook: ' + requiredHooks[i], cb);
			}
		}
	}
	...
	async.eachSeries(
		...
		function (entry, callback) {
			...
			fnHook(state, entry, batch, callback);
		},
		function (error) {
			if (error) {
...
/**
 * Prevents user commands from running if they do not present a specific hook-header to the backend
 *
 * @param {string} name   The name of the message hook to add
 */

CommandCenter.prototype.addRequiredMessageHook = function (name) {
	this.requiredHooks.push(name);
};
