...
function CommandCenter(app) {
	...
	this._responseCache = new ResponseCache(app.name, ttl);
}

exports.CommandCenter = CommandCenter;

CommandCenter.prototype.getApp = function () {
	return this._app;
};

CommandCenter.prototype.getCommands = function () {
...
CommandCenter.prototype.setup = function () {
	if (this._exposed) {
		throw new Error('The "' + this._app.getName() + '" command center has already been exposed.');
	}

	var appAccess = this._app.getAccess();
	var modNames = mage.listModules();

	var count = 0;

	logger.verbose('Exposing commands for app:', this._app.getName());
		...
		for (var j = 0; j < files.length; j++) {
			...
			if (mage.core.access.compare(cmdAccess, appAccess) > 0) {
				...
				logger.verbose(
					...
					'on app:', this._app.getName(), '(' + appAccess + ') (skipping)'
				);
				continue;
			}
	...
	logger.notice(count, 'commands exposed for app:', this._app.getName());
 ...
 *
 * @param modName   The name of the module, you want to get the list of function
 * @returns {object{}}
 */
...
CommandCenter.prototype.getModuleCommands = function (modName) {
	...
	for (var cmdPath in this._commands) {
		var parts = cmdPath.split(cmdPathSplitter);

		if (modName === parts[0]) {
			result[parts[1]] = this._commands[cmdPath];
		}
	}
...
function serializeCommandResult(response) {
	...
	var out = '[null,' + (JSON.stringify(response.response) || 'null');
...
CommandCenter.prototype.setUserCommandTimeout = function (timeout) {
	this._userCommandTimeout = timeout;
};
 ...
 *
 * If the params are an Array, the extra parameters will be removed,
 * or the missing will be added with undefined as value.
...
CommandCenter.prototype.buildParamList = function (cmdInfoModParams, cmdParams) {
	...
	if (Array.isArray(cmdParams)) {
		paramList = cmdParams.slice(0, cmdInfoModParams.length);
...
CommandCenter.prototype.executeCommand = function (cmd, session, cb) {
	...
	var state = new State();
	state.appName = this._app.getName();

	if (this._userCommandTimeout) {
		state.setTimeout(this._userCommandTimeout);
	}
	...
	if (!state.canAccess(cmdInfo.access)) {
		var errorText = 'User command access level not satisfied for command "' + cmd.name +
			'" on app "' + this._app.getName() + '". Access level required is "' + cmdInfo.access +
	...
	state.archivist.createVault('client', 'client', { state: state }, function () {
		...
		var paramList = that.buildParamList(mod.params, cmd.params);
		...
		paramList.push(function () {
			...
			state.close(function (closeError, response) {
				...
				exports.emit('completed', that._app, cmd, durationRaw);
...
CommandCenter.prototype.executeBatch = function (acceptedEncodings, batch, queryId, transportCb) {
	...
	state.appName = this._app.name;

	exports.emit('openPostConnection', this._app);

	function cb(error, content, options) {
		state.close(function () {
			exports.emit('closePostConnection', that._app);
	...
	processBatchHeader(state, batch, function (error) {
		...
		that._responseCache.get(state, queryId, function (error, options, content) {
			...
			function respond(options, content) {
				...
				var cached = that._responseCache.set(state, queryId, options, content);
 ...
 * @param {Object} server      The server to which it should attach the session
 * @param {Function} callback  Return the session and a function to call to close the state
 */

CommandCenter.prototype.processHeaders = function (headers, server, callback) {
	var state = new State();
	state.appName = this._app.getName();
