...
function ResponseCache(appName, ttl) {
	...
	if (this.enabled && mage.core.archivist.topicExists('ucResponseMeta') || mage.core.archivist.topicExists('ucResponseData')) {
		...
		logger.info('User command response enabled for app', appName, 'with TTL:', ttl);
	} else {
		logger.warning('User command response cache disabled for app', appName);
	}
}
