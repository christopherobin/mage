...
function buildErrorResponse(jsonrpcServer, requestObject, code, err) {
	return jayson.utils.response(jsonrpcServer.error(
		code,
		err.message
	), null, requestObject.id);
}

function addSessionToParams(requestObject, session) {
			...
			commandCenter.processHeaders(headers, jsonrpcServer, function (err, session, callback) {
				if (err) {
					...
					if (Array.isArray(request)) {
						error = request.map(function (requestObject) {
							return buildErrorResponse(jsonrpcServer, requestObject,
								jayson.Server.errors.INTERNAL_ERROR, err);
						});
					} else {
						error = buildErrorResponse(jsonrpcServer, request,
							jayson.Server.errors.INTERNAL_ERROR, err);
					}
					return respondError(JSON.stringify(error), 401);
				}

				// Add the session to the params list to be used by the Command Center
				if (Array.isArray(request)) {
					request.forEach(function (requestObject) {
						addSessionToParams(requestObject, session);
					});
				} else {
					addSessionToParams(request, session);
				}
