...
/**
 * Stick a property to the JSON-RPC request params
 *
 * @param {Object}  request   The JSON-RPC request
 * @param {string}  name      What to name the property
 * @param {*}       value     The value to set
 */
function addPropertyToRequest(request, name, value) {
	if (Array.isArray(request)) {
		for (var i = 0; i < request.length; i++) {
			request[i].params = request[i].params || {};
			request[i].params[name] = value;
		}
	} else {
...
/**
 * Extract and remove a property from a JSON-RPC request
 *
 * @param {Object}  params    The JSON-RPC request params
 * @param {string}  name      The name of the property
 * @return {*}      value     The value that was set
 */
function getPropertyFromRequestParams(params, name) {
...
function createJsonrpcRouter(commandCenter) {
	...
	return function (method, params) {
		// Get meta data that was temporarily stuck on the params
		var session = getPropertyFromRequestParams(params, '_mageSession');
		var address = getPropertyFromRequestParams(params, '_clientAddress');
		...
		return function () {
			...
			commandCenter.executeCommand(cmd, session, { address: address }, callback);
		};
	};
}
 ...
 * @param {Object}      request         The JSON-RPC request (optional)
...
function buildErrorResponse(jsonrpcServer, request, code, err) {
	var response = jsonrpcServer.error(code, err);

	return jayson.utils.response(response, null, request ? request.id : null);
}
 ...
 *
 * @param {Object}          jsonrpcServer   The jayson instance
 * @param {Object|Array}    request         The JSON-RPC request
...
function respondError(jsonrpcServer, request, res, err, code, headers) {
	...
	if (Array.isArray(request)) {
		body = request.map(function (requestObject) {
			return buildErrorResponse(jsonrpcServer, requestObject, jayson.Server.errors.INTERNAL_ERROR, err);
		});
	} else {
		body = buildErrorResponse(jsonrpcServer, request, jayson.Server.errors.INTERNAL_ERROR, err);
	}
...
function handleJsonrpcRequest(commandCenter, jsonrpcServer, req, res, err, request) {
	...
	if (err) {
		return respondError(jsonrpcServer, null, res, err.message, 400);
	}

	// parsing didn't fail with an error, yet did not yield a JSON-RPC request object
	if (!request || typeof request !== 'object') {
		return respondError(jsonrpcServer, null, res, 'Bad request', 400);
	}
	...
	commandCenter.processHeaders(headers, function (err, session, callback) {
		...
		// Add the session and source IP to the request so they can be passed on to the command center
		addPropertyToRequest(request, '_mageSession', session);
		addPropertyToRequest(request, '_clientAddress', forwarded(req, req.headers));
...
exports.register = function (commandCenter) {
	...
	httpServer.addRoute('/' + commandCenter.app.name + '/jsonrpc', handler, 'simple');
};
