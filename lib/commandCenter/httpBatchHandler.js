...
function parseBatchData(fullPath, rawData, files) {
	...
	try {
		...
		logger.error.data(cmdData[0]).log('Error parsing command header.');
	}

	assert(Array.isArray(header), 'invalidHeader');
...
exports.register = function (commandCenter) {
	...
	function handler(req, res, path, query, urlInfo) {
		...
		var data = { address: forwarded(req, req.headers) };
	...
	httpServer.addRoute(new RegExp('^/' + commandCenter.app.name + '/'), handler, 'simple');
};
