...
function getZooKeeperClient(options) {
	...
	zooKeeperClient = zooKeeper.createClient(options.hosts, clientOpts);
	...
	mage.once('shutdown', function () {
		var state = zooKeeperClient.getState();
...
ZooKeeperService.prototype.announce = function (port, metadata, cb) {
	...
	logger.verbose('Removing', announcePath, '(this can time out if zookeeper is not reachable).');
			...
			that.client.create(announcePath, dataBuffer, zooKeeper.CreateMode.EPHEMERAL, function (error) {
				if (error) {
					...
					if (error.getCode() === zooKeeper.Exception.NODE_EXISTS) {
						logger.emergency(
							'NODE_EXISTS on our node creation should *never* happen, but it just did.',
							'That can only mean the following.',
							'(1) Double callbacks during the MAGE setup phase.',
							'(2) A race condition between two master processes that both identify as',
							announcePath + '.'
						);
					}
				}
