...
function realAnnounce(client, baseAnnouncePath, port, metadata, cb) {
	...
	logger.debug('Announcing', fullAnnouncePath);
...
ZooKeeperService.prototype.announce = function (port, metadata, cb) {
	...
	logger.verbose('Checking if', this.baseAnnouncePath, 'exists (this can time out if zookeeper is not reachable).');
	...
	this.client.exists(this.baseAnnouncePath, function (error, stat) {
		...
		if (stat) {
			logger.verbose('Announcing service on', that.baseAnnouncePath);

			realAnnounce(that.client, that.baseAnnouncePath, port, metadata, cb);
		} else {
			// otherwise we need to create it

			logger.verbose('Creating base service path:', that.baseAnnouncePath);
