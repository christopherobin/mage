...
var messenger = require('../../../processManager/messenger');
...
function SingleService(name, type, options) {
	...
	// it should send all the services to all the workers.
	if (mage.core.processManager.isMaster) {
		messenger.on('serviceDiscovery.browse', function () {
			Object.keys(that.services).forEach(function (key) {
				messenger.broadcast('serviceDiscovery.up', this.services[key]);
			}, that);
		});
	}
}
...
SingleService.prototype.announce = function (port, metadata, cb) {
	...
	if (mage.core.processManager.isMaster) {
		messenger.broadcast('serviceDiscovery.up', node);
	}
...
SingleService.prototype.discover = function () {
	...
	var that = this;
	messenger.on('serviceDiscovery.up', function (service) {
	...
	if (mage.core.processManager.isWorker) {
		// Tell the master to broadcast the updated list of services
		messenger.send('serviceDiscovery.browse');
	}
