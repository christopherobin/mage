...
// used to cache services data, this is in the module global scope because if 2 submodules discover the same service
 ...
 *
 * @link https://developer.apple.com/library/mac/documentation/Networking/Conceptual/dns_discovery_api/Articles/registering.html#//apple_ref/doc/uid/TP40002478-SW1
...
MDNSService.prototype.announce = function (port, metadata, cb) {
	...
	if (!nodeEnv) {
		return cb(new Error('No value was set for the environment variable "NODE_ENV". This variable is needed for mDNS announcements.'));
	}
	...
	var serviceOptions = {
		...
		name: nodeEnv + '/' + mage.rootPackage.name + ' (' + uuid.v4() + ')',
...
exports.setup = function (name, type, options) {
	if (!mdns) {
		try {
			...
			throw new Error('Could not load node module: mdns2');
		}
	}
