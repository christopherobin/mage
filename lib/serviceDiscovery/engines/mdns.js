...
ServiceType.prototype.listen = function () {
	...
	this.browser.on('serviceUp', function (mdnsService) {
		var host = normalizeHost(mdnsService.host);
		var port = mdnsService.port;

		var service = services[mdnsService.name];
		if (service && service.mdnsAdvertisement) {
			// don't emit for advertised services
			return;
		}

		if (!service) {
			// ignore already known (including previously announced self!)
