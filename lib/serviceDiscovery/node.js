...
for (var i = 0; i < interfacesKeys.length; i++) {
	...
	for (var j = 0; j < interface.length; j++) {
		...
		if (!localIps[ip.family]) {
			// TODO: maybe log? should never happen though unless IPv8 pops tomorrow
			continue;
		}
...
ServiceNode.prototype.isLocal = function () {
	...
	if (!ip) {
		logger.data(this).warning('This node is not announcing any IP! Something is most likely wrong with it.');
		return false;
	}

	var version = 'IPv' + net.isIP(ip);
	if (version === 'IPv0') {
		logger.data(this).warning('Invalid IP announced!');
		return false;
	}
