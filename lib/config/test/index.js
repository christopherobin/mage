...
describe('A matryoshka instance', function () {
	...
	it('should wrap anything.', function (done) {
		...
		var g = new Matryoshka({});
		var h = new Matryoshka({ hello: 'world' });
		...
		// Matryoshka nest.
		assert(h.getValue().hello instanceof Matryoshka);

		// The base values should still be values.
		assert.strictEqual(h.getValue().hello.getValue(), 'world');
	...
	it('should nest objects.', function (done) {
		...
		var matryoshka = Matryoshka(value);
	...
	it('merging two objects should preserve the source of the first and values of the second overwrite.', function (done) {
		var a = { key: 'a' };
		var b = { key: 'b' };
