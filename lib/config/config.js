...
/**
 * Set some module default configuration from code. This is not recommended in favour of a config
 * file in the module directory (which will be loaded automatically).
 *
 * @param {String} modName      The module name.
 * @param {Object} configObject The configuration object to insert.
 * @param {String} modPath      The path of the file calling this (use module.filename).
 */

exports.setModuleDefault = function (modName, configObject, modPath) {
	var obj = { module: {} };
	obj.module[modName] = configObject;

	moduleConfig = Matryoshka.merge(moduleConfig, new Matryoshka(obj, modPath));
	regenerate();
};
...
exports.loadModuleConfig = function (modName, modPath) {
	...
	moduleConfig = Matryoshka.merge(moduleConfig, new Matryoshka(obj, loaded.source));
	regenerate();
};
...
exports.get = function (modPath, alt) {
	return aggregate.get(safePath(modPath), alt);
};
...
exports.getSource = function (modPath) {
	return aggregate.getSourceWithPath(safePath(modPath));
};
...
exports.initialize = function (logObj) {
	...
	var userConf = loadConfig(configDir, process.env.NODE_ENV, true, true);
	...
	case 'false':
		...
		break;
	default:
		// If something is set for some reason, turn everything not true into a false.
		if (userConf.config.developmentMode !== true) {
			userConf.config.developmentMode = false;
		}
	}
