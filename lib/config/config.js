...
// node 0.8 compatibility layer
var compat = require('../mage/compat');
...
function loadConfigFile(configPath) {
	...
	if (extension === '.yaml') {
		...
		return jsYaml.safeLoad(fs.readFileSync(configPath, compat.fileOptions({ encoding: 'utf8' })), options);
	}
	...
	if (extension === '.json') {
		try {
			...
			try {
				jsonlint.parse(fs.readFileSync(configPath, compat.fileOptions({ encoding: 'utf8' })));
			} catch (lintError) {
...
exports.initialize = function (logObj) {
	...
	var userConf = loadConfig(configDir, nodeEnv, true, true);
