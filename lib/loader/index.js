...
Loader.prototype.configure = function (cfg) {
	...
	// set up the cache with this configuration

	if (caches.LocalStorage.test()) {
		this.cache = new caches.LocalStorage(this.appName, this.language, this.density);
	}
};
...
Loader.prototype.loadPackage = function (packageName, cb) {
	...
	return this.cache.getMetaData(packageName, function (error, metaData) {
		if (error) {
		...
		return request(function (error, response) {
			...
			return Package.fromDownload(packageName, response.data, that.cache, function (error, pkg) {
				if (error) {
				...
				that.packages[packageName] = pkg;

				// emit "parsed", allowing post-processors to deal with the content

				that.emit(packageName + '.parsed', pkg);
				...
				that.emit(packageName + '.loaded', pkg);
