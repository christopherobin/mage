...
Loader.prototype.loadPackage = function (name, cb) {
	...
	return this.cache.getMetaData(name, function (error, metaData) {
		...
		try {
			req = http.createRequest(url, options);
		} catch (reqError) {
		...
		return req.send(function (error, response) {
			if (error) {
			...
			return Package.fromDownload(name, response.data, that.cache, function (error, pkg) {
				...
				// create DOM elements for HTML and CSS

				pkg.createHtml();
				pkg.createCss();

				that.emit(name + '.created', pkg);
				that.emit('created', pkg);
