...
Loader.prototype.configure = function (cfg) {
	...
	if (!this.clientConfig.language && this.languages[0]) {
		this.setLanguage(this.languages[0]);
	}

	if (!this.clientConfig.density) {
...
Loader.prototype.registerPackage = function (pkg) {
	...
	// if this is the first time we load this package, run the javascript part immediately
...
Loader.prototype.createPackageRequest = function (packageName) {
	...
	var language = this.clientConfig.language;
	var density = this.clientConfig.density;
	screen = (screen.width || '0') + 'x' + (screen.height || '0');

	return new PackageRequest(this.appName, packageName, language, screen, density);
};
	...
	return cache.getMetaData(pkgRequest, function (error, metaData) {
		...
		return httpRequest(function (error, response) {
			...
			if (response.contentType !== packageType) {
				emitWarning(new LoadError(
					'Downloaded package is not ' + packageType + ' (found: ' + response.contentType + ')',
					response
				));
			}
