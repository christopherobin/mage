...
function renderJs(pageName, content) {
	...
	if (content) {
		// Don't use innerHTML here, IE9 will explode parsing the contents.
		cnt.textContent = content;
	}
...
Loader.prototype.loadPage = function (name) {
	...
	function onReadyStateChange() {
		if ((xhr.readyState && xhr.readyState !== 4) || aborting) {
			return;
		}
		...
		mimetype = xhr.contentType || xhr.getResponseHeader('content-type');
	...
	var url = mageConfig.clientHostBaseUrl + '/app/' + appName + '/' + name + '?language=' + language + '&screen=' + screen + '&density=' + density;
