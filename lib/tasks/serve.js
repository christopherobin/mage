...
exports.shutdown = function (mage, options, cb) {
	function shutdownProcessManager(callback) {
		if (mage.core.processManager) {
			return mage.core.processManager.quit(callback);
		}
		callback();
	}

	function shutdownArchivist(callback) {
		mage.core.archivist.closeVaults();
		callback();
	}

	function shutdownServiceDiscovery(callback) {
		require('../serviceDiscovery').shutdown(callback);
	}
