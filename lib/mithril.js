...
Mithril.prototype.quit = function (graceful, returnCode) {
	...
	datasources.close(function () {
		...
		if (pm) {
			pm.quit(graceful, function (err) {
				if (err) {
					logger.error('Mithril did not shutdown properly (error code:', err + ')');
				}


				if (!mithril.core.processManager.isWorker) {
					daemon.unlock(returnCode ? returnCode : err);
				}

				logger.notice('Mithril shutdown completed, exiting process');
				process.exit(returnCode ? returnCode : err);
			});
		} else {
...
Mithril.prototype.setup = function (pathConfig, cb) {
	...
	var pm = this.core.processManager = require('./processManager');

	pm.on('workers.leftToSpawn', function (count) {
		logger.info(count, 'workers left to spawn');
	});

	pm.start(function () {

		that.core.msgServer.setup();

		// the master process shouldn't do anything beyond this point

		if (pm.isMaster) {
			// master has nothing left to do!
			// we do not call cb on the master
			logger.info('All workers are now up and listening');
			return daemon.lock(0);
		}

		// set up the modules and call cb: the game may now start

		that.setupModules(cb);
	});
};
