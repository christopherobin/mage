...
exports.core = {
	...
	LivePropertyMap:   require('./livePropertyMap').LivePropertyMap,
...
exports.start = function () {
	...
	httpServer = exports.core.httpServer = http.createServer(function (request, response) {
		...
		var url = exports.core.helpers.strChunks(request.url, '?', 2);
		var path = url[0];
		var params = url[1];
		...
		if (params) {
			...
			for (i = 0, len = params.length; i < len; i++) {
				var p = exports.core.helpers.strChunks(params[i], '=', 2);
				if (p.length === 2) {
	...
	try {
		if (bind.unix) {
			httpServer.listen(bind.unix, function () {
				exports.core.logger.info('Server running at ' + httpServer.type + '://' + bind.unix + ', exposed through http://' + expose.host + ':' + expose.port + '/');
				msgServer.start(httpServer);
			});
		} else {
			httpServer.listen(bind.port, bind.host, function () {
				exports.core.logger.info('Server running at ' + httpServer.type + '://' + bind.host + ':' + bind.port + ', exposed through http://' + expose.host + ':' + expose.port + '/');
				msgServer.start(httpServer);
			});
		}
	} catch (listenError) {
		console.error('HTTP server listen() failed:', listenError.message);
		process.exit(1);
	}
};
...
process.on('SIGINT', exports.quit);
process.on('SIGHUP', exports.quit);
process.on('SIGTERM', exports.quit);
