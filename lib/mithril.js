...
var quit = mithril.quit = function (graceful, returnCode) {
	...
	logger.mithril_debug('Shutting down Mithril...');
...
function setupLogger() {
	...
	if (config.get('debug') || config.get('logging.show')) {
		var example = {
            "logging": {
                "terminal": {
                    "channels": [">=info"],
                ...
                "socket" : {
                    "does not take a channel argument" : null,
                    "config": {
                        "path": "/path/needed/for/socket/logging/system/internal/socket/files",
                        "port": 31337,
                ...
                "loggly": {
                    "channels": [">=info"],
                    "config": {
                        "token": "token, see loggly indication on web interface account loging",
                        "subdomain": "subdomain"
                    }
                }
            }
		...
		logger.emergency('Configuring the logger through "debug" or "logging > show" is now invalid.')
				.details('An example of an accurate configuration is available in additional data')
				.details('Please take note that some of those variables are optional; you may set only one of those logger if you want')
				.data(example);

		mithril.fatalError('Configuration is not up to date, exitting...');
	}
...
mithril.setup = function (pathConfig, cb) {
	...
	modules.forEach(function (info) {
		...
		logger.mithril_debug('Exposing module ' + name);
	...
	function setupModule(state, name, cb) {
		...
		if (mod.setup) {
			logger.mithril_debug('Setting up module ' + name);
