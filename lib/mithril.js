...
function loadConfigFiles(pathConfigs) {
	var len = pathConfigs.length;

	if (len === 0) {
		fatalError('No configuration paths provided.');
	}

	var config = loadConfigFile(pathConfigs[0]);

	for (var i = 1; i < len; i++) {
		var data = loadConfigFile(pathConfigs[i]);
...
exports.setup = function (pathConfig, cb) {
	...
	if (Array.isArray(pathConfig)) {
		config = loadConfigFiles(pathConfig);
	} else {
		config = loadConfigFile(pathConfig);
	}
