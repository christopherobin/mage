...
var quit = mithril.quit = function (graceful, returnCode) {
	mithril.core.logger.info('Shutting down Mithril...');
	...
	getClusterManager().quit(graceful, returnCode);
};
...
mithril.setup = function (pathConfig, cb) {
	...
	// start cluster, if required
	if (getClusterManager().start()) {
		// cluster started, so the master should after initializing master-to-master comm
		msgServer.startClusterCommunication();
		return;
	}
