...
Mithril.prototype.setup = function (pathConfig, cb) {
	...
	function setupConfig(callback) {
		try {
			...
			that.core.logger.emergency(error);
	...
	function setupLogging(callback) {
		try {
			...
			that.core.logger.emergency('Fatal configuration error:', error);
			return callback(error);
		}
	...
	function startProcessManager(callback) {
		that.core.processManager.start();
		callback();
	}
	...
	function setupModules(callback) {
		...
		that.setupModules(function (error) {
			if (error) {
				that.core.logger.notice('Mithril setup failed.');
				return callback(error);
			}

			that.core.logger.notice('Mithril setup complete.');
			callback();
		});
	}
