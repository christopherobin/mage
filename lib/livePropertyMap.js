...
LivePropertyMap.prototype.preload = function (cb) {
	// if this.preload is not true-ish, or an empty array, we do not preload any properties
	...
	var db = this.state.datasources.db;
	var cfg = this.config;

	var where = [];
	var params = [];

	for (var key in cfg.fixedValues) {
		var value = cfg.fixedValues[key];
	...
	if (!this.options.allLanguages) {
		where.push('language IN (?, ?)');
		params.push('', this.state.language());
	}
	...
	var sql = 'SELECT * FROM ' + cfg.tableName;
