...
LivePropertyMap.prototype.set = function (propertyName, value, language, tag, ttl) {
	...
	kv.set(this.config.domain + '/data/' + key, JSON.stringify(value), ttl);
...
LivePropertyMap.prototype.del = function (propertyName, language, tag) {
	...
	kv.del(this.config.domain + '/data/' + key);
	...
	if (index !== -1) {
		...
		if (this.propertyList.length > 0) {
			...
			kv.del(this.config.domain + '/datakeys');
		}
	}
};


LivePropertyMap.prototype.delAll = function () {
	// deletes all properties that are mentioned in the property list

	var kv = this.state.datasources.kv;

	for (var i = 0, len = this.propertyList.length; i < len; i++) {
		kv.del(this.config.domain + '/data/' + this.propertyList[i]);
	}
...
LivePropertyMap.prototype.importFromStaticPropertyMap = function (srcMap, language, tags, fnFilter) {
	...
	for (var propertyName in map) {
		...
		for (var i = 0, len = property.length; i < len; i++) {
			...
			this.set(propertyName, obj.value, obj.language, obj.tag);
		}
	}
};
