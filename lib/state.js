...
State.prototype.emitEvents = function (cb) {
	...
	function (error, result) {
		...
		for (var i = 0, len = addresses.length; i < len; i++) {
			...
			comm.send(address.addrName, address.host, actorEvents);
		}
...
State.prototype.emit = function (actorId, path, data, language) {
	...
	if (actorId === this.actorId && (!language || language === this.language())) {
...
State.prototype.emitToActors = function (actorIds, path, data, language) {
	...
	for (var i = 0, len = actorIds.length; i < len; i++) {
		...
		if (done.indexOf(actorId) === -1) {
			this.emit(actorId, path, data, language);
			done.push(actorId);
		}
	}
};
