...
State.prototype.emitToMany = function (filter, path, data, cb) {
	// TODO: if filter.actorIds is this.actorId, we can skip it from the select
	// which if filter.actorIds contains nothing else, that we don't have to select anything
	// we'll definitely still want to do the language check on this.language() / filter.language

	// TODO: this function has to become a queuer, do not check for languages yet, just queue it up
	// on commit, when we really do the event emission to other players, is when we should do this read
	// that read may then also be on a separate state object, so that we avoid write transaction
	// the cb can then also be removed from this function
	// in fact, our own commit should not depend on the successful completion of this operation.
...
State.prototype.close = function (queryId) {
	...
	if (this.sysErrorCode) {
		...
		if (this.myEvents.length > 0) {
			response += ',' + JSON.stringify(this.myEvents);
		}
