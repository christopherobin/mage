...
function createStateTimeout(state, timeout) {
	state.timeout = setTimeout(function () {
		state.error(null, 'timeout: ' + state.getDescription(), function () {
			state.close();
		});
	}, timeout);
}
...
function State(actorId, session) {
	...
	// create timeout from config or default to 3 secs

	var cfg      = mithril.core.config.get('state');
	var timeout  = cfg && cfg.defaultTimeout || 3000;

	this.timeout = null;

	createStateTimeout(this, timeout);
}
