...
exports.getStateCount = function () {
	return stateCount;
};
exports.getSqlReadTimeOut = function () {
	return sqlReadTimeOut;
};
exports.getSqlWriteTimeOut = function () {
	return sqlWriteTimeOut;
};
exports.getSqlLockWaitTimeOut = function () {
	return sqlLockWaitTimeOut;
};
exports.getSqlMiscError = function () {
	return sqlMiscError;
};
...
State.prototype.error = function (code, logDetails, cb) {
	...
	if (logDetails && logDetails.sql) { // For logging MySQL timeouts with the sampler.
		switch (logDetails.error.number) {
		case 1159:
			sqlReadTimeOut++;
			break;
		case 1161:
...
State.prototype.userError = function (code, cb) {
	logger.info('state.userError() is deprecated. From now on, please use state.error(code, null, cb) if you do not want to send anything to the error log.');
...
State.prototype.close = function (cb) {
	...
	logger.debug('Closing state:', this.getDescription());
