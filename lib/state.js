...
State.prototype.userError = function (code, cb) {
	...
	mithril.core.logger.debug('User error: ' + code);
...
function cacheCommandResponse(session, queryId, responseJson, options, cb) {
	var data = responseJson + '\t' + JSON.stringify(options);

	session.set('cmd' + queryId, commandResponseTTL, data, cb);
