...
function State(actorId, session, msgClient, callbackId) {
...
State.prototype.commit = function (cb) {
	...
	if (this.msgClient) {
		for (i = 0, len = this.myEvents.length; i < len; i++) {
			...
			this.msgClient.emit(evt[0], evt[1]);
		}

		if (this.callbackId) {
			this.msgClient.respond(this.callbackId, this.response);
		}
...
State.prototype.rollBack = function (cb) {
	if (this.callbackId) {
		if (this.msgClient) {
			this.msgClient.respond(this.callbackId, null, this.errorCode);
			this.msgClient.send();
		} else {
			// log the response
		}
