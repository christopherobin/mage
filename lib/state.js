...
State.prototype.error = function (code, logDetails, cb) {
	...
	if (logDetails) {
		...
		if (this.actorId) {
			logger.debug('Actor' + this.actorId, 'error code:', code);
		} else {
	...
	if (cb && typeof cb === 'function') {
		...
		logger.error('state.error() got called without a callback (' + this.description + ')');
	}
};
...
State.prototype.close = function (cb) {
	logger.debug('Closing state:', this.description);
...
State.prototype.destroy = function () {
	if (this.datasources) {
		...
		logger.error('Trying to destroy an already destroyed state object (' + this.description + ').');
	}
