...
Mage.prototype.setupModules = function (cb) {
	...
	for (var i = 0, len = this._modulesList.length; i < len; i++) {
		...
		// Try to resolve the default configuration, and parse/load it if found.

		var configPath = '';
		var defaultConf = null;

		try {
			configPath = require.resolve(path.join(modPath, 'default'));
		} catch (e) {}
		...
		case '.json':
			...
			defaultConf = jsonLoad(raw);
			break;
		case '.yaml':
			// The config is yaml, so load and parse with js-yaml in strict mode.
			var yamlLoad = require('js-yaml', true).load;
			defaultConf = yamlLoad(fs.readFileSync(configPath, 'utf8'));
			break;
		default:
			// The extension was not a known type. If there was a config path, then throw an
			// error because this was an unknown type.
			if (configPath) {
				throw new Error('Default configuration for module ' + modName + ' is not a valid type: ' + extension);
			}
		}

		if (defaultConf) {
			this.core.config.setModuleDefault(modName, defaultConf);
		}
