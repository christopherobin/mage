...
Mage.prototype.setupCoreLibs = function () {
	...
	core.config = require('../config').initialize(core.logger);
...
Mage.prototype.addModule = function (name, modPath) {
	...
	this._modulesList.push({ name: name, path: modPath });
};
...
Mage.prototype.setup = function (cb) {
	...
	function setupSavvy(callback) {
		that.core.savvy.setup(callback);
	}
...
Mage.prototype.start = function (cb) {
	...
	function createDashboards(callback) {
		if (!that.dashboard) {
			return callback();
		}

		var appNames = ['support', 'cms'];

		async.forEachSeries(
			appNames,
			function (appName, callback) {
				that.dashboard.createDashboard(appName, callback);
			},
			callback
		);
	}
	...
	function startClientHost(callback) {
		that.core.msgServer.startClientHost(callback);
	}
...
Mage.prototype.isDevelopmentMode = function () {
	return this.core.config.developmentMode;
};
