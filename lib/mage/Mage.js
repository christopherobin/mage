 ...
 *
 * @param {Object}   api
 * @param {Function} api.setup
 * @param {Function} api.start
 * @param {*}        [options]  An options argument to pass on the task's setup and start functions
 */

Mage.prototype.setTask = function (api, options) {
	function noop(mage, options, cb) {
		cb();
	}

	this.task = {
		setup: api.setup || noop,
		start: api.start || noop,
		options: options
	};
};
...
Mage.prototype.setup = function (cb) {
	...
	if (!this.task) {
		this.setTask(require('../tasks/serve'), null);
	}

	this.setRunState('setup');

	this.task.setup(this, this.task.options, function (error, options) {
		// Errors are fatal
...
Mage.prototype.start = function (cb) {
	...
	this.task.start(this, this.task.options, function (error, options) {
		if (error) {
			return that.fatalError();
		}
