...
function Mage(config) {
	...
	try {
		...
		// If we do not run from the game directory we silently fail to parse
		// the package.json file. If we figure out a reliable way to read the
		// package.json file in the future, we will need to update the check
		// in cli/index.js to skip parsing command line options.
	}
