...
// All panopticon instances listen to this emitter for data and ultimately a stop signal.
var panopticaAlert = new EventEmitter();

// Remove the maximum listeners limit.
panopticaAlert.setMaxListeners(0);
 ...
 * @param {Object} mysql         The mysql module.
 * @param {Object} datasources   The datasources module.
 * @param {Object} commandCenter The command center module.
 */

exports.initialize = function (mageInstance, mageLogger, procManager, state, mysql, datasources, commandCenter) {
	mage = mageInstance;
	logger = mageLogger;
	processManager = procManager;

	mageSamplers.initialize(procManager, state, mysql, datasources, commandCenter);
};
...
function exposePanopticonMethod(method) {
	exports[method] = function () {
		panopticaAlert.emit(method, arguments);
	};
}
...
exports.registerMethod = function (name, loggerConstructor, validator) {
	...
	// Tell existing panoptica that a new method exists and to listen to panopticaAlert for it.
	panopticaAlert.emit('newLoggerMethod', name);
...
function delivery(data) {
	var name = data.name;
	var bufferLength = mage.core.config.get(['sampler', 'bufferLength'], 1000);
...
function websocketHandler(connection) {
	...
	connection.once('close', function () {
		logger.debug('Websocket connection for statistics was closed.');
...
function startServing(cb) {
	// Workers don't serve sampler data.
	if (processManager.isWorker) {
		return cb();
	}

	// A route for Observium and other services to query the gathered data with an HTTP request.
	mage.core.savvy.addRoute('/sampler', requestResponseHandler);

	// A websocket for communication with the dashboard.
	var socketServer = mage.core.savvy.addWebSocketRoute('/statistics');

	socketServer.on('connection', websocketHandler);

	// Close the websocket server. This closes all client connections.
...
exports.setup = function (cb) {
	...
	var names = Object.keys(init.intervals);
	...
	var allFinite = names.every(function (name) {
	...
	// Construct panoptica and register them as listeners to panopticaAlert.
	names.forEach(function (name) {
		...
		function methodListenClosure(method) {
			panopticaAlert.on(method, function () {
				panopticon[method].apply(panopticon, arguments);
			});
		}

		// Listen for known logger methods.
		panopticonMethods.forEach(methodListenClosure);

		// Add a listener if a new logger method gets added.
		panopticaAlert.on('newLoggerMethod', methodListenClosure);

		// Add a listener for stopping the panopticon.
		panopticaAlert.once('stop', function () {
			panopticon.stop();
		});
	});

	// Each panopticon involves node timers. Upon MAGE shutdown these need to be dealt with. Each
	// panopticon *could* individually listen to mage, but this would add a lot of new listeners. By
	// listening with panopticaAlert we add only one listener to mage.
	mage.once('shutdown', function () {
		panopticaAlert.emit('stop');
		panopticaAlert.removeAllListeners();
