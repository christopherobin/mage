...
function requestResponseHandler(req, res, path) {
	...
	if (data === undefined) {
		var available = (gatheredData && Object.keys(gatheredData)) || ['none'];

		logger.verbose('Could not find path "' + path + '" in sampler. Available:', available);

		res.writeHead(404, { 'content-type': 'text/plain' });
		res.end('Available: ' + available.join(', '));
		return;
	}
 ...
 *
 * @param {ws.WebSocket} client A websocket connection object.
 * @param {Object} urlInfo      A urlinfo object from the HTTP request.
 */

function websocketHandler(client, urlInfo) {
	logger.debug('Websocket connection for sampler was opened.');
	...
	function safeSend(data) {
		...
		try {
			...
			logger.error('Error sending data to websocket client:', error);
		}
	}

	function deliver(delivery) {
		try {
			// dig into the data set and serialize

			delivery = JSON.stringify(exports.query(queryPath, delivery));
		} catch (error) {
			logger.error('Error serializing sampler data:', error);
			return;
		}
	...
	try {
		queryPath = pathToQuery(urlInfo.pathname);
	} catch (error) {
		safeSend('Failed to parse query path: ' + urlInfo.pathname);
		return;
	}

	// Deliver new data as it trickles in

	exports.on('panopticonDelivery', deliver);

	client.once('close', function () {
		exports.removeListener('panopticonDelivery', deliver);
	});

	// The client may send a command to extract the buffered deliveries
	...
	client.on('message', function (message) {
		...
		if (message === 'sendBufferedData') {
			// deliver everything in the buffer that matches the query

			bufferedData.forEach(deliver);
		} else {
			safeSend('Unknown command: ' + message);
		}
	});
}
...
exports.expose = function (cb) {
	...
	mage.core.savvy.addRoute(SAVVY_ROUTE, websocketHandler, 'websocket');
