...
function urlToPath(urlString) {
	...
	if (typeof parsed.pathname !== 'string') {
		throw new Error('No path in URL: ' + parsed.pathname);
	}
...
function requestResponseHandler(req, res) {
	...
	var data = exports.query(path);

	if (data === undefined) {
		logger.verbose('Could not find path in sampler:', path);
		res.writeHead(404);
		res.end();
		return;
	}
...
function websocketHandler(client) {
	...
	function sendToClient(dataSet) {
		...
		client.send(toSend, closeOnError);
	}
