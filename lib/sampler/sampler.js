...
exports.initialize = function (mageInstance, mageLogger, procManager, state, commandCenter, archivist) {
	mage = mageInstance;
	logger = mageLogger;
	processManager = procManager;

	mageSamplers.initialize(procManager, state, commandCenter, archivist);
};
...
function exposePanopticonMethod(method) {
	var panopticonIds = Object.keys(panoptica);

	exports[method] = function () {
		for (var i = 0; i < panopticonIds.length; i++) {
			var panopticon = panoptica[panopticonIds[i]];
...
/**
 * Function which writes individual samples to a selected vault.
 * 
 * As it is preferable that samples are stored individually, we need to iterate over the samples and
 * save them as such. However panopticon stores samples in a nested object based on paths. Thus we
 * need to walk these paths to extract individual samples.
 * 
 * @param {type} topic - topic which the samples will be persisted
 * @param {Object} deliveredSamples - Aggregated object given by panopticon
 */
function persistToTopic(topic, deliveredSamples) {
	var interval = deliveredSamples.interval;

	var state = new State();

	function processPaths(data, path) {
		for (var key in data) {
			var currentPos = data[key];
			var keyPath = (path) ? path + '/' + key : key;

			if (currentPos.type && currentPos.values) {
				for (var valueId in currentPos.values) {
					var writeKey = {
						path: keyPath,
						valueId: valueId,
						timeStamp: currentPos.values[valueId].timeStamp
					};
					...
					var writeObject = {
						...
						interval: interval,
						dateTime: (new Date(currentPos.values[valueId].timeStamp)).toJSON(),
						data: currentPos.values[valueId]
					};

					state.archivist.set(topic, writeKey, writeObject);
				}
			} else {
				processPaths(currentPos, keyPath);
			}
		}
	}

	processPaths(deliveredSamples.data);
...
exports.setup = function (cb) {
	...
	intervalNames.forEach(function (name) {
		...
		// If the configuration indicates that samples should be persisted to a topic,
		// then pass deliveries to persistence function.
		var sampleTopic = mage.core.config.get(['sampler', 'persist', name]);

		if (sampleTopic) {
			mage.core.archivist.assertTopicAbilities(sampleTopic, ['path', 'valueId', 'timeStamp'], ['set']);

			panopticon.on('delivery', function (samples) {
				persistToTopic(sampleTopic, samples);
			});
		}
	...
	mage.once('shutdown', function () {
		for (var panopticonId in panoptica) {
			panoptica[panopticonId].stop();
		}
