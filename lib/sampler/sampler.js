...
function persistToTopic(deliveredSamples) {
	...
	function processPaths(data, path) {
		for (var key in data) {
			...
			if (data[key].type && data[key].values) {
				for (var valueId in data[key].values) {
					var writeKey = {
						path: keyPath,
						valueId: valueId,
						timeStamp: data[key].values[valueId].timeStamp
					};
					...
					var writeObject = {
						...
						type: data[key].type,
						interval: interval,
						dateTime: (new Date(data[key].values[valueId].timeStamp)).toJSON(),
						data: data[key].values[valueId]
					};

					state.archivist.set(topic, writeKey, writeObject);
				}
			} else {
				processPaths(data[key], keyPath);
			}
		}
	}
