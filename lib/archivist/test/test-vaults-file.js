...
describe('File creation through core API', function () {
	...
	it('should not fail', function (done) {
		...
		fileVault.add(testFilePath, testFileData, 0, function (error) {
			assert.ifError(error, 'FileVault#add returned an error');

			assert.deepEqual(testFileData.content, fs.readFileSync(absPath(testFilePath + testFileData.meta.ext)), 'Write/Read equality mismatch');

			done();
		});
	});

	it('should fail if attempting to create twice', function (done) {
		var fileVault = createVault();
...
describe('File overwriting through core API', function () {
	...
	it('should not fail', function (done) {
		...
		fileVault.set(testFilePath, testFileDataB, 0, function (error) {
...
describe('Listing files through core API', function () {
	...
	it('should not fail', function (done) {
		...
		fileVault.scan(map, function (error, results) {
			...
			assert.strictEqual(results.length, testFilePaths.length, 'Incorrect amount of results found, found: ' + JSON.stringify(results) + ' instead of ' + testFilePaths.length);
