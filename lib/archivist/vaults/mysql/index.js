...
Archive.prototype.listIndexes = function (api, topic, partialIndex, cb) {
	var partialKey = api.key(topic, partialIndex);
	var cols = Object.keys(partialKey.pk);

	var where = {};
	for (var i = 0; i < cols.length; i++) {
		if (partialKey.pk[cols[i]] !== null) {
			where[cols[i]] = partialKey.pk[cols[i]];
		}
	}

	this.vault.select(partialKey.table, cols, where, function (error, rows) {
		if (error) {
			return cb(error);
		}

		var indexes = rows.map(function (row) {
			return api.parseKey({ table: partialKey.table, pk: row });
		});
...
Archive.prototype.get = function (api, value, cb) {
	var key = api.key(value.topic, value.index);

	this.vault.select(key.table, null, key.pk, function (error, result) {
		if (error) {
			return cb(error);
		}
...
MysqlVault.prototype.select = function (table, cols, where, cb) {
	var columns;
