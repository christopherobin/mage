...
MysqlVault.prototype.dropDatabase = function (cb) {
	...
	// Gracefully close all existing connections on the current pool before dropping th database.
	var that = this;
...
/**
 * Creates a table for a given topic name, with given indexes and value type.
 *
 * @param {String} topicName - Name of the topic which is also the name of the table
 * @param {Array} indexes - Array of objects corresponding to the index columns ({ name: columnName, type: fieldType })
 * @param {String} valueType - The value type, usually TEXT for string data, and BLOB for binary data
 * @param {Function} cb
 */

MysqlVault.prototype.createTable = function (topicName, indexes, valueType, cb) {
	var indexString = '';
	var indexNames = [];
	for (var i = 0; i < indexes.length; i += 1) {
		var index = indexes[i];
		indexNames.push(index.name);
		indexString += '  ' + mysql.escapeId(index.name) + ' ' + index.type + ',\n';
	}

	var sql =
		'CREATE TABLE ' + mysql.escapeId(topicName) + ' (\n' +
		indexString +
...
/**
 * Drops a topic table (Use with caution!)
 *
 * @param {String} topicName - Name of the topic which is also the name of the table
 * @param {Function} cb
 */

MysqlVault.prototype.dropTable = function (topicName, cb) {
	var sql = 'DROP TABLE ' + mysql.escapeId(topicName);
