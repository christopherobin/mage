...
//var sqlite = requirePeer('sqlite3');
...
function SQLiteVault(name, logger) {
...
exports.create = function (name, logger) {
	return new SQLiteVault(name, logger);
};


SQLiteVault.prototype.setup = function (cfg, cb) {
	this.sqlite = sqlite;
	this.config = cfg;
	...
	this.db = new this.sqlite.Database(this.config.filename);

	setImmediate(cb);
};


SQLiteVault.prototype.close = function (cb) {
...
SQLiteVault.prototype.createDatabase = function (cb) {
	var cfg = this.config;
	if (!cfg) {
	...
	this.db = new this.sqlite.Database(cfg.filename, null, cb);
};
...
SQLiteVault.prototype.dropDatabase = function (cb) {
	var cfg = this.config;
	if (!cfg) {
		return cb(new Error('No configuration available for vault ' + this.name));
	}
 ...
 */
SQLiteVault.prototype.getMigrations = function (cb) {
...
SQLiteVault.prototype.registerMigration = function (version, report, cb) {
	var values = {
		version: version,
...
SQLiteVault.prototype.unregisterMigration = function (version, cb) {
	var where = {
		version: version
	};
...
SQLiteVault.prototype.select = function (table, cols, where, order, limit, cb) {
	var columns, i, len;
...
SQLiteVault.prototype.insert = function (table, values, cb) {
...
SQLiteVault.prototype.update = function (table, values, where, cb) {
...
SQLiteVault.prototype.updateOrInsert = function (table, pk, values, cb) {
	...
	for (var i = 0; i < cols.length; i++) {
		colName = cols[i];
		update.push(colName + ' = VALUES(' + colName + ')');
	}
...
SQLiteVault.prototype.del = function (table, where, cb) {
