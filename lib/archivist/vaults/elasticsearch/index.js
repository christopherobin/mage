 ...
 * @param {string}   cfg.server          A server objects that matches the one used by node-elasticsearch
...
ElasticsearchVault.prototype.setup = function (cfg, cb) {
	if (!cfg.index) {
		return cb(new Error('No default index specified for Elasticsearch vault'));
	}
	...
	try {
		...
		this.logger.emergency('Could not load elasticsearch. Make sure your npm modules are up to date.');
		return process.nextTick(function () {
			cb(error);
		});
	}

	// instanciate the elasticsearch object, providing it with the server info, and the default index
 ...
 * @param {string}      [target._index] The index, by default uses the one provided in the configuration
 * @param {string}      target._type    The type of data you want to store, the createTarget API sets it to the topic name
 ...
 * @param {string}      [shard]         If is is a string, allows you to override how Elasticsearch will shard stuff
 ...
 * @param {string}      target._type    The type of data you want to store, the createTarget API sets it to the topic name
 * @param {string}      target._id      The id for the document, should be a valid path component
 * @param {string}      [shard]         If is is a string, allows you to override which shard to get the document from
 ...
 * @param {string}      target._type    The type of data you want to store, the createTarget API sets it to the topic name
 * @param {string}      target._id      The id for the document, should be a valid path component
 * @param {string}      [shard]         The shard where to delete the document
