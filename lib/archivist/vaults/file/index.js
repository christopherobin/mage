...
exports.defaultValueHandlers = {
	...
	deserialize: function (data, value) {
		...
		if (index < 1) {
			throw new Error('Could not find header in binary data');
		}

		var header = data.slice(0, index).toString();

		// format with embedded expiration time:

		var m = header.match(/^\s*([a-fA-F0-9]{8})\s+(.+)$/);
		if (!m) {
			throw new Error('Invalid header, expected: 8-hex chars timestamp, whitespace, mediaType');
		}

		// extract expirationTime and mediaType

		var expirationTime = parseInt(m[1], 16);
		var mediaType = m[2].trim();
...
FileVault.prototype.checkExpirations = function (cb) {
	...
	function checkExpiration(path, callback) {
		var ext = pathExtName(path);

		if (ext !== FILE_EXTENSION) {
			that.logger.warning('Ignoring file', path, 'in vault:', that.name, '(filtering on ' + FILE_EXTENSION + ')');
			return callback();
		}
		...
		fs.open(filePath, 'r', function (error, fd) {
			...
			fs.read(fd, expirationBuffer, 0, 8, 0, function (error, num) {
				...
				if (num < 8) {
					...
					that.logger.warning('Skipping expiration because header is too short:', filePath);
					return callback();
				}
				...
				if (!expirationTime) {
					that.logger.verbose('File never expires:', filePath);
					return callback();
				}

				if (expirationTime > now) {
					that.logger.verbose('File will expire in', (expirationTime - now), 'seconds:', filePath);
					return callback();
				}
