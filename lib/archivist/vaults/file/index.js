...
function createKeyFolders(key, fullpath, cb) {
	...
	if (key.indexOf(pathSeperator) <= 0) {
		return cb();
	}

	mkdirp(pathDirName(fullpath), cb);
}
...
FileVault.prototype.addMeta = function (filenameWithoutExt, meta, cb) {
	...
	// Check if the key contains sub-folders. If so, ensure they exist before writting
	createKeyFolders(filenameWithoutExt, path, function (error) {
		if (error) {
			return cb(error);
		}
 ...
 *
 * This will also delete empty sub folders if the 'deleteEmptyDir' is provided. This will only
 * removed sub folders and not the root folder itself.
 *
 * @param {Function} rootPath
 * @param {Boolean} deleteEmptyDir
 * @param {Function} cb
 */
function recursiveFileList(rootPath, deleteEmptyDir, cb) {
	var fileList = [];
	...
	fs.readdir(rootPath, function (error, files) {
		...
		async.eachSeries(files, function (filename, callback) {
			...
			fs.stat(filePath, function (error, stat) {
				...
				// Otherwise recurse
				recursiveFileList(filePath, ((deleteEmptyDir) ? 2 : false), function (error, childFileList) {
					if (error) {
						return callback(error);
					}
			...
			// Check if we don't need to purge
			if (fileList.length > 0 || deleteEmptyDir !== 2) {
				return cb(null, fileList);
			}

			// Check if the folder is empty and we can delete it
			fs.rmdir(rootPath, function (error) {
				if (error) {
					return cb(error);
				}
...
FileVault.prototype.scan = function (map, deleteEmptyDir, cb) {
	var that = this;

	// Ensure cb is set if deleteEmptyDir no provided
	if (!cb) {
		if (typeof deleteEmptyDir !== 'function') {
			throw 'scan used without a valid callback';
		}
	...
	recursiveFileList(this.path, deleteEmptyDir, function (error, files) {
		if (error) {
...
FileVault.prototype._deleteEmptyFolders = function (folders, cb) {
	var that = this;
	var path = pathJoin(this.path, folders);
	...
	fs.rmdir(path, function (error) {
		...
		return that._deleteEmptyFolders(pathDirName(folders), cb);
	});
};
	...
	this.optMeta(filenameWithoutExt, function (error, meta) {
		...
		that.delMeta(filenameWithoutExt, function (error) {
			...
			that.delContent(filenameWithoutExt, meta.ext, function (error) {
				...
				// continue.
				that._deleteEmptyFolders(pathDirName(filenameWithoutExt), cb);
			});
		});
	});
};
...
FileVault.prototype.checkMeta = function (cb) {
	...
	this.scan(null, true, function (error, filenamesWithoutExt) {
		if (error) {
			return cb(error);
		}
