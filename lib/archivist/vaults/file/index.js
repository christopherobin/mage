...
FileVault.prototype.checkExtension = function (filenameWithoutExt, newExt, cb) {
	this.optMeta(filenameWithoutExt, function (error, meta) {
		if (error) {
			return cb(error);
		}
		...
		this.logger.debug(filenameWithoutExt, 'got a new extension', newExt, 'deleting old one.');
...
FileVault.prototype.set = function (filenameWithoutExt, data, cb) {
	...
	this.checkExtension(filenameWithoutExt, data.meta.ext, function () {
		// ignore errors
