 ...
 * will work its way backwards until it has reached the vault root path. This is a cleanup phase
 * after document deletion. If there is an error during directory removal, we just ignore it and
 * call the callback.
 *
 * @param {String} vault - vault to which the given subfolder is bound
 * @param {String} subfolderPath - subfolder path relative to vault root path
 * @param {Function} cb
 */

function purgeEmptyParentFolders(vault, subfolderPath, cb) {
	var fullPath = path.join(vault.path, subfolderPath);
	...
	fs.rmdir(fullPath, function (error) {
		...
		vault.logger.verbose('Purged empty vault subfolder:', fullPath);

		return purgeEmptyParentFolders(vault, path.dirname(subfolderPath), cb);
	});
}


/**
 * Function which scans all subdirectories for a given vault and attempts to delete any empty
 * subfolders. It will traverse to the deepest child of each branch and work backwards deleting any
 * empty folders. This is a cleanup phase during mage setup, and will happen after expirations of
 * existing data. If there is an error during directory removal, we just ignore it and call the
 * callback.
 *
 * @param {String} vault - vault whose subfolders should be scanned
 * @param {String} subfolderPath - internal recursion string (pass in null)
 * @param {Function} cb
 */

function purgeEmptySubFolders(vault, subfolderPath, cb) {
	...
	fs.readdir(fullPath, function (error, files) {
		...
		async.eachSeries(files, function (file, callback) {
			...
			fs.stat(filepath, function (error, stat) {
				...
				// Recurse inwards
				purgeEmptySubFolders(vault, path.join(subfolderPath, file), callback);
			});
		}, function () {
			// Do nothing if vault root path
			if (!subfolderPath) {
				return cb();
			}
	...
	this.optMeta(filenameWithoutExt, function (error, meta) {
		...
		that.delMeta(filenameWithoutExt, function (error) {
			...
			that.delContent(filenameWithoutExt, meta.ext, function (error) {
				...
				// continue.
				purgeEmptyParentFolders(that, path.dirname(filenameWithoutExt), cb);
			});
		});
	});
};
...
FileVault.prototype.checkMeta = function (cb) {
	...
	this.scan(null, function (error, filenamesWithoutExt) {
		...
		async.eachLimit(filenamesWithoutExt, EXPIRE_MAX_PARALLEL, that.optMeta.bind(that), function (error) {
			...
			purgeEmptySubFolders(that, null, cb);
		});
	});
};
