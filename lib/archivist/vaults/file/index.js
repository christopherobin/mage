...
FileVault.prototype._deleteEmptyParentFolders = function (childpath, cb) {
	var that = this;
	var path = pathJoin(this.path, childpath);
	...
	fs.rmdir(path, function (error) {
		...
		return that._deleteEmptyParentFolders(pathDirName(childpath), cb);
	});
};
	...
	this.optMeta(filenameWithoutExt, function (error, meta) {
		...
		that.delMeta(filenameWithoutExt, function (error) {
			...
			that.delContent(filenameWithoutExt, meta.ext, function (error) {
				...
				// continue.
				that._deleteEmptyParentFolders(pathDirName(filenameWithoutExt), cb);
			});
		});
	});
};
...
function purgeEmptySubFolders(rootpath, childpath, cb) {
	childpath = childpath || '';
	var path = pathJoin(rootpath, childpath);
	...
	fs.readdir(path, function (error, files) {
		...
		async.eachSeries(files, function (file, callback) {
			...
			fs.stat(filepath, function (error, stat) {
				...
				// Recurse inwards
				purgeEmptySubFolders(rootpath, pathJoin(childpath, file), callback);
			});
		}, function () {
