...
Archive.prototype.listIndexes = function (api, topic, partialIndex, cb) {
	...
	var check = Object.keys(partialIndex);

	function map(path) {
		try {
			return api.parseKey(path);
		} catch (error) {
			this.logger.warning(error);
		}
	}

	function filter(parsed) {
		...
		for (var i = 0; i < check.length; i++) {
			...
			if (parsedValue === undefined || (givenValue !== null && '' + parsedValue !== '' + givenValue)) {
				return false;
			}
		}
...
FileVault.prototype.scan = function (map, filter, cb) {
