...
var logger = mage.core.logger.context('mage-client-vault');
...
MageClientVault.prototype.write = function (actorIds, key, data, ttl, cb) {
	if (this.state) {
		...
		logger.verbose('Emitting "archivist:write" to', actorIds);
...
MageClientVault.prototype.writeDiff = function (actorIds, key, diff, ttl, cb) {
	if (this.state) {
		...
		logger.verbose('Emitting "archivist:writeDiff" to', actorIds);
...
MageClientVault.prototype.touch = function (actorIds, key, ttl, cb) {
	if (this.state) {
		...
		logger.verbose('Emitting "archivist:touch" to', actorIds);
...
MageClientVault.prototype.del = function (actorIds, key, cb) {
	if (this.state) {
		...
		logger.verbose('Emitting "archivist:del" to', actorIds);
