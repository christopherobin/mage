...
var logger = mage.core.logger.context('mysql-vault');
...
exports.defaultTopicApis = {
	...
	deserialize: function (row, value) {
		...
		value.initWithData(mediaType, data);  // let encoding be detected by the VaultValue
	},
	key: function (value) {
		return {
			table: value.topic,
...
Archive.prototype.create = function (api, value, cb) {
	...
	this.vault.insertOrUpdate(key.table, key.pk, values, cb);
};
...
function MysqlVault(name) {
	...
	this.uri = null;                   // the URI that connections will be established to
...
MysqlVault.prototype.connect = function () {
	...
	this.connection = this.mysql.createConnection(this.uri);

	this.connection.once('error', function (error) {
		if (error.code === 'PROTOCOL_CONNECTION_LOST') {
			logger.debug('Disconnection detected, reconnecting on next SQL statement.');
...
MysqlVault.prototype.select = function (table, where, cb) {
	...
	logger.verbose('Executing:', query, where);
...
MysqlVault.prototype.insertOrUpdate = function (table, pk, values, cb) {
