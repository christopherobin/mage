...
Archive.prototype.add = function (api, value, cb) {
	...
	this.vault.set(api.shard(value), api.createKey(value.topic, value.index), api.serialize(value), value.expirationTime, cb);
};
...
Archive.prototype.set = function (api, value, cb) {
	...
	if (diff) {
		this.vault.applyDiff(api.shard(value), api.createKey(value.topic, value.index), diff, value.expirationTime, cb);
	} else {
		this.vault.set(api.shard(value), api.createKey(value.topic, value.index), api.serialize(value), value.expirationTime, cb);
	}
};
...
Archive.prototype.touch = function (api, value, cb) {
	this.vault.touch(api.shard(value), api.createKey(value.topic, value.index), value.expirationTime, cb);
};


Archive.prototype.del = function (api, value, cb) {
	this.vault.del(api.shard(value), api.createKey(value.topic, value.index), cb);
};
