...
Archive.prototype.add = function (api, value, cb) {
	...
	this.vault.set(api.shard(value), api.key(value), api.serialize(value), value.expirationTime, cb);
};
...
Archive.prototype.set = function (api, value, cb) {
	...
	if (diff) {
		this.vault.applyDiff(api.shard(value), api.key(value), diff, value.expirationTime, cb);
	} else {
		this.vault.set(api.shard(value), api.key(value), api.serialize(value), value.expirationTime, cb);
	}
};
