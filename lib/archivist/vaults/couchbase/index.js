...
CouchbaseVault.prototype.getMigrations = function (cb) {
	...
	self.logger.debug('Loading applied migrations list');

	self.get('schema_migrations', {}, function (error, value) {
		if (error) {
			return cb(error);
		}
...
CouchbaseVault.prototype.registerMigration = function (version, report, cb) {
	...
	self.getMigrations(function (error, versions) {
		if (error) {
			return cb(error);
		}
		...
		self.set('schema_migrations', versions, null, cb);
	});
};
...
CouchbaseVault.prototype.unregisterMigration = function (version, cb) {
	...
	self.getMigrations(function (error, versions) {
		...
		// Find the version in the versions data
		var foundId = null;
		for (var i = 0; i < versions.length; i++) {
			var versionData = versions[i];
			if (versionData.version === version) {
