...
CouchbaseVault.prototype.createDatabase = function (cb) {
	...
	clusterManager.listBuckets(function (error, buckets) {
		...
		async.whilst(function () {
			...
			clusterManager.createBucket(bucketOptions.bucket, createBucket, function (error) {
				if (error) {
				...
				async.series([
					couchbaseHelpers.waitForSet.bind(null, logger, cluster, bucketOptions),
					couchbaseHelpers.waitForGet.bind(null, logger, cluster, bucketOptions),
					couchbaseHelpers.waitForN1QL.bind(null, logger, cluster, bucketOptions),
					couchbaseHelpers.waitForDel.bind(null, logger, cluster, bucketOptions)
				], function (error) {
					if (error) {
						if (error.code === 11 || error.code === 16 || error.code === 42) {
							return setTimeout(callback, READY_RETRY_INTERVAL);
