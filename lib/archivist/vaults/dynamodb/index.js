 ...
 * @peram {boolean}  [wait] Wait for the table to be available, default to true
...
DynamoDbVault.prototype.createTable = function (params, wait, cb) {
	...
	function pollCreatingTable() {
		that.describeTable(params.TableName, function (err, data) {
			...
			that.logger.details('Status:', data.Table.Status)
				.notice('Waiting for table to be ready:', params.TableName);
	...
	this.dynamodb.createTable(params, function (err, data) {
		...
		var log = that.logger.warning.details('Creating a table on DynamoDB in production will take up to several minutes')
...
DynamoDbVault.prototype.registerMigration = function (version, report, cb) {
	var item = {
		version: { 'S': version },
		report: { 'S': report ? JSON.stringify(report) : '' },
