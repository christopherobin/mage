...
function getMediaTypeEncoder(mediaType, fromEncoding, toEncoding) {
	if (mediaTypeEncoders[mediaType]) {
		return mediaTypeEncoders[mediaType][fromEncoding + '-' + toEncoding];
	}
}
...
function getEncoder(mediaType, fromEncoding, toEncoding) {
	...
	for (var i = 0; i < keys.length; i++) {
		...
		if (key[0] === fromEncoding) {
			...
			encoderA = encoder;
			encoderB = getMediaTypeEncoder(mediaType, key[1], toEncoding);
			break;
		}
		...
		if (key[1] === toEncoding) {
			...
			encoderA = getMediaTypeEncoder(mediaType, fromEncoding, key[0]);
			encoderB = encoder;
			break;
		}
	}
...
register('utf8',   'buffer', function (data) { return new Buffer(data); });
register('base64', 'buffer', function (data) { return new Buffer(data, 'base64'); });
register('buffer', 'base64', function (data) { return data.toString('base64'); });
register('buffer', 'utf8',   function (data) { return data.toString('utf8'); });
register('utf8',   'base64', function (data) { return (new Buffer(data, 'utf8')).toString('base64'); });
register('base64', 'utf8',   function (data) { return (new Buffer(data, 'base64')).toString('utf8'); });
