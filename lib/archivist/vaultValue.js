...
VaultValue.prototype.setData = function (mediaType, data, encoding) {
	...
	if (!encoding) {
		throw new Error('Cannot set data when encoding is unknown');
	}

	if (!mediaType) {
		throw new Error('Cannot set data when mediaType is unknown');
	}
...
VaultValue.prototype.getEncoder = function (fromEncoding, toEncoding) {
	if (!this.mediaType) {
		throw new Error('Cannot query for encoders when mediaType is not yet decided');
	}
