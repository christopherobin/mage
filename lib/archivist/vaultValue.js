...
VaultValue.prototype.setOperation = function (operation) {
	if ([null, 'create', 'touch', 'update', 'delete'].indexOf(operation) === -1) {
...
VaultValue.prototype.setData = function (data, encoding) {
	// if we overwrite an existing value, we have to mark the change, unless of course
	// we overwrite the value with what it already is

	if (this.encodings[encoding] && this.encodings[encoding] !== data) {
		this.markAsChanged();
	}
