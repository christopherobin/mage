...
exports.converters = {
	'application/x-tome': function (fromValue, toValue) {
		...
		// TODO: this would be easier if we would move this logic to application/x-tome and call it "convertFrom" for application/json

		if (fromValue.encoding === 'live') {
			toValue.initWithData('application/x-tome', Tome.conjure(fromValue.data), 'live');
		} else {
			toValue.initWithData('application/x-tome', fromValue.data, fromValue.encoding);
		}
	}
};
