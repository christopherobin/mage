...
Archivist.prototype.exists = function (topic, index, cb) {
	// TODO: optimized flow:
	// TODO: iterate over read vaults, just like "get" does (avoid code duplication!)
	// TODO: so we need a sort of "forEach(readVaults, function each() {}, function final() {});
	// TODO: this allows each vault to expose an optimized "exists" function that doesn't retrieve
	// TODO: the full value
...
function getValuesWithOperations(valueMap) {
	var values = [];
	var trueNames = Object.keys(valueMap);

	for (var i = 0; i < trueNames.length; i += 1) {
		var value = valueMap[trueNames[i]];
...
function getValidators(state, values) {
	var validators = [];

	for (var i = 0; i < values.length; i += 1) {
		var validator = createValueValidator(state, values[i]);
		if (validator) {
...
function getOperations(vaults, values) {
	...
	for (var i = 0; i < vaults.length; i += 1) {
		for (var j = 0; j < values.length; j += 1) {
			var operation = createValueOperator(vaults[i], values[j]); // throws
...
Archivist.prototype.distribute = function (cb) {
	var values, validators, operations;

	var writeOrder = configuration.getWriteOrder();
	logger.debug('Preparing distribution of value changes to write-vaults:', writeOrder);

	try {
		// create an array of all values that have an operation on them (unsorted)

		values = getValuesWithOperations(this.loaded);

		// if there are no operations to execute, we're already done

		if (values.length === 0) {
			return setImmediate(cb);
		}

		// create validator functions for values that need it

		validators = getValidators(this.state, values);

		// create functions that execute the operations for all values on all their write vaults

		operations = getOperations(this.getWriteVaults(), values);
	} catch (error) {
		logger.alert(error);
