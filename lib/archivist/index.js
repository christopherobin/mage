...
var testAggressively = mage.isDevelopmentMode();
...
Archivist.prototype.getListVaults = function () {
	...
	for (var i = 0; i < listOrder.length; i++) {
		var vault = this.privateVaults[listOrder[i]] || configuration.getPersistentVault(listOrder[i]);
...
Archivist.prototype.getReadVaults = function () {
	...
	for (var i = 0; i < readOrder.length; i++) {
		var vault = this.privateVaults[readOrder[i]] || configuration.getPersistentVault(readOrder[i]);
...
Archivist.prototype.getWriteVaults = function () {
	...
	for (var i = 0; i < writeOrder.length; i++) {
		var vault = this.privateVaults[writeOrder[i]] || configuration.getPersistentVault(writeOrder[i]);
...
Archivist.prototype._requestVaultValue = function (topic, index) {
	...
	if (testAggressively) {
		configuration.assertTopicAndIndexSanity(topic, index);
	}
...
function applyReadOptions(options, value) {
	...
	if (value.data === undefined) {
		...
		throw new Error('Value (topic: ' + value.topic + ', index: ' + JSON.stringify(value.index) + ') could not be read and was not optional');
	}
...
Archivist.prototype.getValue = function (topic, index, options, cb) {
	...
	function load(callback) {
		...
		logger.debug('Getting topic', topic, 'from read-vaults:', configuration.getReadOrder());
	...
	function postProcess(callback) {
		try {
			applyReadOptions(configuration.getReadOptions(topic, options), value);
		} catch (error) {
...
Archivist.prototype.list = function (topic, partialIndex, options, cb) {
	...
	logger.debug('Listing', topic, 'indexes with partial index', partialIndex, 'on list-vaults:', configuration.getListOrder());
...
Archivist.prototype.distribute = function (cb) {
	...
	function prepare() {
		...
		logger.debug('Preparing distribution of value changes to write-vaults:', configuration.getWriteOrder());
