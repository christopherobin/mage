...
function attemptListIndexesFromVault(state, vault, topic, partialIndex, cb) {
	...
	vault.archive.listIndexes(valueHandler, topic, partialIndex, function (error, indexes) {
		if (error) {
...
Archivist.prototype.listIndexes = function (topic, partialIndex, cb) {
	// load the indexes from a vault

	logger.debug('Loading', topic, 'indexes');
	...
	return async.whilst(
		...
		function (callback) {
			attemptListIndexesFromVault(state, vaults[vaultNum++], topic, partialIndex, function (error, result) {
				if (error) {
