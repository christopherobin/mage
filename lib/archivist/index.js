...
exports.assertTopicSanity = function () {
	// checks for each configured topic:
	// - is there at least one vault set up to do read or write with?
	...
	for (var i = 0; i < topics.length; i++) {
		...
		if (vaultCount === 0) {
			throw new Error('No vaults configured for topic: ' + topic);
		}
	}
};
...
exports.assertTopicAbilities = function (topic, index, operations) {
	...
	if (vaultNames.length === 0) {
		throw new Error('No vaults are associated with topic: ' + topic);
	}
	...
	for (i = 0; i < vaultNames.length; i++) {
		...
		if (!handler.index) {
			throw new Error('No index was configured for topic: ' + topic);
		}

		var handlerIndex = handler.index.slice().sort();
		var requiredIndex = index.slice().sort();

		if (handlerIndex.length !== requiredIndex.length) {
			throw new Error('Expected index: ' + JSON.stringify(index) + ' for topic: ' + topic + ', instead found: ' + JSON.stringify(handler.index));
		}

		for (j = 0; j < handlerIndex.length; j++) {
			if (handlerIndex[j] !== requiredIndex[j]) {
				throw new Error('Expected index: ' + JSON.stringify(index) + ' for topic: ' + topic + ', instead found: ' + JSON.stringify(handler.index));
			}
		}
	}
