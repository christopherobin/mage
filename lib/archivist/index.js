...
exports.registerTopics = function (topics) {
	for (var topic in topics) {
		...
		valueHandlers.registerValueHandlersForTopic(topic, cfg.vaults);
	}
};
...
function validateChangesForVault(state, value, vault, cb) {
	// check the beforeDistribute() logic of the value handler for this vault/topic combo

	var api = valueHandlers.getHandler(vault.name, value.topic);
	if (!api || !api.beforeDistribute) {
		return cb();
	}
	...
	api.beforeDistribute(state, value, cb);
}
...
function applyChangesToVault(value, vault, cb) {
	...
	// there may be nothing to do for this value on this vault
	...
	if (fnOperation) {
		...
		fnOperation.call(vault.archive, api, value, function (error) {
			if (error) {
				logger.alert(error);
			}
		...
		var error = new Error('Operation ' + operation + ' not supported on vault ' + vault.name);
		logger.alert(error);
		cb(error);
	}
}
...
Archivist.prototype.readValue = function (topic, index, options, cb) {
	...
	async.whilst(
		function () {
			...
			return vaults[vaultNum] && value.data === undefined;
		},
		function (callback) {
			attemptReadFromVault(vaults[vaultNum++], value, callback);
		},
		function (error) {
			if (error) {
				return state.error(null, error, cb);
...
Archivist.prototype.distribute = function (cb) {
	...
	var trueNames = Object.keys(loaded);
	var list = [];

	for (var i = 0; i < vaults.length; i++) {
		for (var j = 0; j < trueNames.length; j++) {
			// add vault and value to the list

			list.push({ vault: vaults[i], value: this.loaded[trueNames[j]] });
		}
	}

	function validateChanges(cb) {
		logger.debug('Validating all value changes');

		async.forEachSeries(
			list,
			function (item, callback) {
				// errors should abort (they should already be logged by the user)

				validateChangesForVault(state, item.value, item.vault, callback);
			},
			cb
		);
	}

	function applyChanges(cb) {
		logger.debug('Distributing all value changes');

		async.forEachSeries(
			list,
			function (item, callback) {
				// errors should be ignored (they are already logged)

				applyChangesToVault(item.value, item.vault, function () {
					callback();
				});
			},
			cb
	...
	function clearOperations() {
		for (var i = 0; i < trueNames.length; i++) {
			loaded[trueNames[i]].resetOperation();
		}
	}

	// execute validation and operations

	validateChanges(function (error) {
		if (error) {
			cb(error);
		} else {
