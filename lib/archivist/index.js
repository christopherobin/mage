...
function preDistribute(state, values, cb) {
	...
	for (i = 0; i < trueNames.length; i += 1) {
		...
		switch (topicConfig.beforeDistribute.length) {
			case 2: syncHooks.push({ value: value, topicConfig: topicConfig }); break;
			case 3: asyncHooks.push({ value: value, topicConfig: topicConfig }); break;
		}
	}
...
Archivist.prototype.distribute = function (cb) {
	...
	preDistribute(this.state, values, function (errors) {
		if (errors) {
			return cb(errors);
		}
