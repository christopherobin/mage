...
//     name - the unique identifier for this vault as defined in the game
//            config.
...
function setTopicApisForVault(vaultName, addedVaultApis) {
	var topicNames = Object.keys(addedVaultApis);

	for (var i = 0; i < topicNames.length; i++) {
		var topicName = topicNames[i];

		setTopicApisForVaultAndTopic(vaultName, topicName, addedVaultApis[topicName], true);
	}
}
...
exports.registerTopicsApi = function (vaultName, topicsApi) {
	setTopicApisForVault(vaultName, topicsApi);
};
...
function applyChangesToVault(value, vault, cb) {
	...
	var api = getTopicApi(vault, value.topic);
	if (!api) {
		logger.debug('No topic API for topic', value.topic, 'on vault', vault.name);
		return cb();
	}
