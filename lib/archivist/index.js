...
Archivist.prototype.distribute = function (cb) {
	...
	], function (distributeError, results) {
		// Pull out all operation errors ignoring null values
		var operationErrors = results[1].filter(Boolean);
		cb(distributeError, operationErrors);
	});
};
