...
function attemptReadFromVault(state, vault, value, cb) {
	...
	if (!vault.archive.read) {
		return state.error(null, new Error('Read operations not supported on vault ' + vault.name), cb);
	}
...
function validateChanges(state, value, cb) {
...
Archivist.prototype.readValue = function (topic, index, options, cb) {
	...
	function postProcess(callback) {
		try {
			logger.debug('Applying read options to', topic);
...
Archivist.prototype.distribute = function (cb) {
	var state = this.state;
	var values = [];       // a list of all values in this.loaded (random order)
	var vaultValues = [];  // a list of all values x all vaults (ordered by vault precedence)
	var i, j;
	var vaults = this.getWriteVaults();
	var trueNames = Object.keys(this.loaded);

	for (i = 0; i < trueNames.length; i++) {
		values.push(this.loaded[trueNames[i]]);
	}
	...
	function validateDistribution(cb) {
		...
		async.forEachSeries(
			...
			function (value, callback) {
				...
				validateChanges(state, value, callback);
			},
			cb
		);
	}
