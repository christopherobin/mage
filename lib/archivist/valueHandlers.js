...
exports.getHandlersForTopic = function (topic) {
	// returns { vaultName: handler, vaultName: handler }

	var result = {};

	for (var vaultName in valueHandlers) {
		var handlersByTopic = valueHandlers[vaultName];

		if (handlersByTopic && handlersByTopic[topic]) {
			result[vaultName] = handlersByTopic[topic];
		}
	}
