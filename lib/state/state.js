...
State.prototype.emitEvents = function (cb) {
	...
	mage.session.getActorAddresses(this, actorIds, function (error, addresses) {
		...
		for (var i = 0, len = addresses.length; i < len; i++) {
			...
			mage.core.msgServer.send(address.addrName, address.host, '[' + actorEvents.join(',') + ']');
		}
...
State.prototype.error = function (code, logDetails, cb) {
	...
	if (typeof code === 'number') {
		...
		// turn Error objects into their message or code property

		code = '' + (code.message || code.code || code);
	} else {
		// if no code was passed, default to "server"
