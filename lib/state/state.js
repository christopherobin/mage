...
function State(actorId, session) {
	...
	// make the current access level available directly on the state

	this.access = 'anonymous';

	if (this.session && this.session.meta && this.session.meta.access) {
		this.access = this.session.meta.access;
	}
