var mdns = require('mdns'),
    ...
    logger = mithril.core.logger.context('service-discovery');
...
// announce will throw an Error on failure

ServiceDiscovery.prototype.announce = function (serviceName, port, data) {
	logger.notice('Announcing', serviceName, 'at', this.hostname + ':' + port);

	if (serviceName.length > 14) {
		throw new Error('serviceName (' + serviceName + ') is more than 14 characters (' + serviceName.length + ')');
	}
	...
	mithril.once('shutdown', function () {
		logger.notice('Stopping mdns advertisement:', serviceName);
		advert.stop();
	});
...
ServiceDiscovery.prototype.on = function (eventName, listener) {
	...
	if (m.length !== 3) {
		logger.alert('Unknown event:', eventName);
		return;
	}
	...
	if (serviceName.length > 14) {
		logger.alert('serviceName cannot be more than 14 characters:', serviceName, '(' + serviceName.length + ')');
		return;
	}
	...
	if (!browser) {
		logger.notice('Starting mdns browser', serviceName);

		browser = this.browsers[serviceName] = mdns.createBrowser(mdns.tcp(serviceName));
		browser.start();

		mithril.once('shutdown', function () {
			logger.notice('Stopping mdns browser', serviceName);
			browser.stop();
		});
	}
...
ServiceDiscovery.prototype.serviceEvent = function (listener, info) {
	var host = info.host;
	var port = info.port;
	...
	if (host && !this.options.allowSelfDiscovery) {
		...
		if (parsedHost === this.hostname && this.ports.indexOf(port) !== -1) {
			logger.verbose('Own mdns service notice ignored');
			return;
		}
	}

	logger.verbose('mdns service up/down notice', info);
