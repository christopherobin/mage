...
function getTTL() {
	if (!userTTL) {
		if (mithril.session) {
			...
			logger.mithril_debug('Applying a TTL of', userTTL, 'on the message store.');
		} else {
...
Store.prototype.getAll = function (userId) {
	...
	logger.mithril_debug('MsgServer Store getting all messages from cache for address', userId);

	// returns all messages for this user

	var data = this.cache.get(userId, getTTL());
	if (data && data.messages) {
		logger.mithril_debug('Returning messages', Object.keys(data.messages));

		return Response.pack(data.messages);
	}

	logger.mithril_debug('Found no messages');
...
Store.prototype.confirm = function (userId, msgIds) {
	...
	logger.mithril_debug('MsgServer Store confirming messages', msgIds, 'for address', userId);
...
Store.prototype.connect = function (userId, connArgs) {
	...
	logger.mithril_debug('MsgServer Store connecting to message cache for address', userId);
...
Store.prototype.disconnect = function (userId) {
	...
	logger.mithril_debug('MsgServer Store disconnecting from message cache:', userId);
