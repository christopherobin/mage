...
function getTTL() {
	if (!userTTL) {
		if (mithril.session) {
			...
			mithril.core.logger.debug('Applying a TTL of', userTTL, 'on the message store.');
		} else {
...
Store.prototype.getAll = function (userId) {
	...
	logger.debug('MsgServer Store getting all messages from cache for address', userId);

	// returns all messages for this user

	var data = this.cache.get(userId, getTTL());
	if (data && data.messages) {
		logger.debug('Returning messages', data.messages);

		return Response.pack(data.messages);
	}

	logger.debug('Found no messages');
...
Store.prototype.confirm = function (userId, msgIds) {
	...
	logger.debug('MsgServer Store confirming messages', msgIds, 'for address', userId);
...
Store.prototype.connect = function (userId, connArgs) {
	...
	logger.debug('MsgServer Store connecting to message cache for address', userId);
...
Store.prototype.disconnect = function (userId) {
	...
	logger.debug('MsgServer Store disconnecting from message cache:', userId);
