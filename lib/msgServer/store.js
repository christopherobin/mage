...
Store.prototype.process = function (dataBuffer, dst, cb) {
	...
	case ACTIONS.STORE:
		logger.verbose('Message store action: store', addrName);

		this.store(addrName, command.data);
	...
	// Register this connection address, so everything sent to this user from now on will be
	// auto-forwarded to this address

	case ACTIONS.CONNECT:
		logger.verbose('Message store action: connect', addrName);
	...
	case ACTIONS.FORWARD:
		logger.verbose('Message store action: forward', addrName);
	...
	case ACTIONS.CONFIRM:
		logger.verbose('Message store action: confirm', addrName);
	...
	case ACTIONS.DISCONNECT:
		logger.verbose('Message store action: close', addrName);

		this.disconnect(addrName);
		break;

	// Saving data to the store for all current active actors
	case ACTIONS.BROADCAST:
		logger.verbose('Message store action: broadcast');

		var actorData, actorDst, actorResult;

		var actors = Object.keys(this.cache.store);
		for (var i = 0; i < actors.length; ++i) {
			addrName = actors[i];

			this.store(actors[i], command.data);

			actorData = this.cache.get(addrName);
			if (actorData && actorData.map) {
				actorDst = actorData.map.slice(0);
			}
			actorDst.push(addrName);

			actorResult = this.getAll(addrName);
			cb(actorResult, actorDst);
		}

		// If the dst contains the 'broadcast' string,
		// the message will be sent to all the other relays
		dst = [user];
		user = null;
	...
	default:
		logger.warning('Message store could not recognise action:', command.action);
		break;
	}

	// append store user id to the destination address, if non-empty

	if (dst && dst.length > 0 && user && user.length > 0) {
		dst.push(user);
	}
