...
Message.pack = function (user, action, data) {
	...
	var msgBufferLength = userLength + dataLength + 2;
	var msgBuffer       = new Buffer(msgBufferLength);

	msgBuffer[0] = userLength;
	msgBuffer.write(user, 1);
	msgBuffer[userLength + 1] = action;

	for (var d = 0; d < dataLength; d++) {
		// TODO: why loop if we can copy?

		msgBuffer[d + userLength + 2] = data[d];
	}
...
Store.prototype.process = function (dataBuffer, dst, cb) {
	...
	console.log("command", command.action, "result", ret ? ret.toString() : "null", dst ? dst.map(function(v){ return v.toString() }) : null);
	console.log("store info", command, msgpack.unpack(command.data));
