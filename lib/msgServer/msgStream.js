...
function requestHandler(req, res, path, query) {
	...
	mage.session.resolve(state, sessionKey, function (error, session) {
		...
		if (error === 'badVersion' || error === 'noSession' || error === 'keyMismatch') {
			var response = serializeMessages({
				0: JSON.stringify([['session.unset'], ['io.error.auth.' + error]])
			});

			return sendHttpResponse(res, 200, CONTENT_TYPE_JSON, response);
		}
