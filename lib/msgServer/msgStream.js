...
Address.prototype.setConnection = function (res, transport) {
	...
	case 'shortpolling':
		...
		msgServer.forward(this.name, this.storeRelay);
		break;
		...
		if (!error && !session) {
			return state.close(function (error, response) {
				...
				if (response.myEvents && response.myEvents.length) {
					responseObject[0] = '[' + response.myEvents.join(',') + ']';
				}

				var serialized = serializeMessages(responseObject);

				sendHttpResponse(res, 200, CONTENT_TYPE_JSON, serialized);
			});
		}
...
exports.deliverMessages = function (addrName, msgs) {
	...
	if (address) {
		...
		logger.debug.data('messages', msgs).log('Could not deliver messages to', addrName, '(address gone)');
	}
};
