...
function badHttpRequest(req, res, transport) {
	...
	if (req.method === 'HEAD') {
		...
		res.end('Unknown HTTP transport: ' + transport);
	}
}
...
MsgStream.prototype.handleHttpRequest = function (req, res, query) {
	...
	if (!transport) {
		return badHttpRequest(req, res, query.transport);
	}
	...
	if (!host.setHttpConnection) {
		return badHttpRequest(req, res, query.transport);
	}

	// pass ownership of req/res over to the transport

	host.setHttpConnection(req, res, query);
