...
MsgStream.prototype.handleRequest = function (host) {
	...
	var mmrpNode = mage.core.msgServer.getMmrpNode();
	lookupAddress(addressInfo.address, host, function onSuccess(clusterId, address) {
		...
		if (!mmrpNode) {
			logger.error('MMRP not yet initialized.');
			return;
		}

		if (clusterId !== mmrpNode.clusterId) {
			var invalidate = getAddressInvalidator(addressInfo.type);
			if (!invalidate) {
				return;
			}
			var index = address.indexOf('/');
			var sessionKey = address.substr(index + 1);
			invalidate(sessionKey, host, 'differentHost');
			return;
		}
