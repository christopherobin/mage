...
Address.prototype.setConnection = function (res, transport) {
	...
	case 'longpolling':
		...
		msgServer.connect(this.name, this.storeRelay);
		break;
...
Address.prototype.deliver = function (msgs) {
	...
	var response = serializeMessages(msgs);
	...
	case 'longpolling':
		...
		// msgServer.disconnect(this.name, this.storeRelay);
		break;
	...
	mage.session.resolve(state, sessionKey, function (error, session) {
		if (!error && !session) {
			return state.close(function (error, response) {
				...
				response = serializeMessages({
					0: '[' + response.myEvents.join(',') + ']'
				});

				sendHttpResponse(res, 200, CONTENT_TYPE_JSON, response);
			});
		}
		...
		if (query.confirmIds) {
			...
			msgServer.confirm(addrName, session.host, confirmIds);
		}
...
exports.deliverMessages = function (addressName, msgs) {
	addressName = addressName.toString('utf8');

	var address = addressMap[addressName];

	if (address) {
		logger.verbose.data('messages', msgs).log('Delivering messages to', addressName);

		address.deliver(msgs);
	} else {
		logger.warning.data('messages', msgs).log('Could not deliver messages to', addressName, '(address gone)');
	}
};
