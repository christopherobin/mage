...
function Meta(ttl, dataType, flags) {
	if (Buffer.isBuffer(ttl)) {
		...
		this.data = new Buffer([0, ttl, dataType, flags >> 8, flags]);
	}
}
...
Meta.prototype.deserialize = function (data) {
	try {
		...
		mage.core.logger.error('Received packet which could not be automatically deserialized.');
	} catch (e) {
		mage.core.logger.error('Received packet with malformed data.');
	}
...
Meta.prototype.serialize = function (data) {
	...
	mage.core.logger.error('Received packet which could not be automatically serialized.');
};
