...
MmrpNode.prototype.send = function (envelope, attempts) {
	...
	if (!envelope.routeRemains() || envelope.consumeRoute(this.identity)) {
		this._onMessageToSelf(envelope);
		return 0;
	}
	...
	if (this.isRelay) {
		...
		return this.sendRaw(this.router, envelope.toArgs(), attempts);
	}
	...
	return this.sendRaw(this.dealer, envelope.toArgs(), attempts);
};
	...
	if (this.router) {
		...
		this.router.on('error', function onRouterError(error) {
			if (that.errorHandler) {
				...
				logger.error.data({ relays: that.relays, clients: that.clients }).log('Router error:', error);
			}
		});
	}
};
...
MmrpNode.prototype._onMessageToSelf = function (envelope) {
