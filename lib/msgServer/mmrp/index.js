...
MMRP.prototype.handleHandshake = function (envelope) {
...
MMRP.prototype.handshake = function (route) {
...
MMRP.prototype.connect = function (uri, identity) {
 ...
 */
MMRP.prototype.emitDelivery = function (envelope) {
	var eventPath = ('delivery.' + envelope.type).split('.');

	while (eventPath.length > 0) {
		this.emit(eventPath.join('.'), envelope);
		eventPath.pop();
	}
};
...
MMRP.prototype.onRouterMessage = function (args, senderIdentity) {
...
MMRP.prototype.send = function (envelope, attempts) {
	attempts = attempts || 1;
	...
	if (this.isRelay) {
		...
		logger.verbose(this.clusterId, 'sending envelope through router');
		...
		logger.verbose(this.clusterId, 'sending envelope through dealer');
...
MMRP.prototype.getBroadcastTargets = function (returnRoute, routingStyle) {
...
MMRP.prototype.handleBroadcastRequest = function (envelope) {
...
MMRP.prototype.broadcast = function (envelope, routingStyle) {
...
MMRP.prototype.relayUp = function (uri, identity) {
	// ignore self
