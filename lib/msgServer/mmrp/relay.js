...
Relay.prototype.parseRequestPacket = function (packet) {
	...
	console.log("requestPacket :::", packet.map(function(v){return v.toString(); }));
	...
	console.log('Checking if its for us', metadata.dataPosition);
	...
	if (metadata.dataPosition === 0) {
		...
		if (metadata.flags & meta.FLAGS.REPLY_EXPECTED) {
			...
			console.log("trigger :::", returnPath.map(function(v){return v.toString(); }), data.toString());
		}
		...
		console.log('Received message for me too!');
...
Relay.prototype.parseReplyPacket = function (packet) {
	...
	console.log("replyPacket :::", packet.map(function(v){return v.toString(); }));
...
Relay.prototype.sendReply = function (metadata, packet) {
	...
	console.log("sendReplyPacket :::", packet.map(function(v){return v.toString(); }));
