...
exports.send = function (address, clusterId, message) {
	if (mmrpNode) {
		var bytes = mmrpNode.send(new Envelope('msgServer.send', message, [clusterId, address]));
...
exports.connect = function (address, clusterId, disconnects) {
	...
	if (mmrpNode) {
		var bytes = mmrpNode.send(new Envelope('msgServer.connect', disconnects, [clusterId], [address], 'TRACK_ROUTE'));
...
function setupStore() {
	...
	mmrpNode.on('delivery.msgServer.connect', function (envelope) {
		if (store && !envelope.routeRemains()) {
			store.connectAddress(envelope.returnRoute, String(envelope.messages[0]));
		}
	});
	...
	mmrpNode.on('delivery.msgServer.send', function (envelope) {
		...
		} else if (envelope.route.length === 1) {
	...
	store.on('forward', function (messages, route) {
		if (mmrpNode) {
			mmrpNode.send(new Envelope('messagepack', messages, route));
		}
	});
}
...
function setupMsgStream() {
	...
	var stream = new MsgStream(cfgMsgStream);
	...
	mmrpNode.on('delivery.messagepack', function (envelope) {
		...
		if (address && stream) {
			stream.deliver(address, envelope.messages);
			envelope.consumeRoute();
		}
	});
}
