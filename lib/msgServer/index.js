...
exports.startClientHost = function (cb) {
	...
	if (binding.hasOwnProperty('port')) {
		...
		clientHost.listen(binding.port, binding.host, function (error, address) {
			...
			// TODO - find a way to give a useful address here.x
	...
	clientHost.listen(binding.file, function (error) {
		if (error) {
			switch (error.code) {
			case 'EADDRINUSE':
				logger.emergency(path.resolve(binding.file), 'already exists. Perhaps the server did not shutdown cleanly. Try removing the file and starting again.');
				break;
			case 'EACCES':
				logger.emergency(path.resolve(binding.file), 'cannot be created. Please check your config file.');
				break;
			default:
				logger.emergency(error);
			}
