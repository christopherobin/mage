...
var mmrpNode, service, store, stream;
...
exports.getPublicConfig = function (headers) {
	if (!isEnabled) {
		return null;
	}

	var baseUrl = mage.core.httpServer.getBaseUrl(headers);
...
exports.send = function (address, clusterId, message) {
	if (mmrpNode) {
		var bytes = mmrpNode.send(new Envelope('msgServer.send', [address].concat(message), [clusterId]));
...
exports.confirm = function (address, clusterId, msgIds) {
	...
	if (mmrpNode) {
		var bytes = mmrpNode.send(new Envelope('msgServer.confirm', message, [clusterId]));
...
exports.connect = function (address, clusterId, disconnects) {
	...
	if (mmrpNode) {
		var bytes = mmrpNode.send(new Envelope('msgServer.connect', [address, disconnects], [clusterId], [], 'TRACK_ROUTE'));
...
exports.disconnect = function (address, clusterId) {
	if (mmrpNode) {
		var bytes = mmrpNode.send(new Envelope('msgServer.disconnect', address, [clusterId]));
...
function setupStore() {
	...
	mmrpNode.on('delivery.msgServer.connect', function (envelope) {
		if (store && !envelope.routeRemains()) {
			store.connectAddress(envelope.returnRoute, String(envelope.messages[0]), String(envelope.messages[1]));
		}
	});
	...
	store.on('forward', function (messages, address, route) {
		if (mmrpNode) {
			mmrpNode.send(new Envelope('messagepack', [address].concat(messages), route));
		}
	});
}
