...
function getClientHostConfig() {
	...
	if (!cfg) {
		mithril.core.logger.emergency('No clientHost configured');
		process.exit(-1);
		return;
	}
...
exports.setup = function () {
	...
	mithril.core.logger.mithril_debug('Setting up msgServer');

	if (!pm.isMaster) {
		// set up the clientHost

		mithril.core.logger.mithril_debug('Setting up msgServer clientHost');
	...
	mithril.core.logger.mithril_debug('Setting up msgServer comm');
...
exports.startClientHost = function () {
	mithril.core.logger.mithril_debug('Starting clientHost...');
	...
	try {
		if (binding.file) {
			clientHost.server.listen(binding.file, function () {
				...
				mithril.core.logger.notice('Server running at ' + cfg.protocol + '://' + binding.file);
			});
		} else if (binding.host && binding.port) {
			clientHost.server.listen(binding.port, binding.host, function () {
				mithril.core.logger.notice('Server running at ' + cfg.protocol + '://' + binding.host + ':' + binding.port);
			});
		} else {
		...
		mithril.core.logger.emergency('Error while trying to listen on:', cfg);
