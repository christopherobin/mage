...
exports.startClientHost = function (cb) {
	...
	clientHost.listen(binding.file, function (error) {
		if (error) {
			switch (error.code) {
			case 'EADDRINUSE':
				logger.emergency('The clientHost binding file already exists. Perhaps the server did not shutdown cleanly. Try removing the file and starting again.');
				break;
			case 'EACCES':
				logger.emergency('The clientHost binding file cannot be created. Please check your config file.');
				break;
			default:
				logger.emergency(error);
			}
