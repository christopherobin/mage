...
var msgStream, client, relay, messageStore, ourMetadata;


exports = module.exports = new EventEmitter();
exports.identity = require('os').hostname();
...
function deliverMessagePackageLocal(dst, msgs) {
	...
	msgStream.deliverMessages(dst[0], msgs);
}
...
function sendToClient(address, relay, action, msg, meta) {
	...
	logger.verbose.data({
		action: action,
		message: msg
	}).log('Comm sending to', address, 'on relay', relay);

	msg = Message.pack(address, action, msg);

	exports.emit('sendMessage', msg);
...
exports.broadcast = function (msg) {
	sendToClient('broadcast', exports.identity, store.ACTIONS.BROADCAST, msg);
};
...
exports.connect = function (address, relay) {
	...
	var metaData = new Meta(null, null, meta.FLAGS.REPLY_EXPECTED);

	sendToClient(address, relay, store.ACTIONS.CONNECT, null, metaData);
};
...
exports.forward = function (address, relay) {
	...
	sendToClient(address, relay, store.ACTIONS.FORWARD, null, metaData);
};
...
function setupClient(cb) {
	...
	if (!service) {
		logger.debug('There is no point in setting up an MMRP client without service discovery (skipping).');
		return cb();
	}
	...
	try {
		...
		client = new Client(exports.identity);
...
function handleMessageStoreResponse(data, sendTo) {
	...
	if (!data) {
		logger.verbose('No message store response to handle.');
		return;
	}

	if (!sendTo) {
		logger.debug('Cannot handle message store response without a recipient address.');
		return;
	}

	if (!relay) {
		logger.debug('Cannot handle message store response when relay is not set up.');
		return;
	}

	var packet, metadata;

	// If the address is the 'broadcast' string,
	// it must forward the message to all the other relays
	if (sendTo.length === 1 && sendTo[0].toString() === 'broadcast') {
		var msg = Message.pack('', store.ACTIONS.BROADCAST, data);
		metadata = new Meta();
		packet = [msg, metadata.data];
		relay.sendBroadcast(packet);
		return;
	}

	logger.verbose('Sending reply back through relay.');

	metadata = new Meta(null, null, meta.FLAGS.IS_RESPONSE_PKT);
	packet = sendTo;
...
function setupStoreRelay(host, port, cb) {
	if (processManager.isWorker) {
		return cb();
	}

	// bail out if service discovery has not been enabled

	if (!service) {
		// a relay is optional if there are no workers in this cluster setup and service discovery
		// has not been enabled

		if (!processManager.isMaster) {
			logger.debug('Service discovery is disabled in standalone mode. Skipping MMRP Relay.');
			return cb();
		}

		var error = new Error('Cannot set up store relay on a cluster without an MMRP service');
		logger.emergency(error);
		return cb(error);
	}
	...
	try {
		relay = new Relay(exports.identity);
	...
	if (port === '*') {
		uri = relay.router.getsockopt('last_endpoint');
...
function interconnectRelays() {
	if (!service) {
		logger.debug('Cannot interconnect relays and clients without an MMRP service');
		return;
	}
		...
		if (relay) {
			...
			if (!isVersionMatch(announced)) {
				logger.debug.data({
					...
					'Relay ignoring peer of wrong version:',
					announced.host,
					'[' + announced.data.pid + ']'
				);
				return;
			...
			if (isMasterRelay(announced)) {
				logger.verbose.data({
					...
					'Relay ignoring self:', announced.host, '[' + announced.data.pid + ']'
					);
			} else {
				if (!uri) {
					logger.alert.data(announced.data).log(
						'Service "mmrp" up at', announced.host + ':' + announced.port,
						'but could not resolve host.'
					);
					return;
				}
				...
				logger.notice.data({
					...
					'Relay connecting to', uri, 'on host',
					announced.host, '[' + announced.data.pid + ']'
				);
		...
		if (client) {
			...
			if (isMasterRelay(announced)) {
				if (!uri) {
					logger.alert.data(announced.data).log(
						'Service "mmrp" up at',
						announced.host + ':' + announced.port, 'but could not resolve host.'
					);
					return;
				}
				...
				logger.notice.data({
					...
					'Client connecting to its relay', uri, 'on same host:',
					announced.host, '[' + announced.data.pid + ']'
				);
				...
				logger.verbose.data({
					peer: announced.data
				}).log('Client ignoring', uri, 'on host:', announced.host);
			}
		}
	});
		...
		service.on('down', function (announced) {
			...
			if (!uri) {
				logger.verbose.data({
					...
					'Service "mmrp" down at',
					announced.host + ':' + announced.port, 'but could not resolve hostname.'
				);
				return;
			}
			...
			if (isMasterRelay(announced)) {
				...
				try {
					logger.notice.data({
						...
						'Disconnecting from relay', uri, 'on host',
						announced.host, '[' + announced.data.pid + ']'
					);
...
exports.setup = function (cb) {
	logger.verbose('Setting up msgServer communication');
	...
	if (serviceDiscovery.isEnabled()) {
		...
		logger.warning('Service discovery has not been configured, so mmrp cannot be used over a cluster.');
	}

	// set up mmrp

	if (cfgExpose) {
		logger.warning('Configuration server.mmrp.expose has been deprecated.');
	}
	...
	if (cfgBind.protocol) {
		logger.warning('Configuration server.mmrp.bind.protocol has been deprecated.');
	}

	if (!cfgBind.port) {
		var error = new Error('mmrp bindings incomplete ("host" is optional, "port" is required)');
		logger.emergency(error);
		return cb(error);
	}

	cfgBind.host = cfgBind.host || '*';

	// Set up the relay and/or client

	setupStoreRelay(cfgBind.host, cfgBind.port, function (error) {
		if (error) {
			return cb(error);
		}
...
exports.close = function () {
	...
	if (messageStore) {
		logger.verbose('Closing message store...');
	...
	if (service) {
		logger.verbose('Closing service announcement and discovery...');
