...
exports.registerMessageHook('mithril.session', function (state, cfg, message, cb) {
	...
	mithril.session.resolve(cfg.key, function (error, session) {
		if (error || !session) {
			state.error('badSession', 'Could not resolve session: ' + cfg.key, cb);
		}
...
exports.start = function (server) {
	...
	if (binding.file) {
		...
		console.error('Required configuration missing: (server.clientHost.bind.host and server.clientHost.bind.port) or server.clientHost.bind.file');
		process.exit(-1);
	}
};
...
function processCommand(state, tags, path, params, cb) {
	if (path[0] === '/') {
	...
	if (commandCenter) {
		...
		commandCenter.execute(state, tags, cmdName, params, cb);
	} else {
...
exports.processCommandRequest = function (transportInfo, path, header, message, transportCallback) {
	...
	processCommandHeader(state, header, message, function (error, tags, params) {
		if (error) {
			...
			processCommand(state, tags, path, params, function () {
				state.close();
			});
		}
	});
