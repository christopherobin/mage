...
function executeCommands(res, app, cmdNames, data, files, queryId, transportInfo) {
	...
	app.commandCenter.executeCommands(cmdNames, headerData, paramsData, files, queryId, transportInfo, function onCommandResponse(res, content, options) {
		...
		parser.on('part', function (part) {
			...
			part.on('end', function () {
				...
				if (isFirstPart) {
					...
					if (disp) {
						var m = disp.match(/name="(.+?)"/);
						if (m) {
							var file = { data: data, name: m[1], type: part.headers['content-type'] };
