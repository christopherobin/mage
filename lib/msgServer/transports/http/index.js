...
function bindToFile(filePath, cb) {
	...
	function callback(error) {
		if (error) {
			if (error.code === 'EADDRINUSE') {
				logger.alert(
					filePath, 'already exists. Perhaps the server did not shutdown ' +
					'cleanly. Try removing the file and starting again.', error
				);
			} else {
				logger.alert('Error while binding to', filePath, error);
			}
	...
	try {
		server.listen(filePath, function () {
...
function bindToPort(port, host, cb) {
	function callback(error) {
		...
		if (error) {
			if (error.code === 'EADDRINUSE') {
				logger.alert('Address', boundAddress, 'is in use:', error);
			} else {
				logger.alert('Error while binding to', boundAddress, error);
			}
...
server.once('listening', function () {
	...
	process.once('exit', exports.close);
});
