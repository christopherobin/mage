...
Address.prototype.deliver = function (msgs) {
	...
	if (msgs) {
		...
		if (props.length > 0) {
			...
			logger.mithril_debug('Relaying messages', response);
		}
	}
...
function handleMsgStream(req, res, info, transport) {
	...
	if (req.method === 'HEAD') {
		logger.mithril_debug('Responding 200 to HEAD request');
	...
	mithril.session.resolve(sessionKey, function (error, session, msg) {
		...
		if (!session) {
			logger.mithril_debug('Responding 401: Unknown session');
		...
		logger.mithril_debug('Connecting the message stream to this HTTP request');
...
function handleRoute(request, response, urlInfo) {
	...
	if (!handler) {
		logger.mithril_debug('No registered route exists for this HTTP request');
		return false;
	}

	// execute the handler

	logger.mithril_debug('Following HTTP route');
	...
	handler(request, path, urlInfo.query, function (httpCode, out, headers) {
        ...
        log(path).data(urlInfo.query, 'query');
...
function handleCommand(req, res, urlInfo) {
	...
	if (!app) {
		logger.error('No app "' + appName + '" found');
	...
	if (m && m[1]) {
		...
		parser.on('part', function (part) {
			...
			if (!disp) {
				...
				logger.mithril_debug('Received an unidentifyable part, skipping.');
				return;
			}
			...
			if (!m) {
				...
				logger.mithril_debug('Received an unnamed part, skipping.');
				return;
			}
		...
		parser.on('end', function () {
			logger.mithril_debug('Finished reading multipart', req.method, 'request.');
		...
		req.on('end', function () {
			logger.mithril_debug('Finished reading', req.method, 'request.');
...
function handleHttpRequest(req, res, urlInfo) {
	...
	if (!handled) {
		logger.mithril_debug('No matching route found, assuming HTTP request was for a user command');
...
*/
	...
	logger.mithril_debug('Received HTTP', req.method, 'request:', req.url);
...
mithril.once('shutdown', function () {
	...
	logger.mithril_debug('Closing HTTP server (sending heartbeats)');
