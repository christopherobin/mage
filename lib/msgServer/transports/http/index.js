...
function enableCheckTxt() {
	var filePath = pathJoin(mage.rootPackage.path, 'check.txt');

	exports.addRoute('/check.txt', function (req, res) {
		var stream;
		...
		try {
			...
			logger.error('Error while opening /check.txt:', error);
			return;
		}
		...
		stream.on('error', function (error) {
			if (error.code === 'ENOENT') {
				...
				logger.error('/check.txt not found. Please create it so services can monitor application state.');
			} else {
				res.writeHead(500);

				logger.error('Error while reading /check.txt:', error);
			}
