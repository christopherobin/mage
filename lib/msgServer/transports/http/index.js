...
function handleRoute(req, res, urlInfo) {
	...
	function withCallback() {
		route.handler(req, path, urlInfo.query, function (httpCode, out, headers) {
			...
			// TODO: allow out to be an array of buffers, which we can then one-by-one write out
			//       in the case of on-the-fly gzipped responses, that saves a Buffer.concat call
			//       see web/index.js:134
			res.end(out);
		});
	}
