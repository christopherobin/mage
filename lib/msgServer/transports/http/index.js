...
function registerOrGetAddress(name, storeRelay) {
	...
	if (address) {
		...
		address = new Address(name, storeRelay);
	}
...
function handleMsgStream(req, res, info, transport) {
	...
	var relayId = info.query.relay;	// should come from memcached

	// temp
	relayId = require('os').hostname();

	var addrName = generateSessionAddress(sessionKey);
	var address = registerOrGetAddress(addrName, relayId);
...
function handleCommand(req, res, addrName, urlInfo, data) {
	// TODO: this is where the message has to be parsed and processed
	...
	try {
		...
		msgServer.processCommandRequest(transportInfo, urlInfo.pathname, header, message, function (content, options) {
			console.log('DONE', content, options);
		...
		var statusCode = 400;	// 400: Bad request
...
*/
	...
	req.on('end', function () {
		...
		if ('confirm' in query) {
			msgServer.confirm(addrName, query.relay, query.confirm.split(','));
		}
	});
}
	...
	if (address && address.res) {
		...
		case 'longpolling':
			...
			if (msgs) {
				...
				msgServer.disconnect(addrName, address.storeRelay);
