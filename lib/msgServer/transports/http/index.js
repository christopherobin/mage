...
function handleRoute(req, res, urlInfo) {
	...
	var path = decodeURI(urlInfo.pathname);
	...
	if (!route || !route.handler) {
		logger.verbose.data(req).log('No registered route exists for this HTTP request');
		return false;
	}
...
function handleCommand(req, res, urlInfo) {
	...
	var hookResponse = app.checkRequestHooks(req, path, urlInfo, 'command');
	if (hookResponse) {
		return sendHttpResponse(res, hookResponse.code, hookResponse.headers, hookResponse.output);
	}
