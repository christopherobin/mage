...
function handleMsgStream(req, res, info, transport) {
	...
	if (req.method === 'HEAD') {
		logger.verbose('Responding 200 to HEAD request');
	...
	mithril.session.resolve(sessionKey, function (error, session, msg) {
		...
		if (!session) {
			logger.verbose('Responding 401: Unknown session');
...
function handleRoute(request, response, urlInfo) {
	...
	if (!handler) {
		logger.verbose('No registered route exists for this HTTP request');
		return false;
	}
	...
	if (m && m[1]) {
		...
		parser.on('part', function (part) {
			...
			if (!disp) {
				...
				logger.verbose('Received an unidentifyable part, skipping.');
				return;
			}
			...
			if (!m) {
				...
				logger.verbose('Received an unnamed part, skipping.');
				return;
			}
