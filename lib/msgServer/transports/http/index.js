...
function handleRoute(request, response, urlInfo, data) {
	// parse URL
	...
	handler(request, path, urlInfo.query, data, function (httpCode, out, headers) {
		if (httpCode === false) {
			httpCode = 404;
...
exports.deliverMessages = function (addrName, msgs) {
	...
	if (address && address.res) {
		...
		case 'longpolling':
			...
			// else: Nothing to send yet, pending...
			break;
