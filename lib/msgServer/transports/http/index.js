...
function sendDecompressedHttpResponse(res, statusCode, headers, body) {
	...
	zlib.gunzip(body, function (error, uncompressed) {
		...
		return sendHttpResponse(res, statusCode, headers, uncompressed);
	});
}
...
Address.prototype.setConnection = function (res, transport) {
	...
	case 'longpolling':
		...
		mage.core.msgServer.comm.connect(this.name, this.storeRelay);
		break;

	case 'shortpolling':
		// pull in events

		mage.core.msgServer.comm.forward(this.name, this.storeRelay);
		break;
...
Address.prototype.deliver = function (msgs) {
	...
	case 'longpolling':
		...
		mage.core.msgServer.comm.disconnect(this.name, this.storeRelay);
		break;
...
function handleMsgStream(req, res, info, transport) {
	...
	mage.session.resolve(state, sessionKey, function (error, session, msg) {
		...
		if ('confirmIds' in info.query) {
			mage.core.msgServer.comm.confirm(addrName, session.host, info.query.confirmIds.split(','));
		}
...
exports.addRoute = function (pathMatch, fn, streaming) {
	...
	// create a handler function around fn

	var handler;

	if (streaming) {
		handler = function (req, res, path, urlInfo) {
	...
	if (typeof pathMatch === 'string') {
		...
		routes.exact[pathMatch] = { matcher: pathMatch, handler: handler };
	} else {
		routes.re.push({ matcher: pathMatch, handler: handler });
	}
...
function handleRoute(req, res, urlInfo) {
	...
	if (!route || !route.handler) {
		logger.debug.data(req).log('No route has been registered for this HTTP request (404)');
		res.writeHead(404);
		res.end();
		return;
	}
...
function httpHandler(req, res) {
	...
	// 1) Get session info from archivist
