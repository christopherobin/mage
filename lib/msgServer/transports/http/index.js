...
function handleCommand(req, res, urlInfo, data) {
	...
	if (!commandCenter || cmdNames.length === 0 || typeof data !== 'string') {
		return sendHttpResponse(res, 400);	// 400: Bad request
	}

	// parse the data into a list of header/message pairs

	data = data.split('\n');


	// create a command list
	// format: [{ name: 'obj/sync', header: [hook, hook], message: '{"abc":true}' }, ...]

	var cmdCount = cmdNames.length;

	var commands = new Array(cmdCount);
	...
	try {
		for (var i = 0; i < cmdCount; i++) {
			...
			if (pair && pair.length > 0) {
				pair = pair.split('\t');	// header \t message -> [header, message]

				if (pair.length > 2) {
					return sendHttpResponse(res, 400);	// bad request
				}

				if (pair.length > 0) {
					header = JSON.parse(pair[0]);

					message = pair[1] || '';
				}
			}

			commands[i] = { name: cmdNames[i], header: header, message: message };
		}
	} catch (e) {
		return sendHttpResponse(res, 400);	// bad request
	}
	...
	commandCenter.executeCommands(commands, cacheOptions, transportInfo, function (content, options) {
