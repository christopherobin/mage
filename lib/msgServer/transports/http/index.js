var http = require('http'),
    ...
    mage = require('../../../mage'),
    logger = mage.core.logger.context('http'),
    msgServer = mage.core.msgServer;
...
function handleMsgStream(req, res, info, transport) {
	...
	if (req.method === 'HEAD') {
		logger.verbose.data('headers', req.headers).log('Responding 200 to HEAD request');
	...
	mage.session.resolve(sessionKey, function (error, session, msg) {
		if (error) {
		...
		if (!session) {
			logger.warning.data('headers', req.headers).log('Responding 401: Unknown session');
...
function handleRoute(req, res, urlInfo) {
	...
	logger.verbose('Following HTTP route');

	handler(req, path, urlInfo.query, function (httpCode, out, headers) {
		...
		if (httpCode === false) {
			...
			logger.critical
				...
				.log('Internal server error during HTTP request:', path);
		} else {
			logger.info
				.data('url', req.url)
...
function handleCommand(req, res, urlInfo) {
	...
	if (app.firewall && !app.firewall(req.connection, req)) {
		logger.error
			...
			.data('url', req.url)
			.data('remoteAddress', req.connection.remoteAddress)
			.log('Firewall bounced the HTTP request');
