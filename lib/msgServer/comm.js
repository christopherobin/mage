...
function setupStoreRelay(identity, host, port, cb) {
	...
	// Grab last endpoint (fqdn:port), pull out port and pass on to mdns broadcasting
	if (port === '*') {
	...
	service.announce(port, metadata, function (error) {
		if (error) {
...
function interconnectRelays(cb) {
	...
	service.on('up', function (announced) {
		...
		if (relay) {
			...
			if (!isVersionMatch(announced)) {
				logger.debug.data(announced.data).log('Relay ignoring peer of wrong version:', announced.host, '[' + announced.data.pid + ']');
				return;
			}
			...
			if (isMasterRelay(announced)) {
				logger.verbose.data(announced.data).log('Relay ignoring self:', announced.host, '[' + announced.data.pid + ']');
			} else {
				logger.notice.data(announced.data).log('Relay connecting to', uri, 'on host', announced.host, '[' + announced.data.pid + ']');
		...
		if (client) {
			...
			if (isMasterRelay(announced)) {
				logger.notice.data(announced.data).log('Client connecting to its relay', uri, 'on same host:', announced.host, '[' + announced.data.pid + ']');

				client.connect(uri);
			} else {
				logger.verbose.data(announced.data).log('Client ignoring', uri, 'on host:', announced.host);
			}
		}
	});
	...
	if (relay) {
		service.on('down', function (announced) {
			...
			if (!uri) {
				logger.alert.data(announced.data).log('Service "mmrp" down at', announced.host + ':' + announced.port, 'but no useful IP:', announced.ips);
				return;
			}
			...
			if (isMasterRelay(announced)) {
				...
				try {
					logger.notice.data(announced.data).log('Disconnecting from relay', uri, 'on host', announced.host, '[' + announced.data.pid + ']');
