...
function sendToClient(address, relay, action, msg, meta) {
	...
	logger.debug('Comm sending', msg, 'to', relay);
...
exports.connect = function (address, relay) {
	logger.debug('Connecting on address', address, 'with relay', relay);
...
exports.forward = function (address, relay) {
	logger.debug('Forwarding for address', address, 'with relay', relay);
...
function setupClient(uri) {
	...
	mithril.on('shutdown', function () {
		logger.info('Closing mmrp client...');
...
function resolveHostname(host, cb) {
	...
	logger.debug('Resolving hostname', host);

	dns.resolve(host, 'A', function (resolveError, ips) {   // for now, only IPv4 is supported and tested
		if (!resolveError && ips && ips.length > 0) {
			logger.debug('Found IP addresses', ips, 'for hostname', host, '(using first)');

			return cb(null, ips[0]);
		}

		logger.debug('DNS resolve failed, trying local lookup.');

		dns.lookup(host, 4, function (lookupError, ip) {   // for now, only IPv4 is supported and tested
			if (!lookupError && ip) {
				logger.debug('Found IP address', ip, 'for hostname', host);

				return cb(null, ip);
			}

			logger.error('Resolving hostname', host, 'failed.');
...
function setupStoreRelay(identity, cfgExpose, uri) {
	logger.info('Setting up msgServer store relay');
	...
	try {
		...
		if (e.code === 'EADDRINUSE' || e.message === 'Address already in use') {
			...
			logger.info('Relay binding failed, because the address was already in use:', uri);
			logger.info('You can check which process is using this address by running:', cmd);
		}

		return mithril.fatalError('Failed to set up comm-relay: ' + e.code);
	}
	...
	mithril.on('shutdown', function () {
		logger.info('Closing mmrp relay...');
	...
	if (cfgExpose) {
		...
		if (!host || !port) {
			logger.error('Malformed mmrp exposure configuration. Not exposing this service.');
		} else {
			...
			resolveHostname(host, function (error, ip) {
				...
				// apparently the provided hostname can be resolved, so announce existance through the first found ip

				socketDescription = 'tcp://' + ip + ':' + port;

				logger.info('DNS check passed. Announcing service on:', socketDescription);

				services.announce(serviceName, port, { listen: socketDescription });
			});
		}
	} else {
		logger.info('Not exposing this service (no expose configuration found).');
	}
	...
	services.on(serviceName + '.up', function (host, port, data) {
		logger.info('mdns service up', serviceName, host, port, data);
		...
		if (listen && listen !== socketDescription) {
			...
			if (m) {
				...
				require('dns').lookup(hostname, 4, function (error, ip, family) {	// for now, only IPv4 is supported and tested
					if (error || !ip) {
					...
					logger.info('Relay connecting to', listen);
					relay.connect(listen);
				});
			} else {
				// not a tcp connection, so we do not connect

				logger.info(listen, 'does not seem to be a TCP socket, so ignored.');
			}
		}
	});
	...
	services.on(serviceName + '.down', function (host, port, data) {
		...
		logger.info('mdns service down', serviceName, host, port, data);
	});
}
