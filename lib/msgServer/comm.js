...
function interconnectRelays(cb) {
	...
	service.on('up', function (announced) {
		...
		var isMasterRelay = announced.isLocal() && (parseInt(announced.data.pid, 10) === processManager.getMasterPid());

		// apparently the provided hostname can be resolved, so we can generate a URI for it

		var uri = 'tcp://' + ip + ':' + announced.port;
		...
		if (relay) {
			// ignore other mmrp apps

			if (announced.data.game !== mage.rootPackage.name) {
				return;
			}

			// ignore versions that are not exactly the same as us

			if (announced.data.version !== mage.rootPackage.version) {
				return;
			}
