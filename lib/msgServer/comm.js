...
function setupStoreRelay(identity, host, port, cb) {
	...
	try {
		...
		if (e.code === 'EADDRINUSE' || e.message === 'Address already in use') {
			...
			logger.emergency
				.details('must install zmq module version > 2.2.0 AND zeromq version >= 3.0.0 to use wildcard ports')
				.log('Relay binding failed, because wildcard ports are unsupported');
		}  else {
