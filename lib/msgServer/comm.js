...
function setupStoreRelay(identity, cfgExpose, uri) {
	...
	if (cfgExpose) {
		...
		if (!host || !port) {
			...
			resolveHostname(host, function (error, ip) {
				...
				services.announce(serviceName, port, { listen: socketDescription, packageName: rootPackage.name, packageVersion: rootPackage.version });
			});
		}
	} else {
	...
	services.on(serviceName + '.up', function (host, port, data) {
		...
		if (!listen || listen === socketDescription) {
			logger.verbose('Ignoring mdns service:', listen);
			return;
		}
		...
		if (packageName !== rootPackage.name || packageVersion !== rootPackage.version) {
			logger.verbose('Ignoring mdns service from package:', { name: packageName, version: packageVersion }, '. Looking instead for:', rootPackage);
			return;
		}
