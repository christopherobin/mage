...
function setupStoreRelay(identity, cfgExpose, uri) {
	...
	if (cfgExpose) {
		...
		if (!host || !port) {
			...
			require('dns').lookup(host, 4, function (error, ip, family) {	// for now, only IPv4 is supported and tested
				if (error || !ip) {
					mithril.core.logger.error('DNS check failed. Not announcing service on', socketDescription);
					return;
				}
