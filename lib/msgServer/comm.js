...
function getServiceType() {
	...
	_serviceType = mithril.core.serviceDiscovery.createServiceType('mdns', 'mmrp', 'tcp', version);
	return _serviceType;
}
...
function setupClient() {
	...
	// TODO: make the client get the URI from mdns events
...
function interconnectRelays(relayPort, cb) {
	...
	serviceType.on('up', function (service) {
		...
		if (relay) {
			...
			// connect the client to the relay, if the relay is the master on this very same host

			if (service.isThisHost()) {
				logger.notice.data(service.data).log('Client connecting to', uri, 'on same host:', service.host);
	...
	// announcing our relay presence on the network
	...
	service.announce(function (error) {
		...
		logger.notice('mmrp relay announced on port', relayPort);
		cb();
	});
}
...
exports.setup = function (cb) {
	...
	setupStoreRelay(identity, uri, function (error) {
		...
		interconnectRelays(cfgBind.port, cb);
	});
};
