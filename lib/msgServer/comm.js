...
function setupClient(uri) {
	...
	logger.verbose('Setting up msgServer client');
...
function setupStoreRelay(identity, cfgExpose, uri) {
	logger.verbose('Setting up msgServer store relay');
	...
	if (cfgExpose) {
		...
		if (!host || !port) {
			...
			resolveHostname(host, function (error, ip) {
				...
				logger.verbose('DNS check passed. Announcing service on:', socketDescription);
