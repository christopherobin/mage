...
function interconnectRelays() {
	...
	service.on('error', function (error, service) {
		if (!service.data) {
			logger.verbose.data(service).data(error).log('Ignoring error from incompatible source');
			return;
		}

		if (!isNameMatch(service)) {
			logger.verbose.data(service).data(error).log('Ignoring error from other game');
			return;
		}

		if (!isVersionMatch(service)) {
			logger.verbose.data(service).data(error).log('Ignoring error from other version');
			return;
		}
