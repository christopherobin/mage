...
function interconnectRelays(cb) {
	...
	service.on('up', function (service) {
		// ignore games and versions that are not exactly the same as us
		if ((service.data.game !== mage.rootPackage.name) || (service.data.version !== mage.rootPackage.version)) {
			return;
		}

		// check that we have a valid ipv4 (no ipv6 love? :()
		var ip = service.getIp(4);

		if (!ip) {
			logger.alert.data(service.data).log('service "mmrp" up at', service.host + ':' + service.port, 'but no useful IP:', service.ips);
			return;
		}

		// check if the service is the relay of our master process

		var isMasterRelay = service.isLocal() && (parseInt(service.data.pid, 10) === processManager.getMasterPid());
		...
		if (relay) {
			...
			if (isMasterRelay) {
				...
				logger.notice.data(service.data).log('Relay connecting to', uri, 'on host', service.host, '[' + service.data.pid + ']');
