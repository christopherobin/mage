...
function sendToClient(address, relay, action, msg, meta) {
	...
	try {
		...
		mithril.core.logger.error('Error while sending message through mmrp client:', Object.prototype.toString.call(e), e);
	}
}
...
function resolveHostname(host, cb) {
	...
	mithril.core.logger.debug('Resolving hostname', host);

	dns.resolve(host, 'A', function (resolveError, ips) {   // for now, only IPv4 is supported and tested
		if (!resolveError && ips && ips.length > 0) {
			mithril.core.logger.debug('Found IP addresses', ips, 'for hostname', host, '(using first)');

			return cb(null, ips[0]);
		}

		mithril.core.logger.debug('DNS resolve failed, trying local lookup.');

		dns.lookup(host, 4, function (lookupError, ip) {   // for now, only IPv4 is supported and tested
			if (!lookupError && ip) {
				mithril.core.logger.debug('Found IP address', ip, 'for hostname', host);

				return cb(null, ip);
			}

			mithril.core.logger.error('Resolving hostname', host, 'failed.');
...
function setupStoreRelay(identity, cfgExpose, uri) {
	...
	try {
		...
		if (e.code === 'EADDRINUSE' || e.message === 'Address already in use') {
			...
			mithril.core.logger.info('Relay binding failed, because the address was already in use:', uri);
			mithril.core.logger.info('You can check which process is using this address by running:', cmd);
		}
	...
	if (cfgExpose) {
		...
		if (!host || !port) {
			...
			resolveHostname(host, function (error, ip) {
				...
				mithril.core.logger.info('DNS check passed. Announcing service on:', socketDescription);

				services.announce(serviceName, port, { listen: socketDescription });
			});
		}
	} else {
		mithril.core.logger.info('Not exposing this service (no expose configuration found).');
	}
