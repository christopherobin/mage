...
var cfgCluster = mage.core.config.get(['server', 'cluster'], false);
...
function deliverMessagePackageLocal(dst, msgs) {
	...
	exports.msgStream.deliverMessages(dst[0], msgs);
}
...
function setupClient(cb) {
	if (processManager.isMaster && cfgCluster) {
		return cb();
	}
...
function handleMessageStoreResponse(data, sendTo) {
	...
	if (!sendTo) {
		logger.warning('Cannot handle message store response without a recipient address.');
		return;
	}

	if (!relay) {
		logger.warning('Cannot handle message store response when relay is not set up.');
		return;
	}
