...
function interconnectRelays() {
	...
	service.on('up', function (announced) {
		...
		if (relay) {
			...
			if (isMasterRelay(announced)) {
				...
				if (!uri) {
					logger.alert.data(announced.data).log('Service "mmrp" up at', announced.host + ':' + announced.port, 'but could not resolve host.');
					return;
				}
	...
	if (relay) {
		service.on('down', function (announced) {
			...
			if (!uri) {
				logger.verbose.data({ peer: announced.data }).log('Service "mmrp" down at', announced.host + ':' + announced.port, 'but could not resolve hostname.');
				return;
			}
...
exports.setup = function (cb) {
	...
	if (cfgBind.protocol) {
		logger.warning('server.mmrp.bind.protocol has been deprecated, please remove it from your configuration.');
	}
