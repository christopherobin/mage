...
function sendToClient(address, relay, action, msg, meta) {
	...
	logger.mithril_debug('Comm sending', msg, 'to', relay);
...
exports.connect = function (address, relay) {
	logger.mithril_debug('Connecting on address', address, 'with relay', relay);
...
exports.forward = function (address, relay) {
	logger.mithril_debug('Forwarding for address', address, 'with relay', relay);
...
function setupClient(uri) {
	...
	logger.mithril_debug('Setting up msgServer client');
	...
	mithril.once('shutdown', function () {
		logger.mithril_debug('Closing mmrp client...');
...
function resolveHostname(host, cb) {
	...
	logger.mithril_debug('Resolving hostname', host);

	dns.resolve(host, 'A', function (resolveError, ips) {   // for now, only IPv4 is supported and tested
		if (!resolveError && ips && ips.length > 0) {
			logger.mithril_debug('Found IP addresses', ips, 'for hostname', host, '(using first)');

			return cb(null, ips[0]);
		}

		logger.mithril_debug('DNS resolve failed, trying local lookup.');

		dns.lookup(host, 4, function (lookupError, ip) {   // for now, only IPv4 is supported and tested
			if (!lookupError && ip) {
				logger.mithril_debug('Found IP address', ip, 'for hostname', host);
...
function setupStoreRelay(identity, cfgExpose, uri) {
	logger.mithril_debug('Setting up msgServer store relay');
	...
	try {
		...
		if (e.code === 'EADDRINUSE' || e.message === 'Address already in use') {
			...
			logger.emergency('Relay binding failed, because the address was already in use:', uri);
			logger.emergency('You can check which process is using this address by running:', cmd);
		}
	...
	mithril.once('shutdown', function () {
		logger.mithril_debug('Closing mmrp relay...');
	...
	if (cfgExpose) {
		...
		if (!host || !port) {
			...
			resolveHostname(host, function (error, ip) {
				...
				logger.mithril_debug('DNS check passed. Announcing service on:', socketDescription);

				services.announce(serviceName, port, { listen: socketDescription });
			});
		}
	} else {
		logger.mithril_debug('Not exposing this service (no expose configuration found).');
	}
	...
	services.on(serviceName + '.up', function (host, port, data) {
		logger.notice('mdns service up', serviceName, host, port, data);
		...
		if (listen && listen !== socketDescription) {
			...
			if (m) {
				...
				require('dns').lookup(hostname, 4, function (error, ip, family) {	// for now, only IPv4 is supported and tested
					...
					logger.notice('Relay connecting to', listen);
