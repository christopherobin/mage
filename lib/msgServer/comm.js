...
function setupStoreRelay(identity, cfgExpose, uri) {
	...
	if (cfgExpose) {
		...
		if (!host || !port) {
			...
			mithril.core.logger.info('Resolving hostname', host);

			require('dns').resolve(host, 'A', function (error, ips) {   // for now, only IPv4 is supported and tested
				if (error || !ips || ips.length === 0) {
					mithril.core.logger.error('DNS check failed. Not announcing service.');
					return;
				}
				...
				socketDescription = 'tcp://' + ips[0] + ':' + port;
