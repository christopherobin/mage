function MsgServer(eventManager) {
	var HttpLongPolling = require('./msgStream/transports/longpolling/client.js').HttpLongPolling;
	var HttpShortPolling = require('./msgStream/transports/shortpolling/client.js').HttpShortPolling;

	this.transports = {
		longpolling: HttpLongPolling,
		shortpolling: HttpShortPolling
	};
...
MsgServer.prototype.setupMessageStream = function (config, sessionKey) {
	...
	if (!stream) {
		this.stream = stream = this.createTransport('longpolling', config.httpOptions);
