...
function detectBestTransport(config, force) {
	if ((!force || force === 'websocket') && WebSocketClient.test(config)) {
		return 'websocket';
	}

	if ((!force || force === 'longpolling') && HttpPollingClient.test('longpolling', config)) {
		return 'longpolling';
	}

	if ((!force || force === 'shortpolling') && HttpPollingClient.test('shortpolling', config)) {
		return 'shortpolling';
	}
