(function () {
	...
	mithril.registerModule('sns', mod);
	...
	mod.requestRelation = function (type, actorId, cb) {
		mithril.io.send('sns.requestRelation', { type: type, actorId: actorId }, cb);
	};


	mod.delRelationRequest = function (requestId, cb) {
		mithril.io.send('sns.delRelationRequest', { requestId: requestId }, function (error) {
			if (cb) {
				cb(error);
			}
		});
	...
	mod.delRelation = function (relationId, cb) {
		mithril.io.send('sns.delRelation', { relationId: relationId }, function (error) {
			if (cb) {
				cb(error);
			}
		});
	...
	mod.sync = function (cb) {
		mithril.io.send('sns.sync', {}, null, function (errors, response) {
			if (errors) {
				return cb(errors);
			}
