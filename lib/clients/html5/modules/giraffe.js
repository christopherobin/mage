(function () {
	...
	mod.authenticate = function (tokenData, cb) {
		...
		var hash = SHA256(tokenSecret + nonce + timestamp + 'giraffeUserId=' + giraffeUserId);

		var url = origin + "/giraffe/getsession";

		var http = new XMLHttpRequest();
		http.open("GET", url + "?giraffeUserId=" + escape(giraffeUserId) + "&nonce=" + escape(nonce) + "&timestamp=" + escape(timestamp) + "&hash=" + escape(hash), true);
