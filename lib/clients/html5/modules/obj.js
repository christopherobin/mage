(function () {
	...
	mithril.registerModule('obj', mod);
	...
	ObjObject.prototype.get = function (property) {
		...
		if (this.parentClass && property in this.parentClass.data) {
			return this.parentClass.data[property];
		}
	...
	ObjObject.prototype.getAll = function () {
		var result = {};
		...
		if (this.parentClass) {
			for (property in this.parentClass.data) {
				if (!(property in result)) {
					result[property] = this.parentClass.data[property];
				}
			}
		}
	...
	function ObjCollection(collection) {
		...
		this.parentId = collection.parentId || null;
	...
	mod.getMyCollections = function (type) {
		return mod.getCollections(type, mithril.playerId);
	};


	mod.getObservedCollections = function (type) {
		var playerId = mithril.playerId;
	...
	function mergeSyncData(sync) {
		...
		if (sync.categories) {
			...
			for (i = 0; i < len; i++) {
				var category = new ObjCategory(sync.categories[i]);

				if (category.id in cache.categoriesIdMap) {
					mithril.helpers.replaceObj(cache.categoriesIdMap[category.id], category);
				} else {
		...
		if (sync.classes) {
			for (var key in sync.classes) {
				var cl = new ObjClass(sync.classes[key], key);

				if (key in cache.classesMap) {
					mithril.helpers.replaceObj(cache.classesMap[key], cl);
				} else {
					cache.classesArr.push(cl);
					cache.classesMap[key] = cl;
				}
			}
		}
		...
		if (sync.objects) {
			...
			for (i = 0; i < len; i++) {
				o = new ObjObject(sync.objects[i]);

				if (cache.objectsMap[o.id]) {
					mithril.helpers.replaceObj(cache.objectsMap[o.id], o);
				} else {
		...
		if (sync.collections) {
			for (var collectionId in sync.collections) {
				var info = sync.collections[collectionId];

				var collection = new ObjCollection(info);

				if (info.members) {
					len = info.members.length;

					for (i = 0; i < len; i++) {
						var member = info.members[i];

						o = mod.getObject(member.id);

						if (o) {
							collection.addObject(o, member.slot);
						}
					}
				}
	...
	mod.dropSyncData = function (desync) {
		...
		cache.collectionsArr = cache.collectionsArr.filter(function (collection) {
			return collectionIds.indexOf(collection.id) === -1;
		});
...
*/
		...
		io.on('obj.object.del', function (path, params) {
			for (var i = 0, len = cache.collectionsArr.length; i < len; i++) {
				// TODO: trigger events that the object has been removed from the collction (if it was in there)
				cache.collectionsArr[i].delObject(params.objectId);
			}
		...
		io.on('obj.object.data.edit', function (path, params) {
			var obj = mod.getObject(params.objectId);
			if (!obj) {
				return false;
			}
			...
			for (var key in params.data) {
				obj.data[key] = params.data[key];
			}
		}, true);


		io.on('obj.object.data.del', function (path, params) {
			...
			for (var i = 0, len = params.properties.length; i < len; i++) {
				delete obj.data[params.properties[i]];
			}
		}, true);
		...
		io.send('obj.sync', {}, function (errors, response) {
			if (errors) {
				return cb(errors);
			}
