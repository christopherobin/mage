(function () {
	...
	mod.setup = function (cb) {
		...
		if (!mithril.shop) {
			console.warn('shop module has not been loaded');
		} else {
			mithril.shop.gm = {};
		...
		if (!mithril.gc) {
			console.warn('gc module not loaded.');
		} else {
			mithril.gc.gm = {};
			...
			mithril.gc.gm.addOutConnectors = function (connectors, cb) {
				mithril.io.send('gc.addOutConnectors', connectors, cb);
			};

			mithril.gc.gm.editOutConnectors = function (connectors, cb) {
				mithril.io.send('gc.editOutConnectors', connectors, cb);
			};

			mithril.gc.gm.delOutConnectors = function (connectors, cb) {
				mithril.io.send('gc.delOutConnectors', connectors, cb);
			};
		}
