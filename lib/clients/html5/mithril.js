(function () {
	...
	function setupModules(cb) {
		...
		for (var i = 0, len = mithril.modules.length; i < len; i++) {
			if (mithril.modules[i].module.setup) {
				tasks.push(mithril.modules[i].module);
			}
		}
	...
	mithril.start = function (playerId, sessionKey, cb) {
		mithril.playerId = ~~playerId;
		mithril.sessionId = playerId + ':' + sessionKey;

		mithril.io.start(function (error) {
			if (error) {
				cb(error);
			} else {
				setupModules(cb);
