(function () {
	...
	function getUserIds(actorIds, cb) {
		...
		if (unknown.length > 0) {
			mithril.io.send('gree.getUserIds', { actorIds: unknown }, null, function (error, users) {
				if (error) {
					return cb(error);
				}
	...
	people.getThumbnails = function (actorIds, size, returnChunks, cb) {
		...
		// if given size is one of the following generic ones, translate it

		var sizeMap = {
			normal: 'thumbnailUrl',
			small: 'thumbnailUrlSmall',
			large: 'thumbnailUrlLarge',
			huge: 'thumbnailUrlHuge'
		};
