(function (window) {
	...
	// sysError handling strategies:
	// "discard": automatically discards the queue, so new user commands may be called again
	// "custom": io.retry() or io.discard() will be called externally, until that time no other user commands may be triggered
	// (not yet implemented) "autoRetry": automatically retries again, and again, until it works

	var defaults = {
		timeout: 10000,
		transportErrors: {
			network: { behavior: 'custom' },
			busy: { behavior: 'discard' },
			auth: { behavior: 'discard' }
		}
	};
	...
	function setupCommandSendFunction(hr) {
		...
		function handleTransportError(error) {
			var config = cfg.transportErrors[error];

			if (!config) {
				// nothing to do, so it's probably best to discard

				console.warn('No configuration for dealing with "' + error + '" transport errors.');

				config = { behavior: 'discard' };
			}

			emitEvent('io.error.' + error, { reason: error, behavior: config.behavior });
			...
			case 'discard':
				...
				break;
			case 'custom':
				// io.retry() or io.discard() will be expected to be called externally
				break;
			default:
				console.warn('Unknown error handling strategy:', config.behavior);
				break;
			}
		}
