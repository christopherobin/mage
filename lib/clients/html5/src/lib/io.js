(function () {
	...
	function emitEvent(path, args) {
		path = path.split('.');

		while (path.length > 0) {
			io.emit.apply(io, path.join('.'), args);
			path.pop();
		}
	}
	...
	function onCommandResponse(sysError, msg) {
		...
		if (sysError) {
			if (sysError.reason) {
				io.emit('io.error.' + sysError.reason, sysError);
			} else {
				io.emit('io.error', sysError);
			}
			return;
		}
	...
	io.send = function (commandName, data, options, cb) {
		...
		io.emit('io.send', { command: commandName });
