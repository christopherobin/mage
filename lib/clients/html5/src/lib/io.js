...
// It is now part of the msgServer module
//  - /lib/msgServer/component.JSON
//  - /lib/msgServer/client.js
...
(function (window) {
	...
	var io = mage.io = new mage.EventEmitter();

	io.transports = {};

	var baseUrl = mage.getClientHostBaseUrl();
	var headers = {};

	var m = baseUrl.match(/^[a-z]+:(\/\/)([^:]+:[^:]+)@/i);
	if (m) {
	...
	var simulatedTransportError;

	io.simulateTransportError = function (type) {
		simulatedTransportError = type;
	};


	function setupCommandSendFunction(hr) {
		var cmdEndpoint = baseUrl;
		...
		function onCommandResponse(transportError, responses) {
			...
			if (transportError || simulatedTransportError) {
				...
				handleTransportError(transportError || simulatedTransportError);
				simulatedTransportError = undefined;
				return;
			}
		...
		function sendCmdQueue() {
			...
			for (i = 0, len = cmdHooks.length; i < len; i++) {
				var hook = cmdHooks[i];
			...
			var url = encodeURI(cmdEndpoint + mage.appName + '/' + cmdNames.join(','));
			var urlParams = {};
			...
			hr.send('POST', url, urlParams, data, headers, onCommandResponse);
		}
		...
		sendCommand = function (cmdName, params, cb) {
			...
			function isFile(value) {
				return !!((Blob && value instanceof Blob) || (File && value instanceof File));
			}
			...
			for (key in params) {
				...
				if (Array.isArray(value) && isFile(value[0])) {
					...
					for (var i = 0; i < value.length; i++) {
						// add the blob/file to the files list

						if (isFile(value[i])) {
							fileIds.push(addFile(value[i]));
						}
					}

					// rewrite the parameter to represent all fileIds, so it can be serialized

					params[key] = '__filelist:' + fileIds.join(',');
				} else if (isFile(value)) {
					// add the blob/file to the files list, and rewrite the parameter to its fileId so it can be serialized

					params[key] = addFile(value);
				}
			}
	...
	function setupEventSystem(altSessionKey) {
		...
		var endpoint = baseUrl + '/msgstream';
		var params = { sessionKey: sessionKey };

		stream.setup(endpoint, params, headers, onMessages);
		stream.start();
	}
	...
	io.registerCommandHook = function (name, fn) {
		...
		for (var i = 0; i < cmdHooks.length; i++) {
			var cmdHook = cmdHooks[i];
		...
		cmdHooks.push({ name: name, fn: fn });
	};


	io.send = function (commandName, params, options, cb) {
		sendCommand(commandName, params, cb);
	};
	...
	// on mage setup, apply config and start the command system

	mage.on('configure', function (config) {
	...
	mage.once('created.session', function (session) {
	...
	mage.on('setupComplete', function () {
		setupEventSystem();
	});
