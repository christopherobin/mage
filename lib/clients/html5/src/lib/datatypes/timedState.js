(function (window) {
	...
	*/
	...
	TimedState.prototype.emitChange = function () {
		...
		this.emit('change', this.getCurrentState());
	...
	TimedState.prototype.reset = function (stored, states) {
		...
		this.setState(stored.state, stored.time);
	};
	...
	TimedState.prototype.setState = function (state, saveTime) {
		...
		this.getCfg().stored = { state: state, time: saveTime };
	...
	TimedState.prototype.getCurrentState = function () {
		var cfg = this.getCfg();
		var current = { state: cfg.stored.state, time: cfg.stored.time };
		var now = TimedState.getCurrentTimestamp();
		var rule, nextStateTime;
		...
		do {
			...
			nextStateTime = current.time + rule[0];
	...
	TimedState.prototype.getNextChange = function () {
		...
		var cfg = this.getCfg();
		var next = { state: cfg.stored.state, time: cfg.stored.time };
