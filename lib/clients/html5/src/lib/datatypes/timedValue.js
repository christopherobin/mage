...
(function (window) {
	...
	TimedValue.prototype = new mage.EventEmitter();
	...
	TimedValue.prototype.setValue = function (value, saveTime) {
		...
		if (saveTime) {
			// if the MAGE time module if available, use it to normalize the time to client time

			if (mage.time) {
				saveTime = mage.time.serverTimeToClientTime(saveTime);
			}
		} else {
			// assume saveTime is now

			saveTime = TimedValue.getCurrentTimestamp();
		}

		this.getCfg().stored = { value: value, time: saveTime };

		// emit the change (asynchronously)
	...
	TimedValue.prototype.setUpdateRule = function (updateRule) {
		...
		// timeOffset may be 0, so a truthy check simply won't do

		if (updateRule.hasOwnProperty('timeOffset')) {
			// if the MAGE time module if available, use it to normalize the time to client time

			if (mage.time) {
				updateRule.timeOffset = mage.time.serverTimeToClientTime(updateRule.timeOffset);
			}
		}
	...
	mage.data.addDataType('TimedValue', TimedValue);
