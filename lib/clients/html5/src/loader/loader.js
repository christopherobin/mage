(function (window) {
	...
	function setMaintenance(pageName, content, mimeType) {
		...
		mod.emit('maintenance', pageName, content, mimeType);
	}
	...
	mod.loadPage = function (name) {
		...
		xhr.onreadystatechange = function () {
			...
			if (code >= 500 && code < 600) {
				return setMaintenance(data, xhr.getResponseHeader('content-type'), name);
			}
		...
		var url = 'http://$cfg(server.clientHost.expose.host):$cfg(server.clientHost.expose.port)/app/' + appName + '/' + name;

		url += '?language=' + mithril.getLanguage() + '&screen=' + window.screen.width + 'x' + window.screen.height + '&density=' + mithril.getDensity();
