(function (window) {
	...
	mod.loadPage = function (name) {
		...
		var appName = $app('name'),
			hash = cachedHeader && cachedHeader.hash && encodeURIComponent(cachedHeader.hash) || '',
			url = String.prototype.concat.call(
				'http://$cfg(server.clientHost.expose.host):$cfg(server.clientHost.expose.port)/app/', appName, '/', name,
				'?language=', mithril.getLanguage(),
				'&screen=', window.screen.width + 'x' + window.screen.height,
				'&density=', mithril.getDensity(),
				hash ? '&hash=' : '',
				hash
			);
