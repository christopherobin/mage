...
function buildModuleConstructor(buildTarget, language, contextName, modName, cb) {
	...
	var commandCenter = mithril.core.commandCenters[buildTarget.app.name];
	if (!commandCenter) {
		return cb(null, '');
	}

	var commands = commandCenter.getModuleCommands(modName);
	...
	for (var cmdName in commands) {
		...
		if (params) {
			...
			fn.push(cmdName + ': function (' + params.concat(['cb']).join(', ') + ') { ' +
				'io.send(\'' + entry.execPath + '\', { ' + paramsPasser.join(', ') + ' }, ' +
	...
	out.push(indent(2) +   'api: function (io) {');
	out.push(indent(3) +     'return {');
	out.push(indent(4) +       fn.join(',\n' + indent(4)));
	out.push(indent(3) +     '}');
	out.push(indent(2) +   '}');
	out.push(indent(1) + '}');
