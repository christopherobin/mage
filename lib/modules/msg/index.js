...
exports.loadInbox = function (state, actorId, cb) {
	...
	var query = 'SELECT m.id, m.fromActorId, m.creationTime, m.expirationTime, m.type, c.title, c.body FROM msg_to_actor AS ta JOIN msg AS m ON ta.msgId = m.id JOIN msg_content AS c ON c.msgId = m.id WHERE c.language IN (?, ?) AND (ta.actorId = ? OR ta.actorId IS NULL) AND (m.expirationTime IS NULL OR m.expirationTime >= ?)';
	...
	state.datasources.db.getMapped(query, params, { key: 'id', keepKey: true }, null, function (error, messages) {
		...
		// TODO: the following query may not be optimized and may have to be rewritten to be based on 'FROM msg_to_actor', like the above query.
		// we need to test this with a good test sample
