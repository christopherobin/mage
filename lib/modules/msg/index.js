		...
		db.getMany(query, params, null, function (error, rows) {
			...
			for (var i = 0, len = rows.length; i < len; i++) {
				...
				if (msg) {
					...
					msg.data[row.property] = mage.core.PropertyMap.unserialize(row.type, row.value);
				}
			}
		...
		function (callback) {
			...
			for (var property in msg.data) {
				...
				for (var i = 0, len = prop.length; i < len; i++) {
					...
					var packed = mage.core.PropertyMap.serialize(item.value);
