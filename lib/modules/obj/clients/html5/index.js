(function () {
	...
	var mod = mage.registerModule($html5client('module.obj.construct'));
	...
	ObjCategory.prototype = new mage.data.PropertyMap();
	...
	ObjCategory.prototype.getObjects = function (options) {
		var myCollections = mod.getMyCollections();
		var result = [], o;

		for (var i = 0, len = cache.objectsArr.length; i < len; i++) {
			o = cache.objectsArr[i];

			if (o && o.inCategory(this.name) && o.isContainedIn(myCollections)) {
				result.push(o);
			}
		}
	...
	ObjClassActorData.prototype = new mage.data.PropertyMap();
	...
	function ObjClass(cl, name) {
		...
		var actorId = mage.actor.me.id;

		this.actorData = new ObjClassActorData(this.id, actorId, cl.actorData);
	...
	ObjClass.prototype = new mage.data.PropertyMap();
	...
	ObjObject.prototype = new mage.data.PropertyMap();
	...
	ObjObject.prototype.isContainedIn = function (collections) {
		// checks if the object is part of any of the given collections

		for (var i = 0, len = collections.length; i < len; i++) {
			if (collections[i].contains(this.id)) {
				return true;
			}
		}
	...
	ObjCollection.prototype = new mage.data.PropertyMap();
	...
	function getMyId() {
		if (mage.actor && mage.actor.me) {
			return mage.actor.me.id;
		}
	...
	function mergeSyncData(sync) {
		...
		if (sync.objects) {
			...
			for (i = 0; i < len; i++) {
				...
				var o = cache.objectsMap[info.id];
				if (o) {
					o.sync(info);
				} else {
	...
	mod.setup = function (cb) {
		...
		io.on('obj.collection.object.del', function (path, params) {
			...
			if (params.objectId) {
				...
				if (Array.isArray(id)) {
					for (var i = 0, len = id.length; i < len; i++) {
						member = collection.getObject(id[i]);
						if (member) {
							members.push(member);
						}
					}
				} else {
					member = collection.getObject(id);
					if (member) {
						members.push(member);
					}
				}
			} else if ('slot' in params) {
				member = collection.getObjectBySlotNumber(params.slot);
				if (member) {
					members.push(member);
				}
			}
			...
			for (var j = 0, jlen = members.length; j < jlen; j++) {
				collection.delObject(members[j].object.id);
			}
		}, true);
