	...
	mod.setup = function (cb) {
		...
		io.on('obj.collection.object.del', function (path, params) {
			...
			if (params.objectId) {
				...
				if (Array.isArray(id)) {
					for (var i = 0, len = id.length; i < len; i++) {
						var member = collection.getObject(id[i]);
						if (member) {
							members.push(member);
						}
					}
				} else {
					var member = collection.getObject(id);
					if (member) {
						members.push(member);
					}
				}
			} else if ('slot' in params) {
				var member = collection.getObjectBySlotNumber(params.slot);
				if (member) {
					members.push(member);
				}
			}
			...
			for (var i = 0, len = members.length; i < len; i++) {
				collection.delObject(members[i].object.id);
			}
		}, true);
...
*/
		...
		io.on('obj.object.del', function (path, params) {
			for (var i = 0, len = cache.collectionsArr.length; i < len; i++) {
				...
				if (collection.contains(params.objectId)) {
					io.recursiveEmit('obj.collection.object.del', { objectId: params.objectId, collectionId: collection.id });
				}
			}
