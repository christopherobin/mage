...
exports.getSyncData = function (state, options, cb) {
	...
	if (options.addClasses) {
		out.classData = exports.getAllClasses(state.language(), ['none', 'inherit'], true);
	}
	...
	var sql = 'SELECT id, parent AS parentId, type, slotCount, maxWeight, owner FROM obj_collection WHERE ';
	var where = [];
	var params = [];
	...
	if (options.collectionIds) {
		where.push('id IN (' + db.getPlaceHolders(options.collectionIds.length) + ')');
	...
	db.getMany(sql, params, null, function (error, rows) {
		...
		db.getMany(sql, params, null, function (error, rows) {
			...
			var sql = 'SELECT o.id, o.name, o.weight, o.appliedToObject, o.creationTime FROM obj_object AS o JOIN obj_collection_object AS co ON o.id = co.object WHERE co.collection IN (' + db.getPlaceHolders(collectionIds.length) + ') GROUP BY o.id';
			var params = [].concat(collectionIds);
			...
			db.getMany(sql, params, null, function (error, rows) {
				...
				var sql = 'SELECT od.object, od.property, od.type, od.value FROM obj_object_data AS od JOIN obj_collection_object AS co ON co.object = od.object WHERE od.language IN (?, ?) AND co.collection IN (' + db.getPlaceHolders(collectionIds.length) + ') GROUP BY od.object, od.property';
				var params = ['', state.language()].concat(collectionIds);
...
/*
function trimCollectionData(coll) {
	if (!coll.parentId) {
		delete coll.parentId;
	}
...
exports.getActorSyncData = function (state, cb) {
	exports.getActorCollections(state, state.actorId, ['collectionId', 'parentId', 'collectionType', 'slotCount', 'maxWeight'], function (error, collections) {
		if (error) {
			return cb(error);
		}
		...
		async.forEachSeries(
			...
			function (coll, callback) {
				...
				trimCollectionData(coll);

				coll.members = [];
				coll.owner = state.actorId;

				outCollections[coll.collectionId] = coll;

				exports.getCollectionMembers(state, coll.collectionId, function (err, members) {
					if (err) {
						return callback(err);
					}
			...
			function (error) {
				...
				exports.getActorObjects(state, state.actorId, function (error, objects) {  // get player objects
					if (error) {
						return cb(error);
					}

					var outObjects = objects;
					...
					exports.getObjectDataByOwner(state, state.actorId, function (err, data) {  // get object dataMulti
						if (err) {
							return cb(error);
						}
						...
						var data = {
							classData: exports.getAllClasses(state.language(), ['none', 'inherit'], true),
							objectData: {
...
exports.addCollectionObserver = function (state, collectionId, actorId, cb) {
	...
	state.datasources.db.exec(sql, params, null, function (error) {
		...
		exports.getSyncData(state, { collectionIds: [collectionId] }, function (error, data) {
			if (error) {
				return cb(error);
			}
...
exports.delCollectionObserver = function (state, collectionId, actorId, cb) {
	...
	state.datasources.db.exec(sql, params, null, function (error) {
		...
		state.emit(actorId, 'obj.collections.unsync', { collectionIds: [collectionId] });
...
exports.getCollectionsOwnersAndObservers = function (state, collectionIds, includeLanguage, cb) {
	var db = state.datasources.db;

	var sql;

	if (includeLanguage) {
		sql = 'SELECT c.id, c.owner, p.language FROM obj_collection_observer AS co JOIN player AS p ON p.actor = co.actorId WHERE c.id IN (' + db.getPlaceHolders(collectionIds.length) + ')';
	} else {
		sql = 'SELECT id, owner FROM obj_collection WHERE id IN (' + db.getPlaceHolders(collectionIds.length) + ')';
	}

	var params = collectionIds;
	...
	db.getMany(sql, params, null, function (error, rows) {
		...
		var result = {};	// key: collectionId, value: [{ id: actorId, type: 'owner', language: 'EN' }, { id: actorId, type: 'observer', observationLabel: 'someLabel' language: 'JA' }, ...]
		var types = [];

		// add all owners to the result

		async.forEachSeries(
			rows,
			function (row, callback) {
				var entry = { actorId: row.owner, type: 'owner' };
				...
				result[collectionId] = [entry];

				// add all observers to this collection

				exports.getCollectionObservers(state, collectionId, includeLanguage, function (error, observers) {
					if (error) {
						return callback(error);
					}

					result[collectionId] = result[collectionId].concat(observers);
...
exports.getCollectionObservers = function (state, collectionId, includeLanguage, cb) {
	var sql;

	if (includeLanguage) {
		sql = 'SELECT co.actorId, p.language FROM obj_collection_observer AS co JOIN player AS p ON p.actor = co.actorId WHERE co.collectionId = ?';
	} else {
		sql = 'SELECT actorId FROM obj_collection_observer WHERE collectionId = ?';
	}
	...
	state.datasources.db.getMany(sql, params, null, function (error, rows) {
		...
		for (var i = 0, len = rows.length; i < len; i++) {
			...
			var entry = { actorId: row.actorId, type: 'observer' };
