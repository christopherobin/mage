		...
		function (callback) {
			...
			db.getMany(sql, params, null, function (error, rows) {
				...
				for (var i = 0, len = rows.length; i < len; i++) {
					var object = rows[i];
					// drop unused properties for short transport
					if (!object.appliedToObject) {
						delete object.appliedToObject;
					}
					if (!object.weight) {
				...
				exports.getObjectsProperties(state, selectedObjectIds, { loadAll: true }, function (error, maps) { // load all object data
					...
					out = objToJson(out, { objects: '[' + rows.join(',') + ']' });
					callback();
				});
			});
		}
...
exports.getClassActorProperties = function (state, classId, actorId, options, cb) {
	var config = {
		domain: 'obj/class/' + classId + '/actor/' + actorId
	};

	LivePropertyMap.create(state, config, options, cb);
};
...
exports.getClassesActorProperties = function (state, actorId, classIds, options, cb) {
	...
	for (var i = 0, len = classIds.length; i < len; i++) {
		...
		domains[classId] = 'obj/class/' + classId + '/actor/' + actorId;
	}
	...
	LivePropertyMap.createMany(state, config, options, cb);
};
 		...
 		function (callback) {
 			if (objClass) { // store class 'copy' properties
 				async.forEachSeries(results, function (obj, objCb) {
 					...
 					obj.map.save(objCb); //these get saved with class data, then given back
 				}, callback);
...
exports.delObject = function (state, objectId, cb) { //TODO: delete object data?
	exports.getObjectActors(state, objectId, { owners: true, observers: true }, function (error, actorIds) {
		if (error) {
			return cb(error);
		}
...
exports.countCollectionObjectsByType = function (state, type, owner, cb) {
	getCollectionIdByType(state, type, owner, null, function (err, collectionId) {
		if (error) {
			return cb(error);
		}
		exports.countCollectionObjectsById(state, collectionId, function (err, count) {
			if (error) {
				return cb(error);
			}
			cb(null, count);
...
exports.getFullCollection = function (state, collectionId, cb) { //TODO: often used without data, make data optional?
	var collection = null;
	...
	async.waterfall(
		[
			...
			function (data, callback) {
				...
				exports.getObjectsProperties(state, objectIds, {}, callback);
			},
			function (maps, callback) {
				for (var i = 0, len = collection.objects.length; i < len; i++) {
					var objectId = collection.objects[i].id;
					if (objectId in maps) {
						object.data = maps[objectId];
					}
				}
				callback();
			}
