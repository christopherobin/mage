...
exports.addClass = function (state, name, weight, data, cb) {
	...
	async.waterfall([
		...
		function (callback) {
			var addSql    = 'INSERT INTO obj_class_data (classId, property, tag, language, type, behavior, value) VALUES (?, ?, ?, ?, ?, ?, ?)';
			var paramData = flattenProperties(data);

			async.forEachSeries(paramData, function (property, propCb) {
				var addParams = [id, property.property, property.tag || '', property.language || '', property.type, property.behavior || 'inherit', property.value];
