...
exports.getClassActorProperties = function (state, classId, actorId, options, cb) {
	var config = {
		tableName: 'obj_class_actor_data',
		columns: ['classId', 'actorId', 'property', 'language', 'type', 'value'],
		fixedValues: { classId: classId, actorId: actorId }
	};
...
exports.addObject = function (state, name, weight, propertyMap, tags, quantity, cb) {
	// this function throws no events, because nobody will own the objects yet.
	// only when they get added to a collection, do the owner and observers receive a sync on the objects, and an add-to-collection event
			...
			async.whilst(
				...
				function (subcallback) {
					...
					state.datasources.db.exec(sql, [].concat(params), null, function (error, info) {
						...
						objectIds.push(info.insertId >>> 0);
			...
			if (slotCount) {
				...
				for (i = 0, len = objects.length; i < len; i++) {
					...
					if (o.slot >>> 0) {
						...
						for (j = 0, jlen = currentCollection.length; j < len; j++) {
