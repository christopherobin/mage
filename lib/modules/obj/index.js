...
exports.addObjectToCollection = function (state, objectId, collectionId, options, cb) {
	// options:
	// - slot: Number. A slot number for this object. Optional. If not given and the collection has a slotCount, a free slot is automatically selected.
	// - replaceExistingObjectInSlot: boolean. If true, will replace any currently existing object in the given slot. If false, will fail if the slot is occupied.
	// - noErrorOnFullCollection: boolean. If the object could not be inserted because of no slot being available, or maxWeight being exceeded, no fatal error is thrown and execution continues.
	...
	state.datasources.db.getOne(query, params, true, null, function (err, data) {
		...
		async.series([
			function (callback) {
				...
				exports.getCollectionsActors(state, [collectionId], { owners: true, observers: true, includeLanguage: true }, function (error, results) {
					if (error) {
						return callback(error);
					}
			...
			function (callback) {
				...
				state.datasources.db.getOne(sql, params, null, function (error, row) {
					if (error) {
						return cb(error);
					}
					...
					if (currentTotalWeight + weight > maxWeight) {
						// collection is full
				...
				state.datasources.db.exec(sql, params, null, function (error) {
					...
					if (actors) {
						for (var i = 0, len = removedObjects.length; i < len; i++) {
							for (var j = 0, jlen = actors.length; j < jlen; j++) {
								state.emitToActors(actors[j].actorId, 'obj.collection.object.del', { objectId: removedObjects[i], collectionId: collectionId });
							}
						}
					}
			...
			function (callback) {
				...
				if (actors) {
					for (var i = 0, len = actors.length; i < len; i++) {
						state.emit(actors[i].actorId, 'obj.collection.object.add', { objectId: objectId, collectionId: collectionId, slot: slot });
					}
				}
