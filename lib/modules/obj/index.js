var mage = require('../../mage'),
    LivePropertyMap = mage.core.LivePropertyMap,
	...
	state.datasources.db.getMany(sql, params, null, function (error, categories) {
		...
		state.datasources.db.getMany(sql, params, null, function (error, data) {
			...
			for (var i = 0; i < clen; i++) {
				...
				category.data = new mage.core.PropertyMap();
	...
	state.datasources.db.getMany(sql, params, null, function (error, classes) {
		...
		state.datasources.db.getMany(sql, params, null, function (error, data) {
			...
			for (var i = 0; i < clen; i++) {
				...
				// register the properties on the class
				objClass.data = new mage.core.PropertyMap();
...
function getClassesSyncData(state, language, cb) {
	...
	exports.getClassesActorProperties(state, state.actorId, classIds, { loadAll: true, mapsAreOptional: true }, function (error, propMaps) {
		if (error) {
			return cb(error);
		}
...
exports.addClass = function (state, name, weight, data, cb) {
	var id = null;
	async.waterfall([
		function (callback) {
			var sql = 'INSERT INTO obj_class (name, weight) values (?, ?)';
			var params = [name, weight];
		...
		function (callback) {
			...
			var dataSql     = 'INSERT INTO obj_class_data (classId, property, language, type, behavior, value) VALUES ';
			var frag        = [];
			var dataParams  = [];
			var len         = data.length;
			...
			for (var i = 0; i < len; i++) {
				var property = data[i];
				frag.push('(?, ?, ?, ?, ?, ?)');
				dataParams.push(id, property.property, property.language || '', property.type, property.behavior || '', property.value);
			}
...
exports.editClass = function (state, id, name, weight, data, cb) {
	async.waterfall([
		function (callback) {
			var sql = 'UPDATE obj_class SET name = ?, weight = ? WHERE id = ?';
			var params = [name, weight, id];
		...
		function (callback) {
			...
			var clearSql  = 'DELETE FROM obj_class_data WHERE classId = ?';
			var clearArgs = [id];
		...
		loadAllClasses(state, cb); //finally
	});
};


exports.delClass = function (state, id, cb) {

	//TODO: keep or implement
	loadAllClasses(state, cb);
};
	...
	db.getMany(sql, params, null, function (error, rows) {
		...
		if (options.full || options.includeLanguage) {
			if (options.includeLanguage) {
				// load the language of each actor

				mage.actor.getActorsProperties(state, foundActorIds, { optional: ['language'] }, function (error, propMaps) {
					if (error) {
						return cb(error);
					}
					...
					for (var i = 0, len = result.length; i < len; i++) {
						...
						var props = propMaps[row.actorId];
						if (props) {
							row.language = props.get('language') || state.language();
						} else {
...
exports.addObject = function (state, name, weight, tags, quantity, cb) {
	var results = [];
	var creationTime = mage.core.time;
	quantity = quantity || 1;
...
exports.getCollectionTypeById = function (state, id, owner, options, cb) {
	// options: {
	//   optional: false		// set to true to not fail if not found, and return null instead of the type
	// }

	options = options || {};

	var query = 'SELECT type FROM obj_collection WHERE id = ? AND owner ' + (owner === null ? 'IS' : '=') + ' ?';
	var params = [id, owner];
...
exports.getChildCollections = function (state, parentId, cb) {
	var query = 'SELECT id, parent, type, slotCount, maxWeight, owner FROM obj_collection WHERE parent = ?';
	var params = [parentId];
...
exports.findCollectionById = function (state, collectionId, cb) {
	var query = 'SELECT id, parent, type, slotCount, maxWeight, owner FROM obj_collection WHERE id = ?';
	var params = [collectionId];

	state.datasources.db.getOne(query, params, false, null, function (err, collection) {
		if (err) {
			return cb(err);
		}
		if (!collection) {
	...
	exports.getCollectionsActors(state, [collectionId], { owners: true, observers: true, mustContainOwnerId: requiredOwner }, function (error, actorIds) {
		...
		state.datasources.db.exec(sql, params, null, function (error, info) {
			...
			if (info.affectedRows < 1) {
				return state.error(null, 'obj.removeObjectFromCollection: failed to delete objectId: ' + objectId + ' from collection: ' + collectionId, cb);
			}
