...
(function (window) {
	...
	if (!setup) {
		setup = true;
		mage.loader.renderPage('obj');

		addDialog  = $('#addObjClassDialog');
		editDialog = $('#editObjClassDialog');

		mage.setupModules(['actor', 'obj'], function (error) {
			if (error) {
				return console.error(error);
			}

			var classes = mage.obj.getClassesByName();
			var objHtml = '';
	...
	function addObjectToPlayer(actorId, collectionId, name, weight, tags, quantity) {
		mage.obj.addObjectToPlayer(actorId, collectionId, name, weight, tags, quantity, function (error, objIds) {
			if (error) {
	...
	function loadActorObjDialog(actorId) {
		...
		mage.obj.getFullCollectionsByPlayer(actorId, function (error, items) {
			if (error) {
...
//		li.append(button).append(((objClass.data && objClass.data.name) ? objClass.data.name : objClass.name));
	...
	function editObjClass() {
		...
		var sendData = flattenProperties(buildData(data));

		mage.obj.editClass(curObjClass, objClass.name, objClass.weight, sendData, function (error) {
			var msg;
	...
	function addObjClass() {
		...
		var sendData = flattenProperties(obj.data);

		mage.obj.addClass(name, weight, sendData, function (error, id) {
			if (error) {
	...
	function setupHandlers() {
		...
		$('.delObjClassBtn').live('click', function () {
			if (confirm('Are you sure you want to delete this object class?')) {
				...
				mage.obj.delClass(id, function (error) {
					if (!error) {
						objClass.remove();
						var obj = classMap[id];
