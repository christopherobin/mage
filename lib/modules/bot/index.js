...
exports.createAPI = function (cb) {
	...
	var botApp = new mage.core.app.web.WebApp('bot', { access: 'user', languages: ['en'] });
	...
	for (var i = 0; i < testPages.length; i++) {
		...
		if (!options.route && !options.routes) {
			options.route = page.name;
		}

		botApp.addIndexPage(page.name, page.path, options);
	}
...
exports.register('startBotSession', function (state, args, callback) {
	...
	mage.session.activeSessionExists(state, args.actorId, function (error, exists) {
		...
		mage.session.register(playerState, args.actorId, 'en', { isBot: true }, function (error, session) {
