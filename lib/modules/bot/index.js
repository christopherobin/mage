 ...
 *
 *        VHOST: The fully qualified domain name on which the game is exposed, i.e. deadrising.ak.node.wizcorp.jp
 *
 ...
 *
 * Example Usage:
 *      curl --url http://192.168.2.6/bot/bot.createBotPlayer \
 *          -H "Host: deadrising.ak.node.wizcorp.jp" \
 *          -d $'[{"name":"bot.psk","key":"12345"}]\n{"options":{"name":"imagodbitch","powerPlayer":true,"noTutorial":true}}'
 */
...
exports.register('startBotSession', function (state, args, cb) {
	...
	var kvKey = "session/" + args.actorId;

	state.datasources.kv.getOne(kvKey, false, null, function (error, result) {
		// Return on error
		if (error) {
			return cb(error);
		}
		...
		if (result && !args.options.dropOldSession) {
			return state.error(null, 'Actor already has a session key.', cb);
		}

		// Otherwise register a new session
		mage.session.register(playerState, args.actorId, function (error, session) {
...
exports.register('endBotSession', function (state, args, cb) {
	...
	state.datasources.kv.getOne(kvKey, false, null, function (error, result) {
		...
		var sessionKey = args.actorId + ':' + result.key;

		mage.session.resolve(sessionKey, function (error, session) {
			if (error) {
				return cb(error);
			}

			if (session) {
				session.expire(state, cb);
			} else {
				cb();
			}
		});
...
mage.core.cmd.registerMessageHook('bot.psk', function (state, params, messageData, cb) {
	...
	} else if (config.psk && config.psk !== params.key) {
...
// createAPI phase.
exports.addTestPage = function (name, path, options) {
