...
exports.findPaymentByTransactionId = function (state, transactionId, cb) {
	var sql = 'SELECT id, shopPurchaseId, status FROM apple_appstore_payment WHERE actorId = ? AND appleTransactionId = ?';
...
exports.purchaseWithReceipt = function (state, forActorId, receiptData, cb) {
	...
	async.waterfall([
		...
		function (statusCode, decodedReceipt, callback) {
		...
		function (itemId, callback) {
			...
			mithril.shop.getShopsByContainedItem(state, itemId, function (error, shops) {
				...
				callback(null, itemId, shops[0].name);
			});
		},
...
exports.send = function (httpMethod, path, getParams, postData, cb) {
