...
exports.setup = function (state, cb) {
	cfg = mage.core.config.get('module.appleAppStore');
	...
	if (!cfg) {
		mage.core.logger.error('appleAppStore module has no configuration');
		return cb(true);
	}

	if (mage.shop) {
		mage.shop.enforceCurrency(state, 'apple', { validate: onPaymentValidate, start: onPaymentStart }, cb);
	} else {
		cb();
	}
};
...
exports.purchaseWithoutReceipt = function (state, forActorId, appleProductId, quantity, cb) {
	var shopPurchase, purchaseResponse;

	async.waterfall([
		function (callback) {
			// based on appleProductId, we have to find the item that fits it,
			// so we can trigger shop.startPurchase

			mage.core.logger.debug('finding item with appleProductId :' + appleProductId);

			mage.shop.findItemIdWithProperty(state, 'appleProductId', appleProductId, callback);
		},
		function (itemId, callback) {
			if (!itemId) {
				// the product is no longer on offer

				return state.error(null, 'AppleProductId "' + appleProductId + '" is not on offer.', callback);
			}

			// find the shops that this item is part of

			mage.core.logger.debug('finding shop by contained item: ' + itemId);

			mage.shop.getShopsByContainedItem(state, itemId, function (error, shops) {
				if (error) {
				...
				if (!shops || shops.length === 0) {
					...
					return state.error(null, 'appleProductId "' + appleProductId + '" is not in a shop.', callback);
				}
		...
		function (itemId, shopName, callback) {
			var items = {};
			items[itemId] = Math.max(1, ~~quantity);

			// the startPurchase() call will respond with the purchaseRequest object

			mage.core.logger.debug('starting the shop purchase');

			mage.shop.startPurchase(state, forActorId, shopName, items, function (error, purchaseRequest) {
				if (error) {
					return callback(error);
				}
		...
		function (callback) {
			...
			mage.core.logger.debug('starting shop purchase paid trigger');

			mage.shop.purchasePaid(state, shopPurchase.id, function (error, lastPurchase) {
				if (error) {
					return callback(error);
				}
...
exports.purchaseWithReceipt = function (state, forActorId, receiptData, cb) {
	...
	async.waterfall([
		function (callback) {
			mage.core.logger.debug('verifying receipt');

			exports.rest.send('POST', '/verifyReceipt', {}, { 'receipt-data': receiptData }, function (error, statusCode, response) {
				if (error) {
					return state.error(null, 'Apple responded with error (' + error + ') during verifyReceipt call for receipt: ' + receiptData, callback);
				}

				if (!response) {
					return state.error(null, 'Apple responded with empty response during verifyReceipt call for receipt: ' + receiptData, callback);
				}

				if (response.status !== 0 && response.status !== '0') {
					return state.error(null, 'Apple responded with response.status ' + response.status + ' during verifyReceipt call for receipt: ' + receiptData, callback);
				}

				if (!response.receipt) {
					return state.error(null, 'Apple responded with empty receipt during verifyReceipt call for receipt: ' + receiptData, callback);
				}
		...
		function (response, callback) {
			...
			mage.core.logger.debug('searching for payment by transaction ID');
		...
		function (payment, callback) {
			...
			if (payment) {
				if (payment.status === 'handled') {
					mage.core.logger.debug('already handled');

					cb(null, { status: 'alreadyHandled' });
				} else {
					mage.core.logger.debug('paid payment exists, but not handled');
				...
				mage.core.logger.debug('creating payment');
		...
		function (payment, callback) {
			...
			mage.core.logger.debug('finding item with appleProductId :' + payment.appleProductId);

			mage.shop.findItemIdWithProperty(state, 'appleProductId', payment.appleProductId, callback);
		},
		function (itemId, callback) {
			if (!itemId) {
				// big problem!
		...
		function (callback) {
			...
			mage.core.logger.debug('setting payment to "handled"');
...
exports.rest.send = function (httpMethod, path, getParams, postData, cb) {
	...
	if (postData) {
		postData = JSON.stringify(postData);	// TODO: might be faster to just make this a buffer instantly
	...
	mage.core.logger.debug('Apple: Q', options);
	...
	var request = http.request(options, function (response) {
		...
		if (~~(statusCode / 100) !== 2) {
			mage.core.logger.debug('Apple: A', statusCode);

			return cb('HTTP ' + statusCode);
		}
		...
		response.on('end', function () {
			...
			mage.core.logger.debug('Apple: A', data);
