...
exports.setup = function (state, cb) {
	cfg = mithril.core.config.get('module.appleAppStore');
...
exports.purchaseWithoutReceipt = function (state, forActorId, appleProductId, quantity, cb) {
	...
	async.waterfall([
		function (callback) {
			...
			mithril.core.logger.debug('finding item with appleProductId :' + appleProductId);

			mithril.shop.findItemIdWithProperty(state, 'appleProductId', appleProductId, callback);
		},
		function (itemId, callback) {
			if (!itemId) {
				...
				return state.error(null, 'Actor ' + state.actorId + ' paid for appleProductId "' + appleProductId + '", but it is not on offer.', callback);
			}
			...
			mithril.shop.getShopsByContainedItem(state, itemId, function (error, shops) {
				...
				if (!shops || shops.length === 0) {
					...
					return state.error(null, 'Actor ' + state.actorId + ' paid for appleProductId "' + appleProductId + '", but it is not in a shop.', callback);
				}
