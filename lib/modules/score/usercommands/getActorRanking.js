...
exports.execute = function (state, context, name, actorId, range, cb) {
	mage.score.getLatestRankingListByContext(state, context, name, function (error, list) {
		...
		mage.score.getRankingDataByActor(state, actorId, list.id, function (error, data) {
			if (error) {
				return cb();
			}
			...
			mage.score.getRankingData(state, list.id, { from: data.rank - range, to: data.rank + range }, function (error, records) {
				if (error) {
					return cb();
				}
