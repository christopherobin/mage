...
exports.setup = function (state, cb) {
	var requiredContexts = mage.core.config.get('module.score.contexts') || [];
...
exports.getRankingData = function (state, id, range, cb) {
	...
	var query = 'SELECT rank, score, actor FROM score_rankinglist_ranks WHERE rankinglist = ?';
	var params = [id];
...
exports.getRankingDataByActor = function (state, actorId, rankingListId, cb) {
	var query = 'SELECT rank, score, actor FROM score_rankinglist_ranks WHERE rankinglist = ? AND actor = ?';
	var params = [rankingListId, actorId];
...
exports.getRankingDataByActors = function (state, actorIds, rankingListId, cb) {
	var db = state.datasources.db;
	var query = 'SELECT rank, score, actor FROM score_rankinglist_ranks WHERE rankinglist = ? AND actor IN (' + db.getPlaceHolders(actorIds.length) + ')';
	var params = [rankingListId].concat(actorIds);
...
exports.generateRankingList = function (state, contextName, name, ascending, interval, equalizer, cb) {
	...
	var params = [context.id, name, mage.core.time];
