...
*/
	...
	this.generateBooster = function(id, collection) {
		...
		if (booster) {
			...
			fragment     += '<div class="label">Weight</div><input type="number" class="boosterWeight" value="' + (booster.data.weight || 0) + '" /></div>';
			fragment     += '<div class="cost">Cost</div><input type="number" class="boosterCost" value="' + (booster.unitPrice || 1) + '" /></div></div>';
			var ele = $(fragment).draggable({
				helper: 'clone'
			});
			return ele;
