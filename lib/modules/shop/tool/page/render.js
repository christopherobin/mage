		...
		drop: function( event, ui ) {
			...
			function pushItems() {
				...
				itemObjects.push({
					...
					quantity: 1,
					data: [
						{ property: 'row', type: 'number', value: rowId },
						{ property: 'weight', type: 'number', value: 1}
					]
				});
			}
			...
			mithril.shop.createItemObjects(itemObjects, function (error, objectIds) {		// TODO: This is hack as hell
...
*/
	...
	this.generateCollection = function(id) {
		...
		if (collect) {
			...
			fragment += '<span class="name">' + (collect.data && collect.data.name && collect.data.name.val || collect.data.name) + '</span><button class="removeCollection removeBtn">x</button><br /></div></div>';
	...
	this.generateBooster = function(id, collection) {
		...
		if (booster) {
			...
			fragment     += '<input type="text" class="boosterName" value="' + (booster.data.name && booster.data.name.val || booster.data.name) + '" /><br />';
			fragment     += '<div class="label">Weight</div><input type="number" class="boosterWeight" value="' + (booster.data.weight || 0) + '" /><br />';
			fragment     += '<div class="label">Cost</div><input type="number" class="boosterCost" value="' + (booster.unitPrice || 1) + '" /></div>';
