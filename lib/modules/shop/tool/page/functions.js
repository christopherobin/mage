function gachaLib() {
	...
	this.init = function (cols, cCol, cBooster, render) {
		...
		curBooster  = cBooster;
		renderer    = render;
		cards       = window.mage.obj.getClassesByName();
		filters     = {};
...
//				$('#gachaType option[value="gacha_free"]').attr('selected', true);
	...
	function processCollection(action) {
		...
		if (action === 'addCollection') {
			mage.shop.createShop(identifier, prefix, type, sendData, function (error, newId) {
				collections[newId] = {
					type: type,
					items: {},
					data: data
			...
			mage.shop.editShop(data.identifier, type, sendData, function (error) {
				if (error) {
	...
	function processBooster(action) {
		...
		mage.shop.createItem(identifier, prefix, currencyType, cost, curCollection, sendData, function (error, id) {
			var collection = collections[curCollection];
			if (!collection.items) {
				collection.items = {};
			}
	...
	function editBooster(booster) {
		...
		mage.shop.editItem(curBooster, cost, sendData, function (error) {
			if (!error) {
	...
	function editGacha(card) {
		...
		mage.shop.editItemObject(id, sendData, function (error) {
			if (error) {
...
//**********************************************
	...
	this.setupHandlers = function () {
		...
		$('#historySearch').click(function (){
			...
			mage.shop.getPurchaseHistory(sFrom, sTo, sActorId, function (err, data) {
				if (err) {
					return;
				}
				if (data && data.length) {
		...
		$('.removeCollection').live('click', function() {
			...
			mage.shop.deleteShop(id, function (error) {
				if (error) {
		...
		$('.removeCard').live('click', function() {
			...
			mage.shop.deleteItemObject(id, function (error) {
				if (error) {
		...
		$('.removeBooster').live('click', function(e) {
			...
			mage.shop.deleteItem(shopIdentifier, itemIdentifier, function (error) {
				if (error) {
		...
		$('.removeBoosterRow').live('click', function(e) {
			...
			mage.shop.deleteItemObjects(ids, function (error) {
				if (error) {
