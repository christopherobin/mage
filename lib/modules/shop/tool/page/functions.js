...
//				$('#gachaType option[value="gacha_free"]').attr('selected', true);
	...
	function processCollection(action) { 
		...
		if (action === 'addCollection') {
			mithril.shop.createShop(identifier, prefix, type, data, function (error, newId) {
				collections[newId] = {
					type: type,
					items: {},
					data: data
			...
			mithril.shop.editShop(data.identifier, type, data, function (error) {
				if (error) {
	...
	function processBooster(action) {
		...
		var cost = parseInt($('#boosterPrice').val() || 0);
		...
		mithril.shop.createItem(identifier, prefix, currencyType, cost, curCollection, data, function (error, id) {
			var collection = collections[curCollection];
			if (!collection.items) {
				collection.items = {};
			}
	...
	function editBooster(booster) {
		...
		var cost = parseInt(booster.find('.boosterCost').val());
		...
		mithril.shop.editItem(curBooster, cost, data, function (error) {
			if (!error) {
	...
	function editGacha(card) {
		...
		mithril.shop.editItemObject(id, data, function (error) {
			if (error) {
	...
	this.setupHandlers = function () {
		...
		$('#historySearch').click(function (){
			...
			if (hFrom.datepicker('getDate')) {
				sFrom = parseInt(hFrom.datepicker('getDate').getTime() / 1000);
			}
			if (hTo.datepicker('getDate')) {
				sTo = parseInt(hTo.datepicker('getDate').getTime() / 1000);
			}
