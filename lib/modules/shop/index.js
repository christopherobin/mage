...
function updateShopCache(action, shop) {
	...
	case 'add':
		if (!allShopsMap[id] && !allShopsIdentMap[identifier]) {
			...
			mage.core.logger.error('Attempting to add shop that already exists : ', shop);
		}
		break;
...
function loadData(state, parentMap, keyField, table, cb) {
	...
	state.datasources.db.getMany(query, [], null, function (err, results) {
		...
		for (var key in parentMap) {
			parentMap[key].data = new mage.core.PropertyMap();
		}
...
exports.getShopsByContainedItem = function (state, itemId, cb) {
	...
	for (var id in allShopsMap) {
		var shop = allShopsMap[id];
		if (shop.items && shop.items.indexOf(itemId) !== -1) {
			results.push(shop);
		}
	}
	cb(null, results);
...
exports.purchasePaid = function (state, purchaseId, cb) {
	...
	async.series([
		function (callback) {
			var sql = 'SELECT sp.forActorId, sp.shopId, s.identifier FROM shop_purchase AS sp JOIN shop AS s ON sp.shopId = s.id WHERE sp.id = ?';
			...
			state.datasources.db.getOne(sql, params, true, null, function (error, row) {
				...
				lastPurchase.shopIdentifier = row.identifier;
				lastPurchase.forActorId = row.forActorId;
		...
		function (callback) {
			...
			var params = ['paid', mage.core.time, purchaseId, 'new'];
							...
							exports.hooks.chooseItemObjects(state, lastPurchase.shopId, item, function (error, objects) {
								...
								async.forEachSeries(
									...
									function (o, callback3) {
										...
										mage.obj.hooks.chooseObjectCollection(state, o.className, function (error, collectionId) {
											if (error) {
											...
											mage.obj.addObject(state, o.className, null, tags, o.quantity * itemQuantity, function (error, results) {
												if (error) {
													return callback3(error);
												}
												var ids = [];
												...
												if (collectionId) {
													...
													mage.obj.addObjectsToCollection(state, collectionObjects, collectionId, options, callback3);
												} else {
													callback3();
												}
											});
...
exports.addShop = function (state, identifier, prefix, type, data, cb) {
	var sql  = 'INSERT INTO shop (identifier, type) VALUES (?, ?) ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id)';
	var id;
	...
	state.datasources.db.exec(sql, args, null, function (error, info) {
		...
		var shopId = info.insertId;

		exports.setProperties(state, shopId, propMap, function (error) {
			if (error) {
				return cb(error);
			}
...
exports.addItem = function (state, identifier, prefix, currencyType, unitPrice, shopIdentifier, data, cb) {
	...
	sql     += ' ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id), unitPrice = VALUES(unitPrice)';
	var newId;
...
exports.editItemObject = function (state, id, data, cb) {
	...
	mage.core.LivePropertyMap.create(state, config, { loadAll: true }, function (error, map) {
		if (error) {
			return cb(error);
		}
	...
	async.series([
		function (callback) {
			state.datasources.db.exec(sql, params, null, function (error) {
				...
				state.emit(state.actorId, 'shop.data.edit', { shopId: shopId, properties: propMap.getAll(state.language()) });
				callback();
			});
		},
		function (callback) {
...
exports.delProperties = function (state, shopId, properties, cb) {
	...
	db.exec(sql, params, null, function (error) {
		...
		state.emit(state.actorId, 'shop.data.del', { shopId: shopId, properties: properties });
...
exports.getPurchaseHistory = function (state, fromTimeStamp, toTimeStamp, playerId, cb) {
	...
	if (playerId) {
		where.push('(playerId = ? OR forActorId = ?)');
	...
	base += ' ORDER BY sp.purchaseTime DESC';
