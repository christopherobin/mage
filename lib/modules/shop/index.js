...
exports.getItems = function (state, itemIds, shopNames, cb) {
	...
	var sql = 'SELECT DISTINCT i.id, s.name AS shopName, i.identifier, i.status, i.currencyId, i.unitPrice, c.identifier AS currencyIdentifier FROM shop_currency AS c JOIN shop_item AS i ON i.currencyId = c.id';
	var params = [];
	var where = [];
