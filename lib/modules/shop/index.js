...
exports.getShopsByContainedItem = function (state, itemId, cb) {
	var sql = 'SELECT s.id, s.name, s.type FROM shop AS s JOIN shop_item_shop AS sis ON sis.shopId = s.id AND sis.itemId = ? ORDER BY sis.sortIndex ASC';
	var params = [itemId];
...
exports.getItems = function (state, itemIds, shopNames, cb) {
	...
	var sql = 'SELECT DISTINCT i.id' + (shopNames ? ', s.name AS shopName' : '') + ', i.identifier, i.status, i.currencyId, i.unitPrice, c.identifier AS currencyIdentifier FROM shop_currency AS c JOIN shop_item AS i ON i.currencyId = c.id';
	var params = [];
	var where = [];
...
exports.delShop = function (state, params, cb) {
	var sql  = 'DELETE FROM shop WHERE identifier = ?';
	var args = [params.identifier];
