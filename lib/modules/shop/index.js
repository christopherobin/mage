...
function prepCache(state, cb) {
	...
	async.waterfall([
		...
		function (callback) {
			var sql = 'SELECT id, identifier, status, currencyId, unitPrice FROM shop_item';
			state.datasources.db.getMany(sql, [], null, callback);
		},
		...
		function (shopItems, callback) {
			for (var i = 0, len = shopItems.length; i < len; i++) {
				...
				allShopsMap[shopItem.shopId].items.push(allItemsMap[shopItem.itemId]); //TODO: implement sortIndex here
				allItemsMap[shopItem.itemId].shops.push(allShopsMap[shopItem.shopId]); //TODO: maybe just push the ID.
			}
...
//TODO: change to read from cache map
	...
	if (!itemIds) {
		...
		for (var key in allShopsMap) {
			if (shopNames.indexOf(allShopsMap[key].identifier) !== -1) {
				for (var id in allShopsMap[key].items) {
					if (!result.hasOwnProperty(id)) {
						result[id] = allShopsMap[key].items[id];
					}
				}
			}
		}
		...
		//NYI items that are in both arguments
	}
	cb(null, result);
};
...
exports.addShop = function (state, params, cb) {
	...
	if (params.identifier) {
		...
		id = params.prefix + uuid();
	} else {
		id = uuid();
	}

	var args = [id, params.type];
...
exports.editShop = function (state, params, cb) {
	// params = { prefix: 'gacha:', type: 'gacha_free', data: { priority: 4, name: 'blah, hours: [[], []], recurring: [{}, {}]  } };   'ish

	exports.getShopByIdentifier(state, params.identifier, function (error, shop) {
		var sql  = 'UPDATE shop SET type = ? WHERE identifier = ?';
		var args = [params.type, params.identifier];
		...
		async.series([
			...
			function (callback) {
				exports.getShopProperties(state, shop.id, { loadAll: true }, function (error, propMap) {
					...
					console.log(propMap.data);

					for (var mProp in propMap.data) {
						if (!(mProp in params.data)) {
							propMap.del(mProp, '', '');
						}
					}
					...
					for (var prop in params.data) {
						propMap.set(prop, params.data[prop].val || params.data[prop], params.data[prop].lang || '', params.data[prop].tag || '');
					}
...
exports.delShop = function (state, params, cb) {
	var sql  = 'DELETE si.* FROM shop_item AS si JOIN shop_item_shop AS sis ON sis.itemId = si.id WHERE sis.shopId = ?';


	exports.getShopByIdentifier(state, params.identifier, function (error, result) {
		if (error) {
			return cb(error);
		}
...
exports.addItem = function (state, params, cb) {
	...
	var args = [id, 'visible', knownCurrencyMap[params.currencyType].id, params.unitPrice];

	async.waterfall([
		function (callback) {
			exports.getShopByIdentifier(state, params.shopIdentifier, function (error, result) {
				if (error) {
					return callback(error);
				}
		...
		function (itemId, callback) {
			...
			mithril.core.LivePropertyMap.create(state, config, { loadAll: true }, function (error, map) {
				...
				for (var prop in params.data) {
					map.set(prop, params.data[prop].val || params.data[prop], params.data[prop].lang || '', params.data[prop].tag || '');
				}
...
exports.editItem = function (state, params, cb) {
	...
	async.waterfall([
		...
		function (itemId, callback) {
			...
			var args = [params.unitPrice, itemId[0].id];
...
exports.delItem = function (state, params, cb) {
	...
	var args = [params.shopIdentifier, params.itemIdentifier];
...
exports.addItemObject = function (state, params, cb) {
	...
	async.waterfall([
		...
		function (itemId, callback) {
			...
			var args = [itemId, params.className, params.quantity, (params.tags || '')];
...
exports.editItemObject = function (state, params, cb) {
	var config = {
		tableName: 'shop_item_object_data',
		columns: ['itemObjectId', 'property', 'language', 'type', 'value'],
		fixedValues: { itemObjectId: params.id },
		key: 'itemObjectId'
	};
...
exports.delItemObject = function (state, params, cb) {
...
exports.delItemObjects = function (state, params, cb) {
	var sql  = 'DELETE FROM shop_item_object WHERE id in (' + state.datasources.db.getPlaceHolders(params.ids.length) + ')';
	var args = params.ids;
