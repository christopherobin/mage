...
exports.hooks = {
	getGenericPurchaseMessage: function () {
		return '';
	},
	validateItemChoice: {}
};
...
exports.getItems = function (state, itemIds, shopNames, cb) {
	...
	db.getMany(query, params.concat([]), null, function (error, rows) {
		if (error) {
			return cb(error);
		}
		...
		var query = 'SELECT itemId, property, language, type, value FROM shop_item_data WHERE itemId IN (' + db.getPlaceHolders(itemIds.length) + ')';
		var params = itemIds;
		...
		db.getMany(query, params.concat([]), null, function (error, rows) {
			...
			query = 'SELECT itemId, className, quantity, tags FROM shop_item_object WHERE itemId IN (' + db.getPlaceHolders(itemIds.length) + ')';
		...
		function (callback) {
			...
			state.datasources.db.getMany(query, params, null, function (error, rows) {
				...
				for (var i = 0, len = rows.length; i < len; i++) {
					...
					itemShops[row.itemId] = row.shopName; // this should never be nonsense
				}
				...
				exports.getItems(state, itemIds, null, function (error, items) {
					...
					async.forEachSeries(
						...
						function (item, callback2) {
							...
							// objects = exports.hooks.chooseItemObjects(item);
							...
							async.forEachSeries(
								...
								function (object, callback3) {
									...
									mithril.obj.hooks.chooseObjectCollections(state, object.className, function (error, collections) {
										...
										mithril.obj.addObject(state, collections, object.className, null, new mithril.core.PropertyMap(), tags, object.quantity * itemQuantity, function (error, ids) {
											if (error) {
												return callback3(error);
											}
