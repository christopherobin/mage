...
function defaultChooseItemObjects(state, item, cb) {
	return item.objects;
}
	...
	db.getMany(sql, params.concat([]), null, function (error, rows) {
		...
		db.getMany(sql, params, null, function (error, rows) {
			...
			db.getMany(sql, params, null, function (error, rows) {
				...
				var sql = 'SELECT o.itemId, od.itemObjectId, od.property, od.language, od.type, od.value FROM shop_item_object_data AS od JOIN shop_item_object AS o ON o.id = od.itemObjectId WHERE o.itemId IN (' + db.getPlaceHolders(itemIds.length) + ')';
				var params = [].concat(itemIds);
				...
				db.getMany(sql, params, null, function (error, rows) {
					...
					for (var i = 0, len = rows.length; i < len; i++) {
						var row = rows[i];
						var item = result[row.itemId];

						for (var j = 0, jlen = item.objects.length; j < jlen; j++) {
							var o = item.objects[j];

							if (o.id === row.itemObjectId) {
								if (!o.data) {
									o.data = new mithril.core.PropertyMap();
								}

								o.data.importOne(row.property, row.type, row.value, row.language);
								break;
							}
						}
					}
			...
			state.datasources.db.getMany(query, params, null, function (error, rows) {
				...
				exports.getItems(state, itemIds, null, function (error, items) {
					...
					for (var itemId in items) {
						...
						lastPurchase.items[itemId] = itemQuantities[itemId];
					}
					...
					async.forEachSeries(
						...
						function (item, callback2) {
							...
							exports.hooks.chooseItemObjects(state, item, function (error, objects) {
								if (error) {
									...
									function (o, callback3) {
										...
										mithril.obj.hooks.chooseObjectCollections(state, o.className, function (error, collections) {
											...
											mithril.obj.addObject(state, collections, o.className, null, new mithril.core.PropertyMap(), tags, o.quantity * itemQuantity, function (error, ids) {
												...
												if (lastPurchase.items[item.id].objectIds) {
													lastPurchase.items[item.id].objectIds = lastPurchase.items[item.id].objectIds.concat(ids);
												} else {
													lastPurchase.items[item.id].objectIds = ids;
												}
