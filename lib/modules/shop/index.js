...
exports.enforceCurrency = function (state, identifier, callbacks, cb) {
	...
	state.datasources.db.exec(sql, params, null, function (error, info) {
		...
		exports.getCurrencyId(state, identifier, function (error, id) {
			...
			console.log('Currency has been registerd --->', knownCurrencyMap);
			cb();
		});
	});
};
		...
		async.series([
			...
			function (callback) {
				...
				state.datasources.db.exec(clearSql, clearArgs, null, function (error) {
					...
					var addSql = 'INSERT INTO shop_data (shopId, property, language, type, value) VALUES (?, ?, ?, ?, ?)';
					console.log('data : ', data);
					data = flattenProperties(data);
					
					async.forEachSeries(data, function (property, propCb) {
						var addParams = [shop.id, property.property, (property.language || ''), property.type, property.value];
...
exports.addItem = function (state, identifier, prefix, currencyType, unitPrice, shopIdentifier, data, cb) {
	...
	console.log('Adding this item --->', identifier, prefix, currencyType, unitPrice, shopIdentifier, data);
...
exports.addItemObject = function (state, itemId, className, quantity, tags, data, cb) {
	...
	async.waterfall([
		...
		function (itemObjectId, callback) {
			var clearSql  = 'DELETE FROM shop_item_object_data WHERE itemObjectId = ?';
			var addSql    = 'INSERT INTO shop_item_object_data (itemObjectId, property, language, type, value) VALUES (?, ?, ?, ?, ?)';
			var paramData = flattenProperties(data);

			async.series([
				function (dataCb) {
					state.datasources.db.exec(clearSql, [itemObjectId], null, dataCb);
				},
				function (dataCb) {
					async.forEachSeries(paramData, function (property, propCb) {
						var addParams = [itemObjectId, property.property, (property.language || ''), property.type, property.value];
						state.datasources.db.exec(addSql, addParams, null, propCb);
					}, dataCb);
				}
			], function (error) {
