...
function emitAsyncEvent(eventName, params, cb) {
...
exports.setup = function (state, cb) {
	types = mage.core.config.get('module.sns.relationTypes') || {};
	cb();
};
...
function createRelationRequest(state, type, fromActorId, toActorId, cb) {
	...
	emitAsyncEvent('relationRequested', [state, request], function (error) {
		if (error) {
			return cb(error);
		}
...
exports.delRelationRequests = function (state, requestIds, cb) {
	if (!Array.isArray(requestIds)) {
		return state.error(null, 'bad input to delRelationRequests', cb);
	}
	var inputLen = requestIds.length;

	if (inputLen === 0) {
		logger.debug('nothing to delete in delRelationRequests');
		return cb();
	}
	var db = state.datasources.db;
	var mask = db.getPlaceHolders(inputLen);
	var query = 'SELECT id, type, fromActorId, toActorId FROM sns_relationrequest WHERE id IN (' + mask + ');';

	db.getMany(query, requestIds, null, function (err, requests) {
		if (err) {
			return cb(err);
		}
		var foundLen = requests.length;
		if (foundLen !== inputLen) {
			logger.debug('request id(s) not found for deletion attempt in delRelationRequests');
		}
		var sql = 'DELETE FROM sns_relationrequest WHERE id IN (' + mask + ');';

		db.exec(sql, requestIds, null, function (error) {
			if (error) {
				return cb(error);
			}
			for (var i = 0; i < foundLen; i += 1) {
				var request = requests[i];
				state.emit(request.fromActorId, 'sns.relationrequest.del', { id: request.id });
				state.emit(request.toActorId,   'sns.relationrequest.del', { id: request.id });
			}
			cb();
		});
	});
...
exports.createRelation = function (state, type, actorA, actorB, cb) {
	...
	exports.findRelation(state, type, actorA, actorB, function (error, foundRelation) {
		...
		emitAsyncEvent('relationCreated', [state, relation], function (error) {
			if (error) {
				return cb(error);
			}
...
exports.delRelation = function (state, relationId, cb) {
	...
	exports.getRelation(state, relationId, function (error, relation) {
		...
		emitAsyncEvent('relationDeleted', [state, internal], function (error) {
			if (error) {
				return cb(error);
			}
...
exports.resetRelation = function (state, relationId, cb) {
	...
	exports.getRelation(state, relationId, function (error, relation) {
		...
		state.datasources.db.exec(sql, params, null, function (error) {
			...
			state.emit(relation.actorA, 'sns.relation.edit', { id: relationId, time: time });
			state.emit(relation.actorB, 'sns.relation.edit', { id: relationId, time: time });
