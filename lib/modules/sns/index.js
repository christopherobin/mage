...
exports.setup = function (state, cb) {
	types = mithril.getConfig('module.sns.relationTypes') || {};
	cb();
};
...
exports.getRelationRequests = function (state, options, cb) {
	...
	if (options.actorId) {
		where.push('? IN (fromActorId, toActorId)');
...
exports.requestRelation = function (state, type, fromActorId, toActorId, cb) {
	...
	if (!types[type].requiresApproval) {
		...
		if (types[type].bidirectional) {
			...
			exports.relationRequestExists(state, type, toActorId, fromActorId, function (error, exists) {
				if (error) {
					return cb(error);
				}
...
exports.getRelations = function (state, options, cb) {
	...
	if (actorIds) {
		for (var i = 0, len = actorIds.length; i < len; i++) {
			where.push('? IN (actorA, actorB)');
			params.push(actorIds[i]);
		}
	}
...
exports.findRelation = function (state, type, actorA, actorB, cb) {
	...
	if (types[type].bidirectional) {
		query = 'SELECT id, type, actorA, actorB, creationTime FROM sns_relation WHERE type = ? AND actorA IN (?, ?) AND actorB IN (?, ?)';
		params = [type, actorA, actorB, actorA, actorB];
	} else {
