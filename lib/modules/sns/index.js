...
exports.delRelationRequests = function (state, requestIds, cb) { //follows the simple format of the singular
	if (!Array.isArray(requestIds) || requestIds.length === 0) {
		logger.debug('request Ids not found, nothing to delete in delRelationRequests');
	}
	var inputLen = requestIds.length;
	var db = state.datasources.db;
	var mask = db.getPlaceHolders(requestIds.length);
	...
	db.getMany(query, requestIds, null, function (err, requests) {
		...
		if (foundLen !== inputLen) {
			logger.debug('request id(s) not found for deletion attempt in delRelationRequests'); //log if fruity Ids detected
		}
