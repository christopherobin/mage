...
exports.delRelationRequests = function (state, requestIds, cb) { //follows the simple format of the singular
	...
	if (!Array.isArray(requestIds) || requestIds.length === 0) {
		return state.error(null, 'invalid input to delRelationRequests', cb);
	}

	var mask = requestIds.map(function () {
		return '?';
	}).join(',');

	var query = 'SELECT id, type, fromActorId, toActorId FROM sns_relationrequest WHERE id IN (' + mask + ');';
	var params = requestIds.map(function (requestId) {
		return requestId >>> 0;
	});

	state.datasources.db.getMany(query, params, null, function (err, requests) {
		if (err) {
			return cb(err);
		}

		if (requests.length !== requestIds.length) {
			return state.error(null, 'request id(s) not found in deletion attempt', cb); //catch any fruity Ids
		}

		var sql = 'DELETE FROM sns_relationrequest WHERE id IN (' + mask + ');';
		params = requestIds.concat([]);
