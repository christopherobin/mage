(function () {
	...
	var mod = mage.registerModule($html5client('module.sns.construct'));
	...
	mod.setup = function (cb) {
		...
		mage.io.on('sns.relationrequest.add', function (path, params) {
			cache.requests.push(new RelationRequest(params));
		}, true);


		mage.io.on('sns.relationrequest.del', function (path, params) {
		...
		mage.io.on('sns.relation.add', function (path, params) {
			cache.relations.push(new Relation(params));
		}, true);


		mage.io.on('sns.relation.del', function (path, params) {
		...
		mage.io.on('sns.relation.edit', function (path, params) {
			// Find relation by its id and reset it. For now only creationTime property is supported
			for (var i = 0, len = cache.relations.length; i < len; i += 1) {
				var relation = cache.relations[i];
				if (relation.id === params.id && params.hasOwnProperty('time')) {
					relation.creationTime = params.time;
					break;
				}
			}
		}, true);
