(function (window) {
	...
	var mod = mage.registerModule($html5client('module.gree.construct'));

	var baseUrl;

	if (mage.getClientHostBaseUrl) {
		baseUrl = mage.getClientHostBaseUrl();
	}
	...
	mod.sendCompletedPaymentNotification = function (paymentId, cb) {
		var nonce     = generateNonce(),
			timestamp = generateTimestamp(),
