(function (window) {
	...
	mod.requestSession = function (userId, token, tokenSecret, cb) {
		/* FIXME: use this
		var nonce = '' + randomInt() + randomInt() + randomInt() + randomInt(),
			timestamp = (Date.now() / 1000) >>> 0,
			hash = sha256(tokenSecret + nonce + timestamp + userId);

		mod.getSession(userId, nonce, timestamp, hash, function (error, sessionId) {
		*/
console.log('mod.getSessionInsecure is: ' + typeof mod.getSessionInsecure);
		mod.getSessionInsecure(userId, token, tokenSecret, function (error, sessionId) {
			if (error) {
				return cb(error);
			}
