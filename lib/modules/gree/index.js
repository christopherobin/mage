...
exports.setup = function (state, cb) {
	var cfg = mithril.getConfig('module.gree');
...
exports.getGadgetXml = function () {
	...
	var expose = mithril.getConfig('server.expose');

	if (!expose) {
		return '';
	}

	var url = 'http://' + expose.host + ':' + expose.port;
	...
	xml.push('<Module>');
	xml.push('	<ModulePrefs title="' + mithril.getConfig('app.name') + '">');
...
exports.paymentConfirm = function (request, path, reqParams, cb) {
	...
	if (!oauth.isValidAppId(reqParams.opensocial_app_id) || !oauth.isValidSignature(request.method, 'http://' + request.headers.host + path, reqParams, request.headers.authorization)) {
