 ...
 *
 * @type {?function(!mage.core.State, !String, !String, !Object, !function(?Error))}
 * param {!mage.core.State} state
 ...
 *
 * @type {?function(!mage.core.State, !String, !Object, !function(?Error, ?String))}
 * param {!mage.core.State}        state
 * param {!String}                    userId
 * param {!Object}                    profile
 ...
 *
 * @type {?function(!mage.core.State, !String, !String, !Object, !String, !function(?Error))}
 * param {!mage.core.State} state
 * param {!String}             actorId
 * param {!String}             userId
 * param {!Object}             purchaseInfo
 ...
 *
 * @param {!mage.core.State}        state   MAGE state.
 * @param {!String}                    userId  GREE identifier for the user.
 * @param {?Object}                    options Options passed to mage.core.LivePropertyMap.create().
...
function retrieveUserProfile(state, token, tokenSecret, cb) {
	...
	OAUTH_CLIENT.get(GREE_ENDPOINT + '/people/@me/@self' + params, token, tokenSecret, function (error, data) {
		if (error) {
...
function retrievePaymentInfo(state, userId, paymentId, cb) {
	...
	OAUTH_CLIENT.get(paymentInfoUrl, null, null, function (error, data) {
		if (error) {
...
exports.didLogin = function (state, userId, token, tokenSecret, cb) {
	...
	logger.debug('Gree user login attempt:', userId);
	...
	getUserProperties(state, userId, { optional: ['actorId'] }, function (error, props) {
		...
		retrieveUserProfile(state, token, tokenSecret, function (error, profile) {
			...
			if (typeof exports.onNewPlayer !== 'function') {
				return state.error(null, 'mage.gree.onNewPlayer is not defined', cb);
			}
...
exports.setup = function (state, cb) {
	...
	var env = mage.core.config.get('module.gree.env');
	...
	config = mage.core.config.get('module.gree.environments.' + env);
	if (!config) {
	...
	var clientHost = mage.core.config.get('server.clientHost', {});
