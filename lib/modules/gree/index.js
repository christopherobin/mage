...
console.log('retrieveUserProfile(' + JSON.stringify([token, tokenSecret, params]) + ')');
...
console.log('didLogin.getUserProperties(' + JSON.stringify([userId, error]) + ')');
		if (error) {
			return cb(error);
		}
...
console.log('didLogin.getUserProperties.retrieveUserProfile(' + JSON.stringify([userId, error, profile]) + ')');
			if (error) {
				return cb(error);
			}

			exports.onNewPlayer(state, userId, profile, function (error, actorId) {
console.log('didLogin.getUserProperties.retrieveUserProfile.onNewPlayer(' + JSON.stringify([userId, error, actorId]) + ')');
				if (error) {
					return cb(error);
				}
...
console.log('didLogin.getUserProperties.retrieveUserProfile.onNewPlayer.getActorProperties(' + JSON.stringify([userId, error]) + ')');
					if (error) {
						return cb(error);
					}
