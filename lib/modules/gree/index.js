...
function sendHttpResponse(state, status, message, headers, cb) {
	...
	if (status >= 400) {
		mithril.core.logger.error('[GREE] returning ' + status + ' (' + message + ') to caller');
	}
...
function handleGadgetRequest(request, path, params, cb) {
	...
	if (GREE_APP_ID === appId) {
		...
		mithril.core.logger.error('[GREE] Got a gadget.xml request for an unknown application (id=' + appId + ')');
...
function retrieveUserProfile(state, token, tokenSecret, cb) {
	...
	OAUTH_CLIENT.get(GREE_ENDPOINT + '/people/@me/@self' + params, token, tokenSecret, function (error, data, response) {
		if (error) {
			mithril.core.logger.error('[GREE] Couldn\'t retrieve user profile: ' + JSON.stringify(error));
			return state.error(null, 'Couldn\'t retrieve GREE user profile', cb);
		}

		try {
			data = JSON.parse(data);
		} catch (e) {
			mithril.core.logger.error('[GREE] Failed to parse user profile : ' + JSON.stringify(error));
			return state.error(null, 'Failed to parse user profile', cb);
		}

		if (!data.entry) {
			mithril.core.logger.error('[GREE] Malformed response received for user profile: ' + JSON.stringify(data));
            return state.error(null, 'Malformed user profile', cb);
        }
...
function retrievePaymentInfo(state, userId, paymentId, cb) {
	...
	OAUTH_CLIENT.get(paymentInfoUrl, null, null, function (error, data, response) {
		if (error) {
			mithril.core.logger.error('[GREE] Couldn\'t retrieve info for user ' + userId + '\'s payment ' + paymentId + ': ' + JSON.stringify(error));
			return state.error(null, 'Couldn\'t retrieve payment info', cb);
		}

		mithril.core.logger.debug('[GREE] Retrieved payment information: ' + data);

		try {
			data = JSON.parse(data);
		} catch (e) {
			mithril.core.logger.error('[GREE] Failed to parse info for user ' + userId + '\'s payment ' + paymentId + ': ' + JSON.stringify(error));
			return state.error(null, 'Failed to parse payment info', cb);
		}

		if (!data.entry) {
			mithril.core.logger.error('[GREE] Malformed response received for user ' + userId + '\'s payment ' + paymentId + ': ' + JSON.stringify(data));
			return state.error(null, 'Malformed payment info', cb);
		}
	...
	getUserProperties(state, userId, { load: ['actorId'] }, function (error, props) {
		...
		retrieveUserProfile(state, token, tokenSecret, function (error, profile) {
			...
			if (typeof exports.onNewPlayer !== 'function') {
				mithril.core.logger.error('mithril.gree.onNewPlayer is not defined');
				return state.error(null, 'Cannot create player', cb);
			}
...
function handleInAppPurchaseVerificationRequest(request, path, params, cb) {
	var state = new mithril.core.State();
	mithril.core.logger.debug('[GREE] Received IAP verification request: ' + JSON.stringify(params));
	...
	getUserProperties(state, userId, { load: ['actorId'] }, function (error, props) {
		...
		function verifyPurchase() {
			mithril.core.logger.debug('[GREE] Payment ' + paymentId + ' accepted for user with GREE id ' + userId);
			...
			retrievePaymentInfo(state, userId, paymentId, function (error, data) {
				...
				exports.onVerifyPurchase(state, actorId, userId, data, function (error) {
					if (error) {
						...
						mithril.core.logger.error('[GREE] Payment ' + paymentId + ' refused for user with GREE id ' + userId);
						return sendHttpForbiddenError(state, cb);
					}
...
exports.getSession = function (state, userId, nonce, timestamp, hash, cb) {
	...
	getUserProperties(state, userId, { load: ['actorId', 'token', 'tokenSecret'] }, function (error, props) {
		...
		var actorId = props.get('actorId');
		if (!actorId) {
			mithril.core.logger.error('[GREE] User ' + userId + ' has no associated actor id');
		...
		if (hash !== buildHash(tokenSecret, nonce, timestamp, userId)) {
			mithril.core.logger.error('[GREE] Invallid hash');
...
exports.completePurchase = function (state, userId, paymentId, nonce, timestamp, hash, cb) {
	...
	getUserProperties(state, userId, { load: ['actorId', 'tokenSecret'] }, function (error, props) {
		...
		retrievePaymentInfo(state, userId, paymentId, function (error, data) {
			...
			if (!data) {
				return cb('No payment information returned');
			}
			...
			if (completionCode === undefined) {
				return cb('Purchase hasn\'t been completed yet');
			}
