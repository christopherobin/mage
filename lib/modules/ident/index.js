...
function EntryPoint(engineName, cfg) {
	...
	if (mage.isDevelopmentMode('adminEverywhere')) {
		...
		logger.warning(
			'[deprecated] The access level is no longer part of the ident engine\'s own ' +
...
exports.setup = function (state, cb) {
	var identCfg = mage.core.config.get(['module', 'ident'], {});

	var identTopic = identCfg.topic || 'ident';

	try {
		mage.core.archivist.assertTopicAbilities(identTopic, ['userId'], ['get', 'list', 'set']);
	} catch (e) {
		mage.logger.emergency('The ident module cannot list/read/write to topic:', identTopic);
		return cb(e);
	}
...
exports.register = function (state, engineName, credentials, cb) {
	var entryPoint;
	...
	entryPoint.engine.auth(state, credentials, function (error, userId, engineKey) {
		...
		users.create(state, userId, authSources, cb);
	});
};
