...
function LdapEngine(name, cfg, logger) {
	this.name = name;
	this.cfg = cfg;
	this.logger = logger;
}
...
LdapEngine.prototype.auth = function (state, credentials, cb) {
	...
	var dn = uidAttr + '=' + dnEscape(userId) + ',' + this.cfg.baseDn;
	...
	client.bind(dn, password, function (error) {
		...
		var user = {
			userId: dn,
			displayName: userId  // TODO: I'm sure we can do better than userId here
		};
...
exports.setup = function (name, cfg, logger, cb) {
	...
	try {
		instance = new LdapEngine(name, cfg, logger);
	} catch (err) {
		return cb(err);
	}
