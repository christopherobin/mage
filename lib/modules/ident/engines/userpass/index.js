...
UserPassword.prototype.usernameToUserId = function (username) {
	assert.ok(username, 'Missing username');
...
UserPassword.prototype.auth = function (state, credentials, cb) {
	...
	try {
		...
		assert(credentials.password, 'Missing password');
	} catch (e) {
	...
	this.getUserDoc(state, userId, true, function (error, doc) {
		...
		try {
			assert.equal(that.hash(credentials.password, doc.credentials.salt), doc.credentials.password, 'Invalid password');
		} catch (e) {
			return state.error('ident', e, cb);
		}
