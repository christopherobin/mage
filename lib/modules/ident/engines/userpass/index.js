...
function UserPassword(name, cfg, logger) {
	...
	for (var hashName in hashes) {
		if (hashes.hasOwnProperty(hashName) && this.cfg.hasOwnProperty(hashName)) {
			hash = hashName;
			break;
		}
	}

	assert(hash, 'Please configure this userpass engine with: "hash", "hmac" or "pbkdf2".');

	this.hash = hashes[hash](this.cfg[hash], this.logger);
}
...
UserPassword.prototype.auth = function (state, credentials, cb) {
	...
	this.getUser(state, credentials.username, function (error, user) {
		...
		try {
			assert.equal(that.hash(credentials.password, user.salt), user.password, 'invalidPassword');
		} catch (e) {
			return cb(e);
		}
...
UserPassword.prototype.createUser = function (state, credentials, userId, cb) {
	...
	this.getUser(state, username, function (error, user) {
		...
		crypto.randomBytes(saltSize, function (error, salt) {
			...
			user.password = that.hash(credentials.password, salt);
			...
			mage.ident.addAuthSources(state, userId, authSource, that.cfg.doNotCreate, function (error) {
				if (error) {
					mage.logger.error('failed to addAuthSources');
				}
