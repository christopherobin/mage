...
/**
 * Update user details, such as password etc.
 ...
 * @param {Function} cb
 */
UserPassword.prototype.updateUser = function (state, username, changes, cb) {
	var that = this;
	...
	this.getUser(state, username, function (error, user) {
		...
		var index =  { username: user.username };


		// Note: Right now we only support password changes, but if we do end
		// up supporting more parameters, we should use an async.series and
		// check each thing that should be updatable. It's too dangerouse to
		// update blindly so we need to be explicit here.
		...
		crypto.randomBytes(saltSize, function (error, salt) {
			...
			user.password = that.hashMethod(changes.password, salt);
