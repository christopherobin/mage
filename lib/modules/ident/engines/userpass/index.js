...
function ensureCredentials(credentials) {
	assert(credentials.hasOwnProperty('username') && typeof credentials.username === 'string', 'invalidUsername');
	assert(credentials.hasOwnProperty('password') && typeof credentials.password === 'string', 'invalidPassword');
}
...
UserPassword.prototype.usernameToUserId = function (username) {
	assert(username && typeof username === 'string', 'invalidUsername');
...
UserPassword.prototype.getUserDoc = function (state, userId, optional, cb) {
	if (!userId) {
		return cb('invalidUserId');
	}

	var options = { optional: !!optional, mediaTypes: ['application/json'] };

	state.archivist.get(this.cfg.topic, { userId: userId }, options, function (error, doc) {
		if (!doc) {
			error = 'invalidUserId';
		}
...
UserPassword.prototype.updateUser = function (state, userId, userDoc, cb) {
	var that = this;
	...
	this.getUserDoc(state, userId, false, function (error, doc) {
		...
		state.archivist.set(that.cfg.topic, { userId: userId }, doc);
		cb();
	});
};
...
UserPassword.prototype.auth = function (state, credentials, cb) {
	...
	try {
		userId = this.usernameToUserId(credentials.username);
	...
	this.getUserDoc(state, userId, true, function (error, doc) {
		...
		try {
			assert.equal(that.hash(credentials.password, doc.credentials.salt), doc.credentials.password, 'invalidPassword');
		} catch (e) {
			return cb(e);
		}
