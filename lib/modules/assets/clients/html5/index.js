	...
	mod.init = function (assetMap) {
		...
		for (var context in assetMap.assets) {
			...
			for (var ident in map) {
				...
				newAssetsByIdent[asset.fullIdent] = asset;
				newAssetsByContext[context].push(asset);
			}
		}

		assets = newAssets;
		assetsByIdent = newAssetsByIdent;
		assetsByContext = newAssetsByContext;
	};
