(function () {
	...
	mod.init = function (assetMap) {
		var delimiter, context, assetsInContext, baseUrl, map, ident, entry, relPath, version, language, cacheability, asset;

		// create asset objects

		delimiter = assetMap.descriptorDelimiter;
		...
		for (context in assetMap.assets) {
			...
			for (ident in map) {
				entry = map[ident].split('\t');	// relPath, version, language, cacheability (tab count is guaranteed)
				relPath = entry[0];
				version = entry[1] || '1';    // defaults to 1
				language = entry[2] || null;  // defaults to null
				cacheability = +entry[3];     // defaults to 0

				asset = new Asset(context, ident, delimiter, baseUrl, relPath, version, language, cacheability);
