(function (window) {
	...
	mage.plugins.wizAssetsHandler = mod;
	...
	function assetToCachePath(asset) {
		...
		return mage.getLanguage() + '/' + mage.getDensity() + '/' + asset.fullIdent + '/' + asset.digest + (m ? '.' + m[1] : '');
	}
	...
	function downloadFile(asset, phaseName, retriesRemaining, retryDelay, cb) {
		if (!window.wizAssets) {
			// in simulation mode, we continue silently
			// we use a setTimeout to avoid stack overflow
		...
		window.wizAssets.downloadFile(
			...
			function (error) {
				...
				mod.emit('retryDownloadFile', phaseName, retriesRemaining, asset);

				window.setTimeout(function () {
					downloadFile(asset, phaseName, retriesRemaining - 1, retryDelay, cb);
				}, retryDelay);
			}
		);
	}
	...
	DownloadPlan.prototype.runPhase = function (phaseName, cb) {
		...
		forEach(
			...
			function (asset, callback) {
				...
				downloadFile(asset, phaseName, phase.retries, phase.retryDelay, function (error) {
					if (error) {
						mod.emit('failedDownloadFile', phaseName, error, asset);
					}
	...
	function createDeleteList(cacheMap) {
		...
		var currentLanguage = mage.getLanguage();
		var currentDensity = mage.getDensity();
		...
		for (path in cacheMap) {
			...
			if (m) {
				...
				if ((language !== currentLanguage && mage.appVariants.languages.indexOf(language) !== -1) ||
					(density !== currentDensity && mage.appVariants.densities.indexOf(density) !== -1)) {
					continue;
				}

				asset = mage.assets.getAsset(fullIdent);
	...
	mod.analyze = function (cb) {
		if (!mage.assets) {
			var error = new Error('MAGE assets module not found, skipping WizAssetsHandler.');
		...
		getCacheMap(function (error, currentCacheMap, isSimulation) {
			if (error) {
				// getFileURIs failure callback
			...
			assetList = mage.assets.getAllCacheable();
			assetListLen = assetList.length;

			if (isSimulation) {
				// pretend to need up to 10 assets
				// this way progress can be tested, but won't be too slow

				assetListLen = Math.min(10, assetListLen);
			}

			for (var i = 0; i < assetListLen; i++) {
				asset = assetList[i];
