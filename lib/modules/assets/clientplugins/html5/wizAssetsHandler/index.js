(function (window) {
	...
	DownloadPlan.prototype.registerAssets = function (assets) {
		...
		phases.sort(function (a, b) {
			if (a.maxCacheability && a.maxCacheability < b.maxCacheability) {
				return -1;
			}
	...
	function createDeleteList(cacheMap) {
		var key, path, m, fullIdent, version, language, asset;
		var list = [];
		...
		for (path in cacheMap) {
			...
			m = path.match(/^(.+?)-v([0-9]+)-([A-Z]+)(\.[a-zA-Z0-9]+)?$/);
			if (m) {
				fullIdent = m[1];
				version = m[2];
				...
				if (!asset || asset.version !== version || asset.language !== language) {
