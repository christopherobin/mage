(function () {
	...
	function downloadFile(list, index, callback) {
		...
		mod.emit('beforeDownloadFile', index, list.length, asset);
		...
		wizAssets.downloadFile(
			...
			function (localUrl) {
				...
				mod.emit('afterDownloadFile', index, list.length, asset);
	...
	mod.run = function (cb) {
		...
		wizAssets.getFileURIs(function (currentCacheMap) {
			...
			deleteFiles(deleteList, function () {
				...
				mod.emit('downloadsStart', downloadList.length);

				downloadFile(downloadList, 0, function (error) {

					mod.emit('downloadsComplete', downloadList.length);
