var HttpRequest = require('mage.js').httpServer.transports.http;
...
// expose Asset class to developers
exports.Asset = Asset;
...
function applyAssetMapToStylesheet(content) {
	var rules = content.cssRules || content.rules;
	if (!rules) {
		return content;
	}
...
exports.init = function (assetMap) {
	var context, assetsInContext, baseUrl, map, ident, fullIdent;
	var entry, relPath, digest, cacheability, url, asset;
	...
	for (context in assetMap.assets) {
		...
		for (ident in map) {
			...
			// create the asset object, potentially overwriting an existing asset
...
function addAssetMapsFromPackage(pkg) {
	var raw = pkg.claimContent('text/assetmap');
	var parsed;

	while (raw) {
		// parse the serialized asset map

		try {
			parsed = JSON.parse(raw);
		} catch (error) {
			console.error('Error while parsing asset map:', error);
			continue;
		}
		...
		// try to get another asset map from this package

		raw = pkg.claimContent('text/assetmap');
	}
}

function applyAssetMapToPackage(pkg) {
	function applyAssetMap(contentType) {
		var list = pkg.content[contentType] || [];

		for (var i = 0; i < list.length; i += 1) {
			list[i] = exports.applyAssetMapToContent(list[i]);
		}
	}

	applyAssetMap('text/html');
	applyAssetMap('text/css');
}

// register all asset maps of the packages already downloaded

var packages = mageLoader.listPackages();

packages.forEach(addAssetMapsFromPackage);
packages.forEach(applyAssetMapToPackage);

// when new packages come in:
// - reset existing asset maps if a language change preceded the "parsed" event
// - register all their asset maps
// - rewrite their HTML and CSS to reflect our assets knowledge

var shouldReset = false;

mageLoader.on('parsed', function (pkg) {
	if (shouldReset) {
		exports.resetAll();
		shouldReset = false;
	}

	addAssetMapsFromPackage(pkg);
	applyAssetMapToPackage(pkg);
});

mageLoader.on('language', function () {
	// on language change, we must forget about all assets as they were language-biased by whatever
	// it was before
