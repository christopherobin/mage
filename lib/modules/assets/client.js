...
function addAssetMapsFromPackage(pkg) {
	var list = pkg.content.unknown['text/assetmap'];
	if (!list) {
		return;
	}

	list.forEach(function (assetMap) {
		try {
			assetMap = JSON.parse(assetMap);
		} catch (error) {
			return console.error('Error while parsing asset map:', error);
		}

		exports.init(assetMap);
	});
}
...
mageLoader.listPackages().forEach(addAssetMapsFromPackage);
...
mageLoader.on('parsed', function (pkg) {
	...
	if (pkg.content.html) {
		pkg.content.html = exports.applyAssetMapToContent(pkg.content.html);
	}

	if (pkg.content.css) {
		pkg.content.css = exports.applyAssetMapToContent(pkg.content.css);
	}
});
