...
function keyHandler(evt) {
	...
	if (!posHandler || (evt.target && (evt.target.tagName === 'INPUT' || evt.target.tagName === 'TEXTAREA'))) {
		return;
	}
...
mageLoader.once('assets.display', function () {
	...
	function renderAssetList() {
		...
		var words = currentState.search ? currentState.search.trim().toLowerCase().split(/\s+/) : null;
		...
		for (i = 0; i < checks.length; i++) {
			...
			stats.push(check.files + ' files, ' + format.fileSize(check.size, false) + ' (' + format.fileSize(check.size, true) + ')');
			stats.push('');
		}
	...
	previewer.saveChanges = function (appName, contextName, ident, changes, files, cb) {
		mage.assets.changeAsset(appName, contextName, ident, changes, files, function (error, newLocalizedAssets) {
			if (error) {
				return cb(error);
			}
