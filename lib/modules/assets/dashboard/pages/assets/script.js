(function (window) {
	...
	mage.loader.once('assets.display', function () {
		...
		document.addEventListener('keydown', function (evt) {
			...
			switch (evt.which) {
			case ARROW_UP:
				// go up in the list and prevent scrolling
			...
			case ARROW_DOWN:
				// go down in the list and prevent scrolling
		...
		function renderAssetList(assetMap, contextName, search) {
			...
			function onchange(ident) {
				if (radioNewAsset) {
					cntAssetList.removeChild(radioNewAsset.input);
					cntAssetList.removeChild(radioNewAsset.label);
					radioNewAsset = null;
				}
			...
			btnNewAsset.onclick = function () {
				...
				radioNewAsset = ui.forms.radiobutton('newasset', '1', 'my/new/asset');

				cntAssetList.insertBefore(radioNewAsset.fragment, cntAssetList.firstChild);

				// select the name for immediate editing

				radioNewAsset.input.click();
				radioNewAsset.label.setAttribute('contenteditable', 'true');

				var range = document.createRange();

				range.selectNodeContents(radioNewAsset.label);
		...
		function renderFilters(assetMap) {
			...
			var names = Object.keys(assetMap.contexts).sort();
