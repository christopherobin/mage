...
function calcDigest(absPath, stats, cb) {
	...
	s.on('error', function (error) {
		logger.error('Error while digesting asset', absPath, error);
		cb(error);
	});
}


function Asset(absPath, relPath, context, ident, language, path, cacheability, format, digest, size, clientMatchIf, profiles) {
...
/**
 * Add the page to the list of pages to refer to in the asset map
 */
...
function recursiveFolderIterator(absPath, relPath, parallelLimit, fileHandler, finalCallback) {
			...
			function (entry, callback) {
				...
				fs.stat(entryAbsPath, function (error, stats) {
					...
					if (stats.isFile()) {
						...
						recursiveFolderIterator(entryAbsPath, entryRelPath, 1, fileHandler, callback);
					} else {
						callback();
					}
				});
...
function parsePath(relPath) {
	...
	return {
		...
		subFolders: m[3],               // directory in which basename sits (will contain a trailing slash)
		basename: m[4],                 // filename without file extension or profiles
		profileNames: m[5] ? m[5].split(',').sort() : [],  // all profiles this asset can be used for (defaults to all profiles)
...
AssetMap.prototype.addAsset = function (absPath, relPath, digest, size) {
	...
	var obj = new Asset(absPath, relPath, parsed.context, ident, parsed.language, path, cacheability, parsed.format, digest, size, clientMatchIf, parsed.profileNames);
...
AssetMap.prototype.removeAsset = function (asset) {
	...
	if (asset.route) {
		httpServer.delRoute(asset.route);
	}
...
AssetMap.prototype.exposeAsset = function (asset) {
	if (!httpServer) {
	...
	asset.route = virtualRoot + asset.path;

	httpServer.addRoute(asset.route, function (req, res) {
		if (req.method === 'GET') {
			serveFile(req, res);
		} else {
...
AssetMap.prototype.indexAsset = function (absPath, relPath, stats, cb) {
	var that = this;
	...
	calcDigest(absPath, stats, function (error, digest) {
		...
		var asset = that.addAsset(absPath, relPath, digest, stats.size);

		if (!asset) {
			return cb();
		}

		that.exposeAsset(asset);
...
AssetMap.prototype.indexFolder = function (folder, cb) {
	...
	function fileHandler(absPath, relPath, stats, callback) {
		that.indexAsset(absPath, relPath, stats, callback);
	}

	recursiveFolderIterator(folder, '', DIGEST_MAX_PARALLEL, fileHandler, function (error) {
		logger.time('Asset map indexing took', Date.now() - startTime, 'msec');
...
AssetMap.prototype.reset = function () {
	...
	for (var i = 0, len = assetList.length; i < len; i++) {
		if (assetList[i].route) {
			httpServer.delRoute(assetList[i].route);
		}
	}
};
...
AssetMap.prototype.updateVariants = function (context, ident, changes, files, cb) {
	var that = this;
	...
	for (i = 0; i < changes.length; i++) {
		...
		if (change.originalAsset && (change.operation === 'delete' || typeof change.file === 'number')) {
	...
	function createTempRelPath(change) {
		...
		return createRelPath(context, ident, change.language, change.profiles, change.format + '-tmp' + lastTempId);
	}

	function makeAbs(relPath) {
		var folder = that.src.folders[0];
		if (folder) {
			return pathResolve(pathDirname(process.mainModule.filename), folder, relPath);
		}
	}
	...
	for (i = 0; i < changesCopy.length; i++) {
		...
		targetRelPath = createRelPath(context, ident, change.language, change.profiles, change.format);
		...
		sourceAbsPath = change.originalAsset ? change.originalAsset.absPath : makeAbs(sourceRelPath);
		...
		if (change.originalAsset) {
			...
			change.originalAsset = this.addAsset(targetAbsPath, targetRelPath, change.originalAsset.digest, change.originalAsset.size);

			this.exposeAsset(change.originalAsset);
		}
	...
	for (i = 0; i < changes.length; i++) {
		...
		if (change.operation === 'delete' || typeof change.file !== 'number') {
			continue;
		}

		var file = files[change.file];
		if (!file || !file.data || !file.data.length) {
			logger.error('No file found in change:', change);
			continue;
		}
		...
		// updating the asset map happens once the writes are done, so digests are calculated and cached
	}
	...
	function runWrites(callback) {
		async.forEachSeries(
			...
			function (write, callback) {
				...
				try {
					stream = fs.createWriteStream(write.absPath, { flags: 'w' });
				} catch (error) {
					return safeCallback(error);
				}

				stream.once('error', safeCallback);
				stream.once('close', function (error) {
					if (error) {
						return safeCallback(error);
					}
					...
					fs.stat(write.absPath, function (error, stats) {
						...
						that.indexAsset(write.absPath, write.relPath, stats, function () {
				...
				stream.once('open', function () {
					for (var i = 0; i < write.buffers.length; i++) {
						stream.write(write.buffers[i]);
					}
	...
	async.forEachSeries(
		...
		function (asset, callback) {
			...
			moveFile(asset.absPath, newAbsPath, function (error) {
				...
				var newAsset = that.addAsset(newAbsPath, newRelPath, asset.digest, asset.size);

				that.exposeAsset(newAsset);
...
exports.renameAsset = function (state, appName, context, oldIdent, newIdent, cb) {
	...
	assetMap.reindex(false, function (error) {
		...
		assetMap.renameAsset(state, context, oldIdent, newIdent, function (error) {
			...
			var assetsByLanguage = assetMap.assets[context] ? assetMap.assets[context][newIdent] : null;
...
exports.updateVariants = function (state, appName, context, ident, changes, files, cb) {
	var assetMap = allAssetMapsMap[appName];
	if (!assetMap) {
		return state.error(null, 'Asset map not found', cb);
	}
	...
	assetMap.reindex(false, function (error) {
		...
		assetMap.updateVariants(context, ident, changes, files, function (error) {
			if (error) {
				return state.error(null, error, cb);
			}

			var assetsByLanguage = assetMap.assets[context] ? assetMap.assets[context][ident] : null;
...
exports.getAssetMap = function (appName, cb) {
	...
	map.reindex(false, function (error) {
		if (error) {
			return cb(error);
		}
