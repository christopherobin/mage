...
AssetMap.prototype.getBaseUrl = function (context, headers) {
	...
	logger.debug(
		...
		'(reverting to built-in asset serving, MAGE recommends that you Upload your assets to a',
		'CDN and configure a URL for that in your game\'s config file.)'
	);
	...
	return httpServer.getBaseUrl(headers) + virtualRoot;
};
...
function parsePath(relPath) {
	...
	if (!m) {
		logger.verbose('Not identified as an asset:', relPath);
		return;
	}
