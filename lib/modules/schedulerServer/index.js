...
module.exports = {
	setup: function (state, cb) {
		...
		if (cfg && cfg.debug) {
			...
			options.ttl = 600;
			mithril.core.logger.notice('SchedulerServer: Debug activated. Using prefix ' + options.prefix + ' and ttl=' + options.ttl + '.');
		} else {
			// Use no prefix and persist things forever.
			mithril.core.logger.notice('SchedulerServer: Production mode.');
		}
	...
	scheduleCommand: function (state, client, taskName, scheduleInfo, data, cb) {
		this.registry.scheduleCommand(state, client, taskName, scheduleInfo, data, cb);
	},
	reload: function (state, cb) {
		this.registry.reloadFromStore(state, cb);
	},
	...
	nuke: function (state, cb) {
		this.registry.nukeStore(state, cb);
	},
