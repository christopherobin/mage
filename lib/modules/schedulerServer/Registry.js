...
var rumpelstiltskin = require('rumplestiltskin');
...
var stableHash = function () {
	...
	for (var i in arguments) {
		if (arguments.hasOwnProperty(i)) {
			h.update(rumpelstiltskin.trueName(arguments[i]));
		}
	}
...
Registry.prototype.createSchedule = function (key, info, deleteExisting) {
	...
	if (typeof info === 'object') {
		info = rumpelstiltskin.trueName(info);
	}
...
Registry.prototype.reloadFromStore = function (state, cb) {
	...
	var error = function (err) {
		that.shutdown();
		state.error(null, 'SchedulerServer: Got an error while reloading state from store: ' + err, cb);
	};
