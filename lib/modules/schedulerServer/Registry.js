...
var CommandCenterClient = mage.core.CommandCenterClient;

var logger = mage.core.logger.context('schedulerServer');

/**
 * Uses rumplestiltskin to create a reliable hash from data. Takes any number of arbitrary
 * arguments. Internally this now uses rumplestiltskin for stable hash of each argument. A hash is
 * distilled to an md5 hex string to maintain reasonable size.
 *
 * @return {String}
 * @private
 */
...
/**
 * Constuct a key using the client and a task name.
 *
 * @param  {Registry} registry An instance of Registry.
 * @param  {Object}   client   Client endpoint, eg: { app: 'game', host: 'localhost', port: 80, auth: 'base64str' }
...
/**
 * Invoke tasks associated with a schedule key.
 *
 * @param {Registry} registry    An instance of Registry.
 * @param {String}   scheduleKey A key for the schedule.
 */

function invokeTasks(registry, scheduleKey) {
	logger.debug('Invoking tasks for schedule', registry.infos[scheduleKey].spec);
	...
	for (var i = 0; i < taskKeys.length; i++) {
		...
		if (!task) {
			logger.error('Can\'t find task for key', taskKey);
			continue;
		}

		if (!registry.peers[taskKey]) {
			registry.peers[taskKey] = new CommandCenterClient(task.client, logger);
		}
 ...
 *
 * @param  {Object}           registry An instance of Registry.
 * @param  {String}           taskKey  The key of the task we need to delete.
 * @return {String|undefined}          The task's schedule key.
 */

function deleteTask(registry, taskKey) {
	var task = registry.tasks[taskKey];
	...
	if (registry.tasksForSchedule[schedule]) {
		var index = registry.tasksForSchedule[schedule].indexOf(taskKey);
...
function createSchedule(registry, key, details) {
	if (registry.infos[key]) {
		return true;
	}
	...
	registry.schedules[key].on('run', function () {
		invokeTasks(registry, key);
	});
...
Registry.prototype.reloadFromStore = function (state, cb) {
	logger.debug('Reloading state from store...');
	...
	kv.getOne(this.masterKey, false, null, function (err, schedules) {
		...
		kv.getMany(scheduleKeys, false, null, function (err, taskIndex) {
			...
			kv.getMany(allTaskKeys, false, null, function (err, allTasks) {
				if (err) {
					logger.error('Error loading allTaskKeys', err);
					return errorHandler();
				}
 ...
 *
 * @param {Object}        state
 * @param {}              client       Client endpoint, eg: { app: 'game', host: 'localhost', port: 80, auth: 'base64str' }
 * @param {String}        taskName
...
Registry.prototype.scheduleCommand = function (state, client, taskName, scheduleInfo, data, cb) {
	...
	// If a task got deleted, update the data for the schedule that changed
	...
	if (!this.schedules[scheduleKey]) {
		if (!createSchedule(this, scheduleKey, scheduleInfo)) {
			return state.error(null, new Error('Invalid schedule ' + JSON.stringify(scheduleInfo)), cb);
		}
	...
	if (!addTask(this, taskKey, task)) {
		return state.error(null, new Error('Can\'t add task ' + JSON.stringify(task)), cb);
	}
