...
(function (window) {
	var mithril = window.mithril,
		schedulerServer = mithril.schedulerServer;
	...
	var SchedulerServerAdminPage = function () {
		var self = this;
	...
	SchedulerServerAdminPage.prototype = {
		...
		init: function () {
			...
			self.ui = {
				root: document.querySelector('.schedulerServerTool')
			};
			self.ui.admin = self.ui.root.querySelector('.admin ul.list');
			self.ui.schedules = self.ui.root.querySelector('.schedules ul.list');
			self.ui.scheduleInfo = {
				root: self.ui.root.querySelector('.schedule_info')
			};
			self.ui.scheduleInfo.table = self.ui.scheduleInfo.root.querySelector('.table');
			self.ui.scheduleInfo.events = self.ui.scheduleInfo.root.querySelector('.events');
			self.ui.scheduleInfo.tasks = self.ui.scheduleInfo.root.querySelector('.tasks');
		...
		selectSchedule: function (scheduleKey) {
			...
			schedulerServer.getScheduleInfo(scheduleKey, self.dimmer.async(function (error, result) {
				...
				var numberOfRows = rows.length >>> 0, i;

				self.ui.scheduleInfo.table.textContent = '';
				for (i = 0; i < numberOfRows; ++i) {
					var row = self.ui.scheduleInfo.table.appendChild(document.createElement('tr')),
				...
				var clientsListElement = self.ui.scheduleInfo.tasks.appendChild(document.createElement('ul')),
					sortedTasks = {};
		...
		refresh: function () {
			...
			schedulerServer.listSchedules(self.dimmer.async(function (error, result) {
				if (error) {
					...
					var clickHandlerFor = function (key) {
							return function () {
								self.selectSchedule(key);
							};
						};
					...
					for (var scheduleKey in result) {
						...
						button.addEventListener('click', clickHandlerFor(scheduleKey), false);
					}
	...
	schedulerServer.onceSetup(function () {
		var page = new SchedulerServerAdminPage();
	});
}(window));
