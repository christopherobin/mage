...
var HeartBeat = require(__dirname + '/HeartBeat');
var Duration = require(__dirname + '/Duration');
var Cron = require(__dirname + '/Cron');
var Schedule = require(__dirname + '/Schedule');
var CommandCenterClient = require(__dirname + '/CommandCenterClient');
...
/**
 * Initialises the commandCenterClient singleton if necessary.
 *
...
function getCommandCenterClient() {
	if (!(commandCenterClient instanceof CommandCenterClient)) {
		commandCenterClient = new CommandCenterClient({
			...
			port: mithril.core.config.get('module.scheduler.port', 80),
			auth: mithril.core.config.get('module.scheduler.auth')
		});
	}
...
exports.schedule = function (appName, taskName, schedule, data, cb) {
	...
	var auth = mithril.core.config.get('module.scheduler.callbackAuth');
	if (auth) {
