...
exports.schedule = function (appName, taskName, schedule, data, cb) {
	...
	var params = {
		...
		client: {
			app: appName,
			host: config.get('server.clientHost.expose.host'),
			port: config.get('server.clientHost.expose.port')
		},
		taskName: taskName,
		schedule: schedule
	};
	...
	logger.info("Scheduling task " + appName + "/" + taskName + " to run on " + JSON.stringify(schedule));
