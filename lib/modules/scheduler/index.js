    ...
    getCommandCenterClient = function () {
    	if (!(commandCenterClient instanceof CommandCenterClient)) {
    		commandCenterClient = new CommandCenterClient({
    			app:  mage.core.config.get('module.scheduler.name', 'shokoti'),
    			host: mage.core.config.get('module.scheduler.host'),
    			port: mage.core.config.get('module.scheduler.port', 80),
    			auth: mage.core.config.get('module.scheduler.auth')
    		});
    	}
 ...
 * @param {Object}   data
 * @param {Function} cb       Callback called upon completion
 * @api public
 */
exports.schedule = function (appName, taskName, schedule, data, cb) {
	...
	var params = {
		...
		client: {
			app: appName,
			host: mage.core.config.get('server.clientHost.expose.host'),
			port: mage.core.config.get('server.clientHost.expose.port')
		},
		taskName: taskName,
		schedule: schedule
	};
	...
	var auth = mage.core.config.get('module.scheduler.callbackAuth');
	if (auth) {
	...
	mage.core.logger.info("Scheduling task " + appName + "/" + taskName + " to run on " + JSON.stringify(schedule));
