...
function Cron(crontab, start, end) {
	if (isString(crontab) && crontab.length > 0 && (crontab[0] === '"' || crontab[0] === '{')) {
		try {
	...
	FIELD_NAMES.forEach(function (fieldName, i) {
		var rem = cr[i];
		var minStart = SPECS[i].range[0];
		var maxEnd = SPECS[i].range[1];
		...
		if ((m = FIELD_RE.exec(rem))) {
			...
			while ((m = FIELD_ITEM_RE.exec(rem))) {
				...
				var end = Number(m[2]);
				var wildcard = m[3];
				var step = Number(m[4]) || 1;
				var value = Number(m[5]);
		...
		Object.defineProperty(self, fieldName, { enumerable: false, writable: false, value: prop });
	});
...
/**
 * Sets the Cron to the next due date and return the due date.
 *
