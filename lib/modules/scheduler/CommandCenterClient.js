...
function onTransportError(cmdCtrClient, failedQueue) {
	...
	logger.debug('Will retry', failedQueue.length, 'command(s)...');
...
function sendQueue(cmdCtrClient) {
	...
	var req = http.request(reqOptions, function (res) {
		...
		res.on('close', function () {
			logger.error('Disconnected while receiving response from peer', cmdCtrClient.peer);
			onTransportError(cmdCtrClient, queue);
		});

		if (res.statusCode !== 200) {
			logger.error('Got an unexpected status from peer', cmdCtrClient.peer, ':', res.statusCode);
			// Will not retry
		}
	});

	req.on('error', function (e) {
		logger.error('Got a transport error from peer', cmdCtrClient.peer, ':', e.message);
		onTransportError(cmdCtrClient, queue);
	});
