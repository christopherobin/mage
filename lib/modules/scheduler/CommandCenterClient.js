...
function onTransportError(cmdCtrClient, failedQueue) {
	...
	cmdCtrClient.logger.debug('Will retry', failedQueue.length, 'command(s)...');
...
function sendQueue(cmdCtrClient) {
	...
	var req = http.request(reqOptions, function (res) {
		...
		res.on('close', function () {
			cmdCtrClient.logger.error('Disconnected while receiving response from peer', cmdCtrClient.peer);
			onTransportError(cmdCtrClient, queue);
		});

		if (res.statusCode !== 200) {
			cmdCtrClient.logger.error('Got an unexpected status from peer', cmdCtrClient.peer, ':', res.statusCode);
			// Will not retry
		}
	});

	req.on('error', function (e) {
		cmdCtrClient.logger.error('Got a transport error from peer', cmdCtrClient.peer, ':', e.message);
		onTransportError(cmdCtrClient, queue);
	});
...
function CommandCenterClient(config, logger) {
	this.logger = logger;
	this.app = config.app;
