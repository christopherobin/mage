...
exports.httpReqLogin = function (request, path, params, cb) {
	...
	exports.send('get', { token: user.token, tokenSecret: user.tokenSecret }, '/Gameplatform/UserMe', { user_id: user.userId }, null, function (error, data) {
		if (error) {
		...
		exports.getUserProperties(state, user.userId, { load: ['actorId'] }, function (error, props) {
			...
			if (actorId) {
				// The player already exists, so we can save changes and return.

				props.save(callback);
			} else {
				// New player event

				exports.onNewPlayer(state, user, user.userId, function (error, actorId) {
					if (error) {
						return callback(error);
					}
...
exports.httpReqGetSession = function (request, path, params, cb) {
	...
	function callback(error, response, headers) {
		state.close();
	...
	exports.getUserProperties(state, giraffeUserId, { load: ['actorId', 'token', 'tokenSecret'] }, function (error, userProps) {
		...
		if (!actorId || !token || !tokenSecret) {
			return state.error(null, 'User ' + giraffeUserId + ' has no actorId, token or tokenSecret.', callback);
		}
		...
		if (hash !== validhash) {
			return state.error(null, 'Hash is not valid for giraffeUserId ' + giraffeUserId, callback);
		}
		...
		exports.send('get', { token: token, tokenSecret: tokenSecret }, '/Gameplatform/UserMe', { user_id: giraffeUserId, access_token: token, access_secret: tokenSecret }, null, function (error, data) {
			if (error) {
				state.error(null, error, callback);
			}

			if (!data || !data.profile) {
				return state.error(null, "Couldn't retrieve user information from giraffe service.", callback);
			}
			...
			exports.onLogin(state, actorId, data, function (error) {
				...
				mithril.session.register(state, actorId, function (error, session) {
					...
					callback(null, JSON.stringify(response), { 'content-type': 'application/json; charset=utf-8' });
				});
			});
		});
	});
