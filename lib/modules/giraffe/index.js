...
function messageHook(state, memo, params, message, cb) {
...
exports.send = function (httpMethod, user, path, getParams, postData, cb) {
	...
	oauth.getProtectedResource(url + '?' + getParams, 'GET', user.token, user.tokenSecret,  function (error, data, response) {
		if (error) {
...
function pushNotificationToUser(state, giraffeUserId, message, options, cb) {
	...
	exports.getUserProperties(state, giraffeUserId, { load: ['token', 'tokenSecret'] }, function (error, props) {
		...
		exports.send('get', user, '/Gameplatform/ApplicationNotificationMessage', params, null, cb);
	});
}
