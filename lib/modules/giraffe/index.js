	...
	async.waterfall([
		...
		function (data, callback) {
			if (!data || !data.playerId) {
				return state.error('auth', 'Giraffe user is not registered.', callback);
			}
		...
		function (data, callback) {
			...
			if (!data || !data.token || !data.tokenSecret) {
				return state.error('auth', 'User has no token/tokenSecret', callback);
			}
			...
			if (hash !== validhash) {
				return state.error('auth', 'Giraffe hash is not valid', callback);
			}

			// Get latest user data from Giraffe API

			exports.send('get', {token: token, tokenSecret: tokenSecret}, '/Gameplatform/UserMe', { user_id: giraffeUserId, access_token: token, access_secret: tokenSecret }, null, callback);
		},
		function (data, callback) {
			if (!data || !data.profile) {
				return state.error('auth', "Couldn't retrieve Giraffe data", callback);
			}
