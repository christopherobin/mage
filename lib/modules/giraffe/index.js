...
var tryAuthenticate = function (state, request, path, reqParams, resolvePlayer, cb) {
	...
	var query = 'SELECT a.id AS playerId, ad2.value AS token, ad3.value AS tokenSecret FROM actor AS a JOIN actor_data AS ad1 ON a.id = ad1.actor AND ad.property = ? AND ad1.value = ? JOIN actor_data AS ad2 ON a.id = ad2.actor AND ad2.property = ? JOIN actor_data AS ad3 ON a.id = ad3.actor AND ad3.property = ?';
