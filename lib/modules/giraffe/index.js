...
var handleHttpRequest = function (request, path, params, cb) {
	mage.core.logger.info('Giraffe: received request: ' + path);
...
function resolveActor(state, actorId, optional, cb) {
	mage.actor.getActorProperties(state, actorId, { load: ['giraffeUserId'] }, function (error, props) {
		if (error) {
			return cb(error);
		}
...
function resolveActors(state, actorIds, cb) {
	...
	mage.actor.getActorsProperties(state, actorIds, { load: ['giraffeUserId'] }, function (err, props) {
		if (err) {
			return cb(err);
		}
		var map = {};
...
exports.getUserProperties = function (state, userId, options, cb) {
	...
	mage.core.LivePropertyMap.create(state, domain, options, cb);
};
...
exports.getUsersProperties = function (state, userIds, options, cb) {
	...
	mage.core.LivePropertyMap.createMany(state, domains, options, cb);
};
...
exports.setup = function (state, cb) {
	cfg = mage.core.config.get('module.giraffe');

	if (!cfg) {
		mage.core.logger.error('Giraffe module has no configuration');
		return cb(true);
	}
	...
	for (var api in cfg.expose) {
		...
		if (httpServer) {
			...
			mage.core.logger.error('Could not add route ' + path + ', because there is no HTTP server.');
		}
	}

	oauth = new OAuth('', '', cfg.consumer.key, cfg.consumer.secret, '1.0', null, 'HMAC-SHA1');

	mage.core.cmd.registerMessageHook('giraffe', messageHook);
...
function resolvePlayer(state, playerId, cb) {
	...
	mage.actor.getProperties(state, state.actorId, ['giraffeUser', 'token', 'tokenSecret'], function (err, data) {
		if (err) {
			return cb(err);
		}
...
exports.httpReqLogin = function (request, path, params, cb) {
	...
	function callback(error) {
		state.close(function () {
			if (error) {
				mage.core.logger.error('Giraffe login error:', error);

				// error can be an Error object, so we cast to string (so we can respond on the http transport)
				cb(401, String(error));
			} else {
				cb(200, '{"success":true}');
			}
	...
	exports.send('get', { token: user.token, tokenSecret: user.tokenSecret }, '/Gameplatform/UserMe', { user_id: user.userId }, null, function (error, userData) {
		...
		exports.getUserProperties(state, user.userId, { optional: ['actorId'] }, function (error, props) {
			...
			exports.onNewPlayer(state, user, userData, function (error, actorId) {
				...
				mage.actor.getActorProperties(state, actorId, {}, function (error, actorProps) {
					if (error) {
						return callback(error);
					}
...
exports.getSession = function (state, giraffeUserId, nonce, timestamp, hash, cb) {
	...
	exports.getUserProperties(state, giraffeUserId, { load: ['actorId', 'token', 'tokenSecret'] }, function (error, userProps) {
		...
		exports.send('get', { token: token, tokenSecret: tokenSecret }, '/Gameplatform/UserMe', { user_id: giraffeUserId, access_token: token, access_secret: tokenSecret }, null, function (error, data) {
			...
			mage.session.register(state, actorId, function (error, session) {
				if (error) {
					return cb(error);
				}
...
exports.send = function (httpMethod, user, path, getParams, postData, cb) {
	...
	var fnReturnCb = function (error, data, response) {
		if (error) {
			mage.core.logger.error('Giraffe API call error:', error);
			return cb(error);
		}
		...
		mage.core.logger.debug('GIRAFFE: A', data);
	...
	default:
		mage.core.logger.error('Invalid httpMethod given to giraffe.send:', httpMethod);
		cb(true);
		break;
	}
};
...
function pushNotificationToUser(giraffeUserId, tokenInfo, message, options, cb) {
	...
	if (!message) {
		mage.core.logger.error('No message provided for the push notification to user ' + giraffeUserId);
		return cb('noNotificationMessage');
	}

	if (!tokenInfo.token || !tokenInfo.tokenSecret) {
		mage.core.logger.error('Giraffe user data is missing for user ' + giraffeUserId);
		...
		exports.getUsersProperties(state, userIds, { load: ['token', 'tokenSecret'] }, function (error, props) {
			...
			async.forEachSeries(
				...
				function (actorId, callback) {
					...
					if (!userProps) {
						mage.core.logger.error('No user properties found for user', userId);
						return callback();
					}

					var message = messages[actorId];
					if (!message) {
						mage.core.logger.error('No notification message for user', userId);
						return callback();
					}
