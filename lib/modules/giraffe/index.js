...
function tryAuthenticate(state, request, path, reqParams, resolvePlayer, cb) {
	...
	var query = 'SELECT ad.actor AS playerId, ad2.value AS token, ad3.value AS tokenSecret FROM actor_data AS ad JOIN actor_data AS ad2 ON ad2.actor = ad.actor AND ad2.property = ? JOIN actor_data AS ad3 ON ad3.actor = ad.actor AND ad3.property = ? WHERE ad.property = ? and ad.value = 32';
	...
	state.datasources.db.getOne(query, params, false, null, function (error, result) {
		...
		if (result && result.token !== user.token) { //user has switched device or something
			//store new values
			var pm = new mithril.core.propertyMap();
			pm.add(token, user.token);
			pm.add(tokenSecret, user.tokenSecret);
			mithri.actor.setProperties(state, result.playerId, pm, function (err) {
				if (err) {
					return cb(err);
				}
