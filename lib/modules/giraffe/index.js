...
var tryAuthenticate = function (state, request, path, reqParams, resolvePlayer, cb) {
	...
	if (!reqParams.hasOwnProperty('opensocial_app_id') || !oauth.isValidAppId(reqParams.opensocial_app_id)) {
	...
	var query = 'SELECT a.id AS playerId, ad2.value AS token, ad3.value AS tokenSecret FROM actor AS a JOIN actor_data AS ad1 ON a.id = ad1.actorId AND ad.property = ? AND ad1.value = ? JOIN actor_data AS ad2 ON a.id = ad2.actorId AND ad2.property = ? JOIN actor_data AS ad3 ON a.id = ad3.actorId AND ad3.property = ?';
