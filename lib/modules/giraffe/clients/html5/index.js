(function () {
	...
	var mod = mage.registerModule($html5client('module.giraffe.construct'));
	...
	mage.io.registerCommandHook('giraffe', function (messageData) {
	...
	// requestSession asks the mage server to generate a session ID for this user
	...
	function requestSession(cb) {
		...
		mod.getSession(info.userId, info.nonce, info.timestamp, info.hash, function (error, sessionId) {
			...
			mage.session.setSessionKey(sessionId);
			cb();
		});
	}
	...
	// request a mage session to be started.
