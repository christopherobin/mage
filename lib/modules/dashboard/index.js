...
var registeredPages = {};
var appNames = ['dev', 'cms', 'support'];

mage.useModules('ident');



function createDashboard(appName, cb) {
	...
	function addPage(page) {
		...
		if (!page.hasOwnProperty('listed') || page.listed) {
			registeredPages[appName].push({
				name: page.fileName,
				fullName: page.name
			});
		}

		logger.verbose('Added dashboard page:', page.fileName, 'to app:', appName);
	}
	...
	// And add login, since they come first

	loader = app.addIndexPage('loader', pathJoin('./node_modules/mage/lib/modules/dashboard', 'dashboard/dashboardLoader'));
	loader.registerComponent('login', pathJoin('./node_modules/mage/lib/modules/dashboard', 'dashboard/pages/login'), { assetMap: true });
...
exports.setupDashboardApps = function (cb) {
	async.forEachSeries(
		appNames,
		function (appName, callback) {
			createDashboard(appName, callback);
		},
		cb
	);
};
...
exports.getPages = function (appName) {
	return registeredPages[appName] || [];
};
