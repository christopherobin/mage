...
function setNode(resolved) {
	...
	if (upPath && upPath.fullPath !== resolved.fullPath) {
		values.push({
			toString: function () {
				return '..';
			},
	...
	Object.keys(resolved.node.folders).forEach(function (folderName) {
		values.push({
			toString: function () {
				return folderName;
			},
			click: function () {
				setNode(resolve(folderName));
			}
		});
	});
	...
	resolved.node.files.forEach(function (fileName) {
		values.push({
			toString: function () {
				return stripFileExt(fileName);
			},
			click: function () {
				setNode(resolve(fileName));
			}
		});
	});
	...
	if (resolved.fileName) {
		...
		radios.inputs[stripFileExt(resolved.fileName)].setAttribute('checked', 'checked');
	}
}
