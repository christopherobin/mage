...
var items = [
	{ name: 'Start', cb: function () { setNode('/'); } },
	{ name: 'MAGE', cb: function () { setNode('/node_modules/mage'); } },
	{ name: 'Dependencies', cb: function () { setNode('/node_modules'); } }
];
...
ui.router.listen(/^docs$/, function () {
	...
	if (current.relPath) {
		...
		if (current.fileName) {
			fullPath = pathJoin(current.relPath, current.fileName);
		} else {
			fullPath = current.relPath;
		}
