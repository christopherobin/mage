...
mage.httpServer.setCmdMode('free'); // user commands do not block each other on the dashboard
...
function login(engineName, credentials, errorElm) {
	...
	mage.ident.login(engineName, credentials, function (error) {
		if (error) {
			loggingIn = false;
		...
		mageLoader.loadPackage('postlogin', function (error) {
			if (error) {
...
mage.ident.getEngines(function (error, engines) {
	...
	var pkg = mageLoader.getPackage('login');
