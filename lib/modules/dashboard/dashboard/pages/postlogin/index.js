...
var pkg = require('mage-loader.js').getPackage('postlogin');
...
ui.sidebar.load(function (error) {
	if (error) {
		phantomMsg({ exit: 1, msg: 'Error loading dashboard: ' + error });
	} else {
...
mage.eventManager.on('session.unset', function (path, data) {
