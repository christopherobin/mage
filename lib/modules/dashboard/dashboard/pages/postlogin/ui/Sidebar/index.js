...
function Sidebar(parent) {
	...
	for (var i = 0; i < dotCount; i++) {
		...
		dot.style.animation = 'pulse 1s linear -' + Math.round((dotCount - 1 - i) * 1000 / dotCount) + 'ms infinite alternate';
		dot.style.webkitAnimation = 'pulse 1s linear -' + Math.round((dotCount - 1 - i) * 1000 / dotCount) + 'ms infinite alternate';
	...
	msgServer.on('io.resend', function () {
		// lower the semaphore by 1 to compensate for the "io.send" event that is about to fire
		that.sidebarLocks -= 1;
	});

	msgServer.on('io.response', function () {
...
Sidebar.prototype.addPage = function (name, fullName, cb) {
	...
	mageLoader.once(name + '.loaded', function () {
		...
		button.onclick = function () {
			if (that.sidebarLocks > 0) {
				console.warn('Trying to open', name, 'while sidebar was locked.');
				return;
			}
