...
TomeRenderer.prototype._json = function (value) {
	...
	span.className = Tome.typeOf(value);
	...
	span.onclick = function (e) {
		...
		var lastType = Tome.typeOf(value);

		span.oninput = function () {
			try {
				lastType = span.className = Tome.typeOf(JSON.parse(this.textContent));
			} catch (err) {
				span.className = lastType + ' error';
			}
		};
...
DocEditor.prototype.setDocument = function (doc, mediaType) {
	...
	if (this.mediaType !== mediaType) {
		...
		if (!Renderer) {
			throw new Error(
				'No renderer found for mediaType: ' + mediaType + '. ' +
				'Supported: ' + Object.keys(DocEditor.renderers)
			);
		}
