			...
			mithril.setup(function (error) {
				if (error) {
					...
					mithril.gm.getTools(function (error, toolsList) {
						...
						var toolLinks = document.querySelectorAll('.toolLink');
						for(i = 0; i < toolLinks.length; i++) {
							var target = toolLinks[i].getAttribute('data-target');
							toolLinks[i].onclick = (function(to) {　return function () { mithril.loader.loadPage(to) }})(target);
						}
