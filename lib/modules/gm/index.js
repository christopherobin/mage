...
exports.setup = function (state, cb) {
	var sql = 'SELECT g.actor, g.username, g.password, gd.property, gd.language, gd.type, gd.value FROM gm AS g LEFT JOIN gm_data AS gd ON g.actor = gd.actor';
	
	state.datasources.db.getMany(sql, [], null, function (error, rows) {
		if (error) {
			return cb(error);
		}
		...
		for (var i = 0, len = rows.length; i < len; i++) {
			...
			if (!gmCacheMap[row.actor]) {
				gmCacheMap[row.actor] = {
					username: row.username,
					password: row.password,
					data: new mithril.core.PropertyMap()
				};
			}

			if (row.property) {
				gmCacheMap[row.actor].data.importOne(row.property, row.type, row.value, row.language);
			}
		}
...
function messageHook(state, params, messageData, cb) {
	...
	cache.get(key, function (error, value) {
		...
		if (!value) {
			...
			var sql = 'SELECT actor FROM gm WHERE actor = ?';
			var params = [id];
			...
			state.datasources.db.getOne(sql, params, false, null, function (error, result) {
				...
				if (!result) {
					...
					cache.set(key, JSON.stringify(data), null, cb);
				}
			});
		}
	});
...
//	console.log('toolApp ', util.inspect(toolApp, false, null));
	...
	var commandCenter = mithril.addCommandCenter(
		'tool',
		{ hooks: ['mithril.session', 'gm.admin'] },
		commands
	);
		...
		if (exports.checkPassword(password, gm.password)) {
			mithril.session.register(state, gm.actor, function (error, session) {
				if (!error) {
					...
					gmCacheMap[gm.actor].session = sessionKey
					state.respond(sessionKey);
				} 
...
exports.addGm = function (state, username, newPassword, rights, callback) {
	var actorId;
	var password;
	var propMap;
	...
	async.waterfall([
		...
		function (callback) {
			...
			password = crypto.createHmac('sha1', config.hashkey).update(newPassword).digest('hex');
		...
		function (info, callback) {
			propMap = new mithril.core.PropertyMap();
...
exports.editGm = function (state, actor, password, rights, cb) {
	...
	if (password) {
		...
		state.datasources.db.exec(sql, args, null, function (error) {
			...
			gmCacheMap[actor].password = pass;
...
exports.delGm = function (state, actor, cb) {
...
exports.setProperties = function (state, actorId, propertyMap, cb) {
	...
	state.datasources.db.exec(sql, params, null, function (error) {
		...
		gmCacheMap[actorId].data = propertyMap;
		cb();
	});
};
