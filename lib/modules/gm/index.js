...
function messageHook(state, params, messageData, cb) {
	if (!state.session) {
		return state.error(null, 'The session message-hook is required for the GM message-hook to work.', cb);
	}

	if (state.session.meta && state.session.meta.isAdmin) {
		return cb();
	}

	return state.error('auth', 'This actor is not marked as administrator.', cb);
}

mage.core.cmd.registerMessageHook('gm.admin', messageHook);
...
exports.createTools = function (cb) {
	...
	async.forEachSeries(
		...
		function (error) {
			...
			toolApp.commandCenter.expose({ hooks: ['mage.session', 'gm.admin'] }, commands);
...
exports.login = function (state, username, password, cb) {
	...
	state.datasources.db.getOne(sql, [username], false, null, function (error, gm) {
		...
		state.datasources.db.getOne(rightsSql, [gm.actor], false, null, function (error, result) {
			...
			mage.session.register(state, gm.actor, null, { isAdmin: true }, function (error, session) {
				if (error) {
					return cb(error);
				}
