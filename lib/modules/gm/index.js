...
exports.createTool = function (appName, cb) {
	...
	toolApp.assetMap.setup({ baseUrl: {} });

	// Set up a loader (login screen)
	// And add login and home, since they come first

	toolApp.setIndexPage(pathJoin(__dirname, 'tool/loader'));
	addPage(pathJoin(__dirname, 'tool/pages/login'), { assetMap: true });
	addPage(pathJoin(__dirname, 'tool/pages/home'));
	...
	async.forEachSeries(
		...
		function (modName, callback) {
			...
			var assetsPath = pathJoin(modPath, 'tool/assets');
			var pagesPath = pathJoin(modPath, 'tool/pages');
