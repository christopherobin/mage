...
(function (window) {
	...
	function addGm() {
		...
		mage.gm.createGm(username, password, rights, function (error, id) {
			if (error) {
	...
	function editGm() {
		...
		mage.gm.editGm(curGm, newPass, rights, function (error) {
			if (!error) {
			...
			buttons: [
				{
					...
					click: function () {
						if (confirm('Are you sure you want to delete this Gm?')) {
							mage.gm.deleteGm(curGm, function (error) {
								if (!error) {
	...
	mage.loader.on('gm.display', function () {
		if (!setup) {
			setup = true;
			...
			mage.gm.getGms(function (error, gms) {		// List Gms
				if (error) {
