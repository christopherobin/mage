...
// Make default function used in this file in case window.console does not exist
...
Object.keys(consoleLogChannels).forEach(function (methodName) {
	if (typeof log[methodName] === 'function') {
		...
		log['_' + methodName] = log[methodName];
	} else if (typeof log[methodName] === 'object') {
		// Exist as object in IE9, rebinding to use it as function
		log['_' + methodName] = Function.prototype.bind.call(log[methodName], log);
	}
});
...
ServerWriter.prototype.addChannel = function (channelName) {
	if (!logger.hasOwnProperty('sendReport')) {
		log._error('logger.sendReport usercommand is not exposed.', channelName);
		return;
	}
	...
	var clientInfo = {
		...
		clientConfig: mage.getClientConfig()
	};
	...
	logger.on(channelName, function (args) {
		...
		mage.msgServer.queue(function () {
			logger.sendReport(channelName, report.message, report.data, function (error) {
				if (error) {
					log._error('Could not forward logs to remote server:', error);
				}
			});
		});
	});
...
function getOrCreateWriter(writerType) {
	...
	if (!WriterClass) {
		log._error('Unknown writer type:', writerType);
		return;
	}
...
logger.overrideConsole = function () {
	Object.keys(consoleLogChannels).forEach(function (methodName) {
		...
		log[methodName] = function readFromConsole() {
			logger.emit(channelName, arguments);
		};
	});
};
