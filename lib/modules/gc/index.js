...
function addCachedNode(node) {
	var data = node.data;

	var propMap = new mithril.core.PropertyMap();

	for (var i = 0, len = data.length; i < len; i++) {
		propMap.importOne(data.property, data.type, data.value, data.language || '');
	}
...
//			addCachedNode(node);		// if performance is an issue later, use these update cache functions
			...
			async.series([
				...
				function (extraCallback) {
					exports.addInConnectors(state, ins, extraCallback);
				}
			], callback);
		});
	}, function (error) {
		...
		exports.setup(state, function (error) {
			if (error) {
				return cb(error);
			}
...
exports.editNodes = function (state, nodes, cb) {
	...
	async.series([
		function (callback) {
			state.datasources.db.exec(clearSql, delNodes, null, function (error) {
				if (error) {
					return callback(error);
				}
...
/*		// if too slow, use cache update functions
		for (var i = 0, len = nodes.length; i < len; i++) {
			delCachedNode(nodes[i]);
		}
*/
...
//			addCachedOutConnector(connector);
			callback();
		});
...
exports.editOutConnectors = function (state, connectors, cb) {

}


exports.delOutConnectors = function (state, connectors, cb) {
	// TODO -- wrote this, but 100% untested
...
exports.addInConnectors = function (state, connectors, cb) {
	// TODO -- finish function
	cb();
}


exports.editInConnectors = function (state, connectors, cb) {

}


exports.delInConnectors = function (state, connectors, cb) {
