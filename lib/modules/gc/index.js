...
exports.getManageCommands = function () {
	return ['gmsync', 'addNodes', 'editNodes', 'delNodes'];
};
...
exports.addNodes = function (state, nodes, cb) {
	...
	// TODO -- Add connectors
	// TODO -- Add to allNodesMap, allNodesArr, etc
	async.forEachSeries(nodes, function (node, callback) {
		...
		state.datasources.db.exec(sql, params, null, function (error, info) {
			...
			// TODO -- extract this connector flatten out
			var outs = [];
				...
				for (var type in node.cout) {
					...
					for (var onState in types) {
						...
						for (var i = 0, len = onStates.length; i < len; i++) {
							outs.push({ node: nodeId, type: type, onState: onState, target: onStates[i] });
						}
					}
				}
			}
			...
			// async call to add node data to the node, then add connectors
			async.series([
...
exports.delNodes = function (state, nodes, cb) {
	...
	// TODO -- Remove from allNodesArr, allNodesMap, etc

	state.datasources.db.exec(sql, nodes, null, function (error) {
		if (error) {
			return cb(error);
		}
