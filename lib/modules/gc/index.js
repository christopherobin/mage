...
exports.setNodeProgress = function (state, actorId, nodeId, newState, save, cb) {
	...
	exports.emit('progressChanged', [state, exports.getNode(nodeId), newState], function (error) {
		...
		if (save) { // notify the client
			state.emit(actorId, 'gc.node.progress.edit', { nodeId: nodeId, state: newState });

			// we do the actual write last
			exports.getNodeActorProperties(state, nodeId, actorId, { load: ['progress']}, function (error, data) {
				if (error) {
					return cb(error);
				}
...
exports.loadNodeActorData = function (state, nodeIds, actorId, cb) {
	...
	exports.getNodesActorProperties(state, nodeIds, actorId, { loadAll: true }, function (error, pms) {
		if (error) {
			return cb(error);
		}
	...
	state.datasources.db.getMany(query, params, null, function (err, results) {
		...
		for (var i = 0; i < len; i++) {
			...
			if (row.node in nodesMap) {
				...
				if (!targetNode) {
					// node connected to non-existing node
				} else {
					if (!(row.type in targetNode.rcout)) {
						targetNode.rcout[row.type] = {};
					}

					if (!(row.onState in targetNode.rcout[row.type])) {
						targetNode.rcout[row.type][row.onState] = [];
					}

					targetNode.rcout[row.type][row.onState].push(node.id);
				}
			}
		}
...
// adds nodes to the cache and returns the added nodes in a format that they can be emitted to the client
...
// edits cached nodes and returns an array of nodes edited in a format that can be emitted to the client
			...
			async.forEachSeries(nodes, function (node, nodeCallback) {
				...
				async.series([
					function (procCallback) {
						...
						addOutConnectors(state, outs, function (error) {
							allNodesMap[nodeId].cout = node.cout;
							procCallback();
						});
					},
					function (procCallback) {
						...
						addInConnectors(state, ins, function (error) {
							allNodesMap[nodeId].cin = node.cin;
							procCallback();
						});
					},
					function (procCallback) {
						...
						async.forEachSeries(data, function (property, dataCallback) {
							...
							allNodesMap[nodeId].data = propMap;
