...
exports.getNodesProgress = function (state, nodeIds, actorId, cb) {
	...
	exports.getNodesActorProperties(state, nodeIds, actorId, { optional: ['progress'], mapsAreOptional: true }, function (error, data) {
		if (error) {
			return cb(error);
		}
...
exports.loadNodeData = function (state, nodesMap, cb) {
	...
	state.datasources.db.getMany(query, params, null, function (err, results) {
		...
		for (var id in nodesMap) {
			nodesMap[id].data = new mage.core.PropertyMap();
		}
			...
			async.forEachSeries(nodes, function (node, nodeCallback) {
				...
				async.series([
					...
					function (procCallback) {
						...
						if (len === 0) {
							cachedNode.data = new mage.core.PropertyMap();
							return procCallback();
						}
