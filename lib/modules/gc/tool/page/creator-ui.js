...
*/
    ...
    // Keyboard Handlers
    $('.mage-page[data-page="gc"]').keydown(function(event) {
...
*/
    ...
    $('._jsPlumb_endpoint').live('mouseenter', function(event, ui) {
		...
		function menuItemClick(aEle, mEle) {
			var type = mEle.attr('data-type');
			loadDialog('add', type, 'Add ' + type, id, connect);
		}

		function addItemToMenu(type) {
			function maskFn(mask) {
				return mask === type;
			}

			if(parentMask.some(maskFn)) {
				var obj = { klass: 'addMenu greenGradient', data: { 'data-type': sType }, click: menuItemClick };
				menus['Add ' + sType] = obj;
			}
		}
...
function setCrumb(type, id) {
    ...
    if(id) {
        ...
        for(var i = 0; i < ancestors.length; i++) {
            newCrumb = '<div class="crumb" data-id="' + ancestors[i].id + '" data-type="' + ancestors[i].type + '"><div class="cName">' + ancestors[i].type + '</div><div class="crumbDetail"></div></div>';
            crumb = $(newCrumb).appendTo($('#breadcrumb'));
