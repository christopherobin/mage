...
Nodes.prototype.getPropertyData = function (objType, field) {
	...
	var optional     = field.attr('data-optional');

	var value        = field.find('.dataValue');
	var data         = { property: propertyName };

	var defaultSerializer = creator.defaultSerializer;
	...
	switch (type) {
		...
		case 'defaultSelect':
			value = field.find('.dataValue option:selected').val();
			if (value !== '') {
				data.type  = 'string';
				data.value = value;
			}
...
Nodes.prototype.setPropertyData = function (objType, data, field, form) {
	...
	var defaultUnserializer = creator.defaultUnserializer;
...
Nodes.prototype.getNodeRepresentation = function (node, params) {
	...
	if (window.tool.gc.language) {
		if (!params) {
			params = {};
		}

		params.language = window.tool.gc.language;
	}

	if (this.types[type].getNodeRepresentation) {
		desc = this.types[type].getNodeRepresentation(node, params);
	} else if (creator.getNodeRepresentation) {
		desc = creator.getNodeRepresentation(node, params);		// if one is defined for the game
	}
