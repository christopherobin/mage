(function () {
	...
	var mod = mage.registerModule($html5client('module.gc.construct'));
	...
	function GcNodeActor(nodeId, data) {
		this.initPropertyMap('gc/node/' + nodeId + '/actor/' + mage.actor.me.id, data);
	}

	GcNodeActor.prototype = new mage.data.PropertyMap();
	...
	function GcNode(node, actorData) {
		...
		for (var connectorType in this.cout) {
			var connectors = this.cout[connectorType];

			for (var onState in connectors) {
				var nodeIds = connectors[onState];

				for (var i = 0, len = nodeIds.length; i < len; i++) {
					var nodeId = nodeIds[i];

					var rev = coutReverseMap[nodeId];
					if (!rev) {
						rev = coutReverseMap[nodeId] = {};
					}

					if (!rev[connectorType]) {
						rev[connectorType] = [this.id];
					} else {
						if (rev[connectorType].indexOf(this.id) === -1) {
							rev[connectorType].push(this.id);
						}
					}
				}
			}
	...
	GcNode.prototype = new mage.data.PropertyMap();


	GcNode.prototype.getOutNodes = function (connectorType, onState) {
		var result = [];
		var conn = this.cout ? this.cout[connectorType] : null;

		if (conn) {
			var nodeIds = conn[onState] || conn.any;

			if (nodeIds) {
				for (var i = 0, len = nodeIds.length; i < len; i++) {
					var node = mod.getNode(nodeIds[i]);
	...
	GcNode.prototype.getOutNodesToSelf = function (connectorType) {
		var result = [];
		var rev = coutReverseMap[this.id];
		if (rev) {
			var nodeIds = rev[connectorType];
			if (nodeIds) {
				for (var i = 0, len = nodeIds.length; i < len; i++) {
					var node = mod.getNode(nodeIds[i]);
	...
	mod.setup = function (cb) {
		mage.io.on('gc.node.progress.edit', function (path, params) {
		...
		mage.io.on('gc.node.progress.del', function (path, params) {
			var node = cacheMap[params.nodeId];
			if (node) {
				node.progress = null;
				params.node = node;
