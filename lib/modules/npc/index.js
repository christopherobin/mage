...
exports.getManageCommands = function () {
	return ['sync', 'editNpc', 'addNpc', 'delNpc'];
};
...
exports.addNpc = function (state, identifier, data, cb) {
	...
	async.waterfall([
		...
		function (callback) {
			var dataSql     = 'INSERT INTO npc_data (npc, property, language, type, value) VALUES ';
			var frag        = [];
			var dataParams  = [];
			var len         = data.length;
			...
			for (var i = 0; i < len; i++) {
				var property = data[i];
				frag.push('(?, ?, ?, ?, ?)');
				dataParams.push(id, property.property, property.language || '', property.type, property.value);
			}
...
exports.editNpc = function (state, id, identifier, data, cb) {
	async.waterfall([
		function (callback) {
			var sql = 'UPDATE npc SET identifier = ? WHERE actor = ?';
