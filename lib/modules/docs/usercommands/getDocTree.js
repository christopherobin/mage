...
function find(result, matcher, absPath, relPath, cb) {
	function analyzeFile(file, callback) {
		...
		fs.stat(fileAbsPath, function (error, stats) {
			...
			if (stats.isDirectory()) {
				return find(result, matcher, fileAbsPath, fileRelPath, callback);
			}
	...
	fs.readdir(absPath, function (error, files) {
		...
		async.forEachSeries(files, analyzeFile, cb);
	});
}
...
exports.execute = function (state, cb) {
	...
	find(result, /\.md$/, mage.rootPackage.path, '', function (error) {
		if (error) {
