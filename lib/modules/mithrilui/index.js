...
function prepareFileOutput(cacheKey, output, headers, createHash, cb) {
...
function getManifestOutput(pckg, language, cb) {
	...
	prepareFileOutput(cacheKey, output, headers, false, cb);
}
...
function getPageOutput(pckg, pageName, language, options, hash, cb) {
	...
	page.render(language, function (error, renderedPage) {
		...
		prepareFileOutput(cacheKey, output, headers, true, cb);
	});
}
...
function getLoaderPageOutput(pckg, language, cb) {
	...
	var pageName = mithril.core.config.get('module.mithrilui.delivery.loader.pageName');
...
function setupRoutes(cb) {
	...
	if (!httpServer) {
		mithril.core.logger.error(null, 'Could not add MithrilUI routes, because there is no HTTP server.');
		return cb();
	}

	httpServer.addRoute(/^\/mui\//, function (request, path, params, cb) {
...
exports.setup = function (state, cb) {
	usePageCache  = !!mithril.core.config.get('module.mithrilui.delivery.page.serverCache');
	compressPages = !!mithril.core.config.get('module.mithrilui.delivery.page.compress');
	useManifest   = !!mithril.core.config.get('module.mithrilui.delivery.package.useManifest');
