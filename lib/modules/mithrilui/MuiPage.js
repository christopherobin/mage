...
function getReplacementData(package, type, label, cb) {
	...
	if (label in buildPaths) {
		...
		path = mithril.getConfig('module.mithrilui.' + package + '.paths.' + label);

		// last last chance to find path
		if (!path) {
			path = mithril.getConfig(label);
		}
	}

	if (path) {
		getFilesRecursive(path, fileMatcher, fileSplitter, cb);
	} else {
...
function embedReplacements(package, data, cb) {
	...
	async.forEachSeries(
		...
		function (match, callback) {
			...
			getReplacementData(package, type, label, function (error, newdata) {
				if (error) {
					callback(error);
				} else {
					if (newdata !== false) {
...
function loadViewsFiles(page, filetype, cb) {
	var result = [];

	var viewBasePath = mithril.getConfig('module.mithrilui.' + page.package + '.paths.views');
	var views = page.views;

	if (!viewBasePath) {
		mithril.fatalError('Embedding views, but no views path has been set in config: module.mithrilui.' + page.package + '.paths.views.');
	}
...
function embedViews(data, page, cb) {
	var viewBasePath = mithril.getConfig('module.mithrilui.' + page.package + '.paths.views');
	var views = page.views;
	...
	async.waterfall([
		...
		function (data, callback) {
			...
			loadViewsFiles(page, 'html', function (error, viewdata) {
				if (error) {
					callback(error);
				} else {
		...
		function (data, callback) {
			...
			loadViewsFiles(page, 'css', function (error, viewdata) {
				if (error) {
					callback(error);
				} else {
		...
		function (data, callback) {
			...
			loadViewsFiles(page, 'js', function (error, viewdata) {
				if (error) {
					callback(error);
				} else {
...
MuiPage.prototype.registerPackage = function (packageName) {
	var cfg = mithril.getConfig('module.mithrilui.' + packageName + '.delivery.page');
...
function loadAndHandleFile(page, language, path, cb) {
	...
	async.waterfall([
		...
		function (data, callback) {
			embedViews(data, page, callback);
		},
		function (data, callback) {
			embedReplacements(page.package, data, callback);
		},
/*
		function (data, callback) {
...
MuiPage.prototype.render = function (language, cb) {
	...
	var pagesPath = mithril.getConfig('module.mithrilui.' + this.package + '.paths.pages');
	if (!pagesPath) {
