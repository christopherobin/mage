...
function embedViews(file, views) {
	var viewBasePath = mithril.getConfig('module.mithrilui.paths.views');

	if (!viewBasePath) {
		mithril.core.logger.error('Embedding views, but no views path set.');
	}
...
MuiPage.prototype.render = function (language) {
	// returns { html: '', css: '', js: '' }
	...
	output.html = embedViews(output.html, this.views);
	output.js   = embedViews(output.js,   this.views);
	output.css  = embedViews(output.css,  this.views);
