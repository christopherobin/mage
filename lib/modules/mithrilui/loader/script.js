(function () {
	...
	function MithrilAssets() {
		...
		Asset.prototype.getUrl = function () {
			...
			return this.baseUrl + this.relPath + '?v' + this.version;
		};
		...
		this.init = function (assetMap, cb) {
			console.log('initializing asset map', assetMap);
			...
			xhr.onreadystatechange = function () {
				...
				for (var i = 0, len = parts.length; i < len; i++) {
					...
					case 'text/javascript':
						cnt = document.createElement('script');
				...
				if (assetMap) {
					window.mithril.assets.init(assetMap, nextStep);
				} else {
					nextStep();
				}
			};
	...
	window.mithril = {};
	window.mithril.assets = new MithrilAssets();

	if (window.WizAssetsHandler) {
		window.mithril.assets.registerAssetsHandler(new window.WizAssetsHandler());
	}

	window.mithril.mui = new MithrilUi();
