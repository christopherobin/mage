...
(function (window) {
	...
	mod.create = function (topic, index, data, mediaType, encoding, expirationTime, cb) {
		...
		if (encoding === 'live') {
			...
			if (!result) {
				return cb && cb(new Error('Cannot encode ' + mediaType));
			}
	...
	mod.read = function (topic, index, options, cb) {
		...
		if (value) {
			if (!value.data) {
				if (!options.optional) {
					return cb && cb(new Error('Required value does not exist'));
				}
			}

			return cb && cb(null, value.data);
		}
		...
		ucRead(topic, index, options, callback);
	};
