...
var pkg = require('mage-loader.js').getPackage('archivist');
...
function renderIndex(cfg) {
	...
	for (var i = 0; i < cfg.index.length; i++) {
		...
		inps.push(
			'<div><label for="' + id + '">' + field + '</label>: ' +
			'<input type="text" id="' + id + '" name="' + name + '"></input></div>'
		);
...
function openDocument(topic, index) {
	...
	var options = {
		maxAge: 0,
		mediaTypes: ['application/x-tome', 'application/json', 'text/plain']
	};

	mage.archivist.getValue(topic, index, options, function (error, value) {
		if (error) {
			return notifications.send('Error retrieving document');
		}
		...
		btnSave.form.onsubmit = function () {
			var doc = docEditor.getDocument();

			mage.archivist.set(value.topic, value.index, doc, value.mediaType, 'live');
...
function search(topic, partialIndex, indexFields) {
	...
	mage.archivist.rawList(topic, partialIndex, {}, function (error, results) {
		...
		if (error) {
			return notifications.send('Error while listing', 'Topic: ' + topic);
		}
...
function setupDocView() {
	...
	// install the copy/paste component on the docview container
	mage.dashboard.ui.clipboard.install(docEditor.cnt);
}
