...
(function () {
	...
	mage.loader.once('archivist.loaded', function () {

		mage.loader.renderPage('archivist');
		...
		var docEditor = new mage.dashboard.ui.DocEditor(docview);
		...
		function openDocument(topic, index) {
			...
			mage.archivist.getValue(topic, index, { mediaTypes: ['application/x-tome', 'application/json', 'text/plain'] }, function (error, value) {
				...
				if (error) {
					docview.textContent = 'Error retrieving document: ' + error;
		...
		function search(topic, partialIndex, indexFields) {
			...
			mage.archivist.rawList(topic, partialIndex, {}, function (error, results) {
				...
				if (error) {
					return writeToFooter('Error: ' + error);
				}
		...
		mage.loader.once('archivist.display', function () {
