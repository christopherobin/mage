...
function executeOperation(archivist, change) {
	...
	case 'add':
		archivist.add(change.topic, change.index, change.data, change.mediaType, change.encoding, change.expirationTime);
		break;
	case 'set':
		archivist.set(change.topic, change.index, change.data, change.mediaType, change.encoding, change.expirationTime);
		break;
	case 'touch':
	...
	for (i = 0; i < changes.length; i++) {
		...
		if (change.diff) {
			...
			try {
				...
				issues.push(
					'Could not execute "' + change.operation + '" on ' +
					JSON.stringify({ topic: change.topic, index: change.index })
				);
			}
		}
	}
	...
	state.archivist.mgetValues(toLoad, { optional: true }, function (error, values) {
		...
		for (var i = 0; i < values.length; i++) {
			...
			if (!value) {
				...
				issues.push('Could not load ' + JSON.stringify(toLoad[i]));
				continue;
			}
			...
			try {
				...
				issues.push(
					...
					JSON.stringify({ topic: value.topic, index: value.index })
				);
			}
		}
