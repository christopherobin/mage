...
function getFromCache(trueName, maxAge) {
	...
	return value && (typeof maxAge !== 'number' || value._writtenAt > Date.now() - maxAge * 1000) ? value : undefined;
}
...
VaultValue.prototype.applyDiff = function (diff) {
	...
	if (api && api.diff && api.diff.set) {
		...
		console.warn('Received a diff for topic "' + this.topic + '" but value does not support diffs (media type "' + this.mediaType + '")');
	}
...
operations.exec.applyDiff = function (topic, index, diff, expirationTime) {
	...
	if (!value) {
		return console.warn('Got a diff for a non-existent value:', topic, index, diff);
	}
...
mage.msgServer.on('archivist:set', function (path, info) {
	if (!distributing) {
		...
		operations.exec.set(key.topic, key.index, rawValue.data, rawValue.mediaType, rawValue.encoding, info.expirationTime);
	}
});
