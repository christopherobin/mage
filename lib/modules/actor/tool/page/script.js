	...
	*/
	...
	function setupHandlers() {
		...
		$('.playAs').live('click', function (e) {
			...
			mithril.gm.play(id, function (error, response) {
				...
				if (response.url) {
					...
					window.open(response.url, 'Zombie Jombie');
				} else {
		...
		$('#getAllPlayers').click(function (e) {
			...
			mithril.player.getPlayers(null, { sort: 'desc' }, limit, null, function (error, players) {
				if (error) {
	...
	mithril.loader.renderPage('actor');
	...
	mithril.setupModules(['actor', 'player'], function (error) {
		if (error) {
			return console.error(error);
		}
