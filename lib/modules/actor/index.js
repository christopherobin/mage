...
exports.getActor = function (state, id, cb) {
	...
	state.datasources.db.getOne(query, params, true, null, function (error, actor) {
		...
		//exports.getActorProperties(state, id, { loadAll: true }, function (error, props) {
		exports.getProperties(state, id, null, function (error, props) {
			if (error) {
				return cb(error);
			}
...
/* using the live propertymap:
exports.getActorProperties = function (state, actorId, options, cb) {
	var config = {
		tableName: 'actor_data',
		columns: ['actor', 'property', 'language', 'type', 'value'],
		fixedValues: { actor: actorId }
	};
...
exports.delProperties = function (state, actorId, properties, cb) {
	...
	var sql = 'DELETE FROM actor_data WHERE actor = ? AND property IN (' + db.getPlaceHolders(properties.length) + ')';
