...
exports.setup = function (state, cb) {
	...
	if (!clientAppId) {
		return state.error(null, 'Please configure: module.cronClient.clientAppId');
	}

	if (!serverAppId) {
		return state.error(null, 'Please configure: module.cronClient.serverAppId');
	}

	if (!serverEndpoint) {
		return state.error(null, 'Please configure: module.cronClient.serverBaseURL');
	}
 ...
 * @param {string|number} schedule A timestamp or crontab-formatted schedule with seconds.
