...
var clientEndpoint = cfg.clientBaseUrl || httpServer.getBaseUrl();
...
exports.runJob = function (jobId, message) {
	...
	function callback(error) {
		if (error) {
			logger.error.data(error).log('Error during cron job:', jobId);
	...
	state.archivist.createVault('client', 'client', { state: state }, function (error) {
		if (error) {
