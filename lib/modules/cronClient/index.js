...
function setup() {
	...
	clientEndpoint = cfg.clientBaseUrl || msgServer.getClientHost().getClientHostBaseUrl();
	...
	if (!clientEndpoint) {
		throw new Error(
			'Please configure: module.cronClient.clientBaseUrl or server.clientHost.expose'
		);
	}
	...
	logger.debug(
		'Setting up cronClient to connect to', serverAppId, 'at', serverEndpoint,
		'on behalf of app:', clientAppId
	);
