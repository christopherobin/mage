...
// force expire a session
exports.expire = function (state, actorId, cb) {
	/*
	mithril.actor.getActorProperty(state, actorId, { load: ['language'] }, function (error, props) {
		if (error) {
			return cb(error);
		}
		var session = new Session(actorId, props.get('language') || defaultLanguage);
	...
	*/
	state.session.expire(state, function (error) {
		if (error) {
			return cb(error);
		}
		state.actorId = null;
