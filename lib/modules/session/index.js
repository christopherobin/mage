...
function registerSession(state, session) {
	...
	state.archivist.set('session', { actorId: actorId }, session, null, null, getNewExpirationTime());
...
mage.core.msgServer.msgStream.addAddressType('session', {
	invalidate: function (sessionKey, host, reason) {
		var actorId, state;

		try {
			actorId = parseKey(sessionKey).actorId;
		} catch (error) {
