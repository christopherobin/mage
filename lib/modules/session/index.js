...
/**
 * This function turns '0.1.2' into '>=0.1.0 <0.2.0'
 *
...
function createCompatibilityRange(version) {
	var next = semver.inc(version, 'minor');

	var chunks = version.split('.', 3);

	// replace the patch and trailing build information with '0'
	chunks[2] = '0';

	var prev = chunks.join('.');
...
if (!appVersion) {
	logger.alert('Invalid or missing version information in package.json, you should really look into this. Your package.json version:', mage.rootPackage.version);
}

var appVersionRange = createCompatibilityRange(appVersion);
...
function sessionHasSupportedVersion(session) {
	...
	logger.verbose('Tested session version', sessionVersion, 'against range', appVersionRange, '(valid: ' + (isSupported ? 'yes' : 'no') + ')');
...
function registerSession(state, session, emitToActorId) {
	var expirationTime = parseInt(Date.now() / 1000, 10) + exports.sessionTTL;
	...
	if (emitToActorId) {
		state.emit(emitToActorId, 'session:set', {
...
exports.register = function (state, actorId, language, meta, cb) {
	...
	try {
		...
		registerSession(state, session, actorId);
	} catch (error) {
...
exports.reassignSession = function (state, fromActorId, toActorId, cb) {
	getActorSession(state, fromActorId, function (error, session) {
		...
		registerSession(newState, session, fromActorId);
