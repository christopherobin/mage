...
var cache = require('../../datasources/membase').getClient(mage.core.config.get('data.kv'));
...
exports.setup = function (state, cb) {
	var ttlCode   = mage.core.config.get('module.session.ttl', '10m');
	var keyLength = mage.core.config.get('module.session.keyLength', 16);

	var sessionTTL = mage.core.helpers.timeCodeToSec(ttlCode);
...
function Session(actorId, language) {
	...
	this.key = null;
	this.host = null;
	this.creationTime = mage.core.time;
}
...
exports.getActorAddresses = function (state, actorIds, cb) {
	...
	state.datasources.kv.getMany(keys, false, null, function (error, data) {
		if (error) {
			return cb(error);
		}
...
function resolve(key, cb) {
	...
	getAndTouch(state, kvkey, function (error, result) {
		...
		if (versionConfig && result.version !== versionConfig.version) {
			...
			mage.core.logger.debug('Auth message:', message);
...
exports.register = function (state, actorId, cb) {
	...
	mage.actor.getActorProperties(state, actorId, { optional: ['language'] }, function (error, props) {
		if (error) {
			return cb(error);
		}
