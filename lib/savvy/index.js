...
if (cluster.isMaster) {
	...
	// workers proxy requests to the master process

	clientHost.server.on('upgrade', function (req) {
	...
	clientHost.addRoute(ROUTE, function (req) {
		proxy(req);
	}, true);
}
...
function upgradeHandler(request, socket, head) {
	...
	console.log('wsRoutes:', Object.keys(wsRoutes));
...
function requestHandler(request, response) {
	...
	if (!route) {
		logger.error('Could not extract route from HTTP request:', request.url);
	...
	if (!handler) {
		logger.error('Route', route, 'is not registered on savvy');
		response.statusCode = 404;
		return response.end();
	}
