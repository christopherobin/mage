/* Savvy can receive requests in 2 transport types:
 * - http
 * - websocket
 *
 * And it can receive these in 2 ways:
 * - As worker, on client host, then proxied into savvy.sock for master to pick up
 * - As master, directly on savvy.sock (as master)
 */
...
exports.addRoute = function (routePath, routeFunction) {
	...
	if (httpRoutes.hasOwnProperty(route)) {
		logger.debug('Overwriting Savvy HTTP route:', route);
	} else {
		logger.debug('Adding Savvy HTTP route:', route);
	}
 ...
 * websocket connection object is returned so that event listeners can be appended. See also:
 * https://github.com/einaros/ws#server-example
 *
 * @param {string}   routePath     The path on the HTTP server on which to respond.
...
exports.addWebSocketRoute = function (routePath, routeFunction) {
	...
	if (wsRoutes.hasOwnProperty(route)) {
		logger.debug('Overwriting Savvy WebSocket route:', route);
	} else {
