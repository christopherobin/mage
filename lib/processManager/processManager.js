...
processManager.startTime = process.env.CLUSTER_START_TIME ? parseInt(process.env.CLUSTER_START_TIME, 10) : Date.now();
...
processManager.getConfiguredNumWorkers = function () {
	var numberOfWorkers = mage.core.config.get(['server', 'cluster'], false);
	numberOfWorkers = (numberOfWorkers === true) ? require('os').cpus().length : numberOfWorkers;
...
WorkerManager.prototype.killWorker = function (worker, graceful, cb) {
	...
	var shutdownGracePeriod = graceful ? mage.core.config.get(['server', 'shutdownGracePeriod']) : 2;
...
processManager.initialize = function (mageInstance, mageLogger) {
	...
	if (cluster.isMaster) {
		process.once('SIGINT', function () {
			logger.notice('Caught SIGINT, shutting down gracefully.');
			mage.quit(true);
		});

		process.once('SIGTERM', function () {
			logger.notice('Caught SIGTERM, shutting down gracefully.');
			mage.quit(true);
		});
...
processManager.quit = function (graceful, exitCode) {
	...
	if (workerManager) {
		workerManager.shutdown(graceful, function () {
