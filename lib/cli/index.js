...
program
	.command('completion')
	.option('--bash-completion <filename>', 'enables tab completion', function (filename) {
		tabComplete.addTabCompletionToProject(mage.rootPackage.path, filename);
	})
	.action(function () {
		tabComplete = require('./tabcompletion');
	});
