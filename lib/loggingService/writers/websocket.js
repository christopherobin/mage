...
WebsocketWriter.prototype.setupWebsocketServer = function () {
	...
	// when destroyed, kill the websocket server
...
WebsocketWriter.prototype.setupChannelSyncServer = function () {
	...
	this.channelSync.bind(this.uriAnnouncer, function (error) {
		if (error) {
			throw new Error('Could not set log channel announcer for websocket logger');
		}

		mage.once('shutdown', function () {
			if (that.channelSync.unbind) {
				that.channelSync.unbind();
			} else {
				fs.unlink(that.pathAnnouncer, function (error) {
					if (error) {
...
WebsocketWriter.prototype.setupLogCollector = function () {
	...
	this.collector.bind(this.uriCollector, function (error) {
		...
		mage.once('shutdown', function () {
			if (that.collector.unbind) {
				that.collector.unbind();
			} else {
				fs.unlink(that.pathCollector, function (error) {
					if (error) {
						logger.warning(error);
					}
				});
		...
		that.collector.on('message', function (msg) {
			msg = msg.toString();
