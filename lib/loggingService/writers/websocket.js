...
WebsocketWriter.prototype.setupWebsocketServer = function () {
	...
	server.on('connection', function (conn) {
		var connId = id++;
		var logger = mage.core.logger.context('logger');
		...
		conn.on('message', function (message) {
			...
			logger.verbose.details(channels.join(', ')).log('Websocket client changed channel list.');
		});

		// On the first message, log the fact that a connection has been made.
		conn.once('message', function () {
			logger.info('Client connected to logger websocket.');
		});
	});
