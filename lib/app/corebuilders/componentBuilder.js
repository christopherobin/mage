...
function buildMageConfig(app, baseUrl) {
	...
	// The app's URLs needs to be hard coded into the build so the game knows
	// which server to connect to.

	var config = {
		appName: app.name,
		appConfig: {},
		appVersion: mage.rootPackage.version,
		appVariants: {
		...
		server: {
			baseUrl: baseUrl,
			app: mage.core.app.getPublicConfig(baseUrl, app),
			msgStream: mage.core.msgServer.getPublicConfig(baseUrl),
			commandCenter: app.commandCenter.getPublicConfig(baseUrl),
			savvy: mage.core.savvy.getPublicConfig(baseUrl)
		},
		developmentMode: mage.isDevelopmentMode()
	};

	app.emit('mage-config', config);
...
exports.build = function (buildTarget, clientConfig, req, contextName, data, cb) {
	...
	if (!parentIndexPage) {
		...
		if (parentIndexPage.ignoreCache) {
			...
			buildList.push({ path: dirname(parentIndexPage.key), buildTarget: parentIndexPage });
	...
	function buildScripts(builder, isTarget, buildTarget, callback) {
		...
		builder.buildScripts(function (error, script) {
			...
			builder.buildAliases(function (error, aliases) {
				...
				result.push('window.mageConfig = ' + JSON.stringify(buildMageConfig(app, baseUrl)) + ';');
