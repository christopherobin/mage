...
*/
		...
		if (contextName === 'js') {
			...
			buildFunction = function (cb) {
				...
				builder.buildScripts(function (error, str) {
					if (error) {
						return cb(error);
					}
					...
					builder.buildAliases(function (error, str) {
						if (error) {
							return cb(error);
						}
			...
			buildFunction = builder.buildStyles.bind(builder);
		}
		...
		buildFunction(function (err, str) {
			if (err) {
				return callback(err);
			}
