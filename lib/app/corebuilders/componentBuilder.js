...
exports.build = function (buildTarget, clientConfig, contextName, data, cb) {
	...
	if (indexPage) {
		...
		if (indexPage.ignoreCache) {
			// make sure we ignore the components that were embedded in the index page
			ignoreList = ignoreList.concat(indexPage.ignoreCache);

			logger.debug.data('files', indexPage.ignoreCache).log('Ignoring prerequisite', indexPage.key);
		} else {
			// we don't know the components embedded in indexPage, so we need to build
			buildList.push({ path: indexPage.key, buildTarget: indexPage });

			logger.debug('Building prerequisite:', indexPage.key);
		}
		...
		for (var i = 0; i < indexPage.components.length; i += 1) {
			...
			if (componentName === data) {
				...
				logger.debug('Building target component:', componentPath);
			} else if (component.ignoreCache) {
				// if we already evaluated the target page, destroy any caches coming after it,
				// since this build may invalidate them

				if (currentComponentPath) {
					component.ignoreCache = null;

					logger.debug('Destroying ignore-cache of dependent:', componentPath);
				} else {
					ignoreList = ignoreList.concat(component.ignoreCache);

					logger.debug.data('files', component.ignoreCache).log('Ignoring prerequisite', componentPath);
				}
				...
				logger.debug('Building prerequisite:', componentPath);
			}
		}
	} else {
	...
	function buildComponent(entry, callback) {
		...
		builder.build(function (err, obj) {
			...
			if (requireNeeded) {
				// Only inject require if the component we are building is
