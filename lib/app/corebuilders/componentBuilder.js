...
function buildMageConfig(app, headers) {
	...
	var config = {
		...
		server: {
			baseUrl: mage.core.httpServer.getBaseUrl(headers),
			app: mage.core.app.getPublicConfig(headers, app),
			msgStream: mage.core.msgServer.getPublicConfig(headers),
			commandCenter: app.commandCenter.getPublicConfig(headers),
			savvy: mage.core.savvy.getPublicConfig(headers)
		},
