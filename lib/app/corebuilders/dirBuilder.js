...
exports.build = function (page, language, contextName, dirPath, cb) {
	...
	helpers.getFileContents(dirPath + '/' + contextName + '.buildlist', { optional: true }, function (error, buildList) {
		...
		if (buildList) {
			...
			return fileOrDirBuilder.buildList(page, language, contextName, buildList, cb);
		}
		...
		helpers.readDirectory(dirPath, matcher, function (error, contents) {
			...
			fileOrDirBuilder.buildList(page, language, contextName, buildList, cb);
		});
	});
};
