...
exports.build = function (buildTarget, clientConfig, req, contextName, dirPath, cb) {
	...
	helpers.readDirectory(dirPath, matcher, function (error, contents) {
		...
		if (contents.files.indexOf(contextName + '.buildlist') === -1) {
			...
			return fileOrDirBuilder.buildList.build(buildTarget, clientConfig, req, contextName, buildList, cb);
		}
