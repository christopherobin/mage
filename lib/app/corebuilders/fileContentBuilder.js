...
exports.build = function (buildTarget, clientConfig, req, contextName, data, cb) {
	...
	while (true) {
		...
		if (line) {
			...
			if (!context) {
				logger.error('No compatible context found for:', (line.contextName || contextName));
	...
	async.forEachSeries(
		...
		function (item, callback) {
			...
			exports.build(buildTarget, clientConfig, req, contextName, item.key, function (error, keydata) {
				if (error) {
				...
				var subTarget = new BuildTarget(buildTarget.app, item.builderName, item.key, item.context, null, buildTarget.options, allowPostProcessing);

				subTarget.rootPath = buildTarget.rootPath;

				subTarget.build(clientConfig, req, function (error, buildResult) {
					if (error) {
						callback(error);
					} else {
