...
exports.build = function (page, language, contextName, data, cb) {
	if (!data) {
		return cb(null, '');
	}
	...
	async.forEachSeries(
		...
		function (item, callback) {
			var builder = page.app.builders[item.builderName];

			if (!builder) {
				mithril.core.logger.error('Builder "' + item.builderName + '" does not exist on application "' + page.app.name + '".');
				return callback('noSuchBuilder');
			}

			builder(page, language, item.contextName, item.key, function (error, buildResult) {
				if (error) {
					callback(error);
				} else {
