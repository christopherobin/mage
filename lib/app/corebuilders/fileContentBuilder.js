...
exports.build = function (buildTarget, clientConfig, contextName, data, cb) {
	...
	while (true) {
		...
		if (line) {
			...
			if (!context) {
				mage.core.logger.error('No compatible context found for:', (line.contextName || contextName));
	...
	async.forEachSeries(
		...
		function (item, callback) {
			...
			exports.build(buildTarget, clientConfig, contextName, item.key, function (error, keydata) {
				if (error) {
				...
				var subTarget = new mage.core.app.BuildTarget(buildTarget.app, item.builderName, item.key, item.context, null, null, allowPostProcessing);

				subTarget.rootPath = buildTarget.rootPath;

				subTarget.build(clientConfig, function (error, buildResult) {
					if (error) {
						callback(error);
					} else {
