...
exports.build = function (buildTarget, clientConfig, req, contextName, filePath, cb) {
	fs.stat(filePath, function (error, stats) {
		...
		if (stats.isDirectory()) {
			return dirBuilder.build(buildTarget, clientConfig, req, contextName, filePath, cb);
		}

		if (stats.isFile()) {
			return fileBuilder.build(buildTarget, clientConfig, req, contextName, filePath, cb);
		}
...
exports.buildList = {
	build: function (buildTarget, clientConfig, req, contextName, filePaths, cb) {
		...
		async.mapSeries(
			...
			function (file, callback) {
				if (file && file.length) {
					return exports.build(buildTarget, clientConfig, req, contextName, file, callback);
				}
