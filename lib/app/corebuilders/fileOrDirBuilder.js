...
function build(page, language, contextName, filePath, cb) {
	fs.stat(filePath, function (error, stats) {
		...
		if (stats.isDirectory()) {
			dirBuilder.build(page, language, contextName, filePath, cb);
		} else if (stats.isFile()) {
			fileBuilder.build(page, language, contextName, filePath, cb);
		} else {
...
exports.buildList = function (page, language, contextName, filePaths, cb) {
	...
	async.mapSeries(
		...
		function (file, callback) {
			if (file && file.length > 0) {
				build(page, language, contextName, file, callback);
			} else {
				callback();
			}
		},
