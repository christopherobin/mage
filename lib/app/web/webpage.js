...
Page.prototype.getPaths = function (context) {
	// generate paths for this page, within this context

	var rootPath = '/app/' + this.app.name;
	var pagePaths = [];

	if (this.options.isAppIndex) {
		// app/game
		...
		if (ext && ext.length > 0) {
			...
			for (var i = 0; i < ext.length; i++) {
				pagePaths.push(rootPath + '/' + this.name + '.' + ext[i]);
			}
		} else {
			// app/game/landing

			pagePaths.push(rootPath + '/' + this.name);
		}
	}
...
Page.prototype.getPath = function (context) {
	var paths = this.getPaths(context);
	return paths[0] || null;
};
