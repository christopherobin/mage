...
/*
function buildPageDir(buildTarget, language, context, pageName, cb) {
	var page = buildTarget.app.getBuildTarget('page.' + pageName + '.' + context.name);

	mithril.core.app.builders.build('dir', buildTarget, language, context.name, page.path, function (error, data) {
		if (error) {
			return cb(error);
		}
/* NOT HERE
...
function buildManifest(buildTarget, language, context, cb) {
	...
	var manifest = buildTarget.app.getManifest();

	switch (context.name) {
	case 'url':
		return cb(null, manifest.routes[0]);
	case 'manifest':
		return cb(null, manifest.generate(language));
	}
...
function buildMithrilPage(buildTarget, language, context, cb) {
	mithril.core.logger.info('Building page', buildTarget.name, '(' + context.name + '), for language', language);
	...
	if (buildTarget.options.assetmap) {
		subTargets.push(new BuildTarget(buildTarget.app, buildTarget.name + '.assetmap', 'assets', contexts.get('assetmap'), [], {}, true));
	}

	subTargets.push(new BuildTarget(buildTarget.app, pagePath, 'dir', contexts.get('html'), [], null, true));
	subTargets.push(new BuildTarget(buildTarget.app, pagePath, 'dir', contexts.get('css'),  [], null, true));
	subTargets.push(new BuildTarget(buildTarget.app, pagePath, 'dir', contexts.get('js'),   [], null, true));
...
/*
	...
	case 'css':
	case 'js':
		// key is the page name in $web.html(myPageName)
		buildPageDir(buildTarget, language, context, key, cb);
		break;
*/
	case 'mithrilpage':
