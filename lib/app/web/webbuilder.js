...
function buildManifest(buildTarget, clientConfig, context, cb) {
	mithril.core.logger.verbose('Building manifest (' + context.name + '), for client config').data(clientConfig);

	var manifest = buildTarget.app.getManifestBuildTarget();

	if (!manifest) {
		// no manifest set up
		mithril.core.logger.warning('No manifest exists for application', buildTarget.app.name);
...
function buildMithrilPage(buildTarget, clientConfig, context, cb) {
	mithril.core.logger.verbose('Building page', buildTarget.describe(), '(' + context.name + '), for config', JSON.stringify(clientConfig));
	...
	if (buildTarget.options.assetMap) {
		subTargets.push(new BuildTarget(buildTarget.app, 'assets', buildTarget.app.assetMap, contexts.get('assetmap'), [], {}, true));
	}
...
exports.build = function (buildTarget, clientConfig, contextName, key, cb) {
	...
	if (contextName === 'mithrilpage') {
		...
		buildMithrilPage(buildTarget, clientConfig, context, cb);
