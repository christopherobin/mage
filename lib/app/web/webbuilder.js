...
function buildMithrilPage(buildTarget, clientConfig, context, cb) {
	mithril.core.logger.info('Building page', buildTarget.describe(), '(' + context.name + '), for config', JSON.stringify(clientConfig));
	...
	if (buildTarget.options.assetMap) {
		subTargets.push(new BuildTarget(buildTarget.app, 'assets', buildTarget.options.assetMap, contexts.get('assetmap'), [], {}, true));
	}
