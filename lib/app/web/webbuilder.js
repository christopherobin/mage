...
function buildManifest(buildTarget, clientConfig, context, cb) {
	mithril.core.logger.info('Building manifest (' + context.name + '), for client config', JSON.stringify(clientConfig));
...
function buildMithrilPage(buildTarget, clientConfig, context, cb) {
	mithril.core.logger.mithril_debug('Building page', buildTarget.describe(), '(' + context.name + '), for config', JSON.stringify(clientConfig));
