...
function makeClientConfig(language, density, screen, defaultClientConfig) {
	var obj = {
		language: language && language.toLowerCase() || defaultClientConfig.language,
		density: Number(density || defaultClientConfig.density)
	};
	...
	if (Array.isArray(screen)) {
		...
		obj.screen = defaultClientConfig.screen;
	}
...
/**
 * This function creates an HTTP response for the given buildTarget, and returns meta data
 * that comes with it. That meta data will probably include a hash and possibly a sourcemap
 * collection.
 *
...
function buildToHttpResponse(buildTarget, clientConfig, req, cb) {
	var app = buildTarget.app;

	logger.debug('Building:', buildTarget.describe(), 'for app:', app.name, 'using client config:', clientConfig);

	app.emit('build', buildTarget, clientConfig);

	buildTarget.build(clientConfig, req, function (error, data, meta) {
		if (error) {
			return cb(error);
		}

		var headers = {
			'content-type': buildTarget.context.mimetype,
			'last-modified': (new Date()).toUTCString(),   // disclaimer: some platforms really care about this
			pragma: 'no-cache'                             // no browser caching for WebApp downloads
		};
		...
		function respond() {
			...
			cb(null, headers, data, meta);
		}


		// gzip compress contents

		if (app.delivery.hasOwnProperty('compress')) {
			// deprecation warning

			logger.warning(
				'The configuration entry "apps.' + app.name + '.delivery.compress"',
				'has been deprecated in favor of auto-detection.'
			);
		}
		...
		gzip.on('end', function () {
			data = Buffer.concat(buffers, nread);

			logger.verbose('Build target', buildTarget.describe(), 'size after gzip compression:', data.length, 'bytes.');
...
function createCachedRequestHandler(buildTarget, responses) {
	...
	return function (req, path, params, cb) {
		...
		var clientConfig = app.getBestConfig(params);
		...
		if (!clientConfig) {
			logger.warning.data({
				...
				request: req
			}).log('HTTP 404: No compatible client config found');
		...
		if (!response) {
			logger.warning('HTTP 404: No response found for client config', clientConfig._key);
			return cb(404);
		}

		// check hash

		if (response.meta && response.meta.hash && response.meta.hash === params.hash) {
			logger.debug('HTTP 200: Use cache');
			return cb(200, 'usecache', CONTENT_TYPE_PLAIN);
		}

		// check last-modified

		if (req.headers['if-modified-since'] && response.headers['last-modified']) {
			var testMDate = (new Date(req.headers['if-modified-since'])).getTime();
			var realMDate = (new Date(response.headers['last-modified'])).getTime();

			logger.verbose('Returning 304 if not modified since', req.headers['if-modified-since']);

			if (!isNaN(testMDate) && realMDate <= testMDate) {
		...
		logger.debug('HTTP 200: Sending response');
...
function createRealTimeBuildRequestHandler(buildTarget) {
	...
	return function realTimeBuildRequestHandler(req, path, params, cb) {
		...
		// check last-modified (a bit silly with build on-demand, but so be it)

		if (req.headers['if-modified-since']) {
			var testMDate = (new Date(req.headers['if-modified-since'])).getTime();
			var realMDate = Date.now();
		...
		return buildToHttpResponse(buildTarget, clientConfig, req, function (error, headers, data) {
			if (error) {
				logger.alert('Build error:', error);
				return cb(500);
			}
...
function getLowestDensity(densities) {
	return densities.reduce(function (prev, curr) {
		return curr < prev ? curr : prev;
	});
}


function getSmallestScreen(screens) {
	return screens.reduce(function (prev, curr) {
		return curr[0] * curr[1] < prev[0] * prev[1] ? curr : prev;
	});
}
...
function WebApp(name, options) {
	...
	// fill in defaults for the configuration if none was given

	var delivery = this.delivery = options.delivery || {};

	if (delivery.hasOwnProperty('serverCache')) {
		logger.warning('The "apps.' + name + '.delivery.serverCache" configuration is deprecated.');
	}
	...
	// store all languages, densities and screen resolutions
	...
	// Make a default client config that we can use as a fallback from now on, based on the real
	// configuration. That means: first configured language, lowest density, lowest resolution

	var lowestDensity = getLowestDensity(this.densities);
	var smallestScreen = getSmallestScreen(this.screens);

	this.defaultClientConfig = makeClientConfig(this.languages[0], lowestDensity, smallestScreen);

	// make a list of every possible client config
	...
	that.densities.forEach(function (density) {
		that.languages.forEach(function (language) {
			that.screens.forEach(function (screen) {
				var config = makeClientConfig(language, density, screen, that.defaultClientConfig);
	...
	this.prebuild = !mage.isDevelopmentMode('buildAppsOnDemand'); // only prebuild when needed (production)
...
WebApp.prototype.addIndexPage = function (name, path, options, assetOptions) {
	...
	var routes = [];
	var context = contexts.get(options.context || 'html');
	var firstRoute;
	...
	// find the first file that matches *.ext

	var pattern = '*.+(' + context.ext.join('|') + ')';
	var filePath = glob.sync(pathJoin(path, pattern))[0];
	if (!filePath) {
		throw new Error('Could not find file matching pattern ' + pattern + ' in path ' + path);
	}

	var page = new BuildTarget(this, 'file', filePath, context, routes, options, true);
...
WebApp.prototype.getComponentPaths = function () {
	// returns all paths that should contain components

	var trg = this.buildTargets;
	var result = [];

	result = result.concat(trg.indexPages.map(function (page) {
		return dirname(page.key);
	}));
	...
	// filter out index pages that are not componentized

	result = result.filter(function (folder) {
		return fs.existsSync(pathJoin(folder, 'component.json'));
	});
 ...
 *
 * @param {Object}   params
 * @param {string}   params.language For example: "ja"
 * @param {number}   params.density  For example: 1
 * @param {number[]} params.screen   For example: [320,240]
 * @returns {Object}                 A client config.
 */

WebApp.prototype.getBestConfig = function (params) {
	params = params || {};

	var clientConfig = makeClientConfig(params.language, params.density, params.screen, this.defaultClientConfig);
	if (!clientConfig) {
		return;
	}

	var configs = this.clientConfigs;
	var best;
	var bestScreen = [-1, -1];
	...
	for (var i = 0; i < configs.length; i++) {
		...
		if (candidate.screen[0] > clientConfig.screen[0] || candidate.screen[1] > clientConfig.screen[1]) {
			continue;
		}

		// if the candidate resolution is smaller than the one we found so far, ignore it

		if (candidate.screen[0] < bestScreen[0] && candidate.screen[1] < bestScreen[1]) {
			continue;
		}
...
WebApp.prototype.getIndexPageVariants = function (pageName) {
	...
	return this.clientConfigs.map(function (clientConfig) {
		...
		if (!response && that.prebuild) {
			throw new Error(
				'No HTTP response has been cached for client config "' + clientConfig._key + '" ' +
				'for index page "' + pageName + '" for app "' + that.name + '"'
			);
		}

		var digest = response ?
			response.meta.hash || generateDigest(response.data) :
...
WebApp.prototype.listBuildTargets = function () {
	...
	// we create mage pages and components last, since they can contain asset maps
	// (which may require indexpages to be digested)
...
WebApp.prototype.getResponsesForClientConfig = function (clientConfig) {
	if (!clientConfig) {
		return [];
	}

	var buildTargets = this.listBuildTargets();
	var responses = [];

	for (var i = 0; i < buildTargets.length; i++) {
		var response = buildTargets[i].responseCache && buildTargets[i].responseCache[clientConfig._key];
		...
		function (buildTarget, callback) {
			async.eachSeries(
				...
				function (clientConfig, callback) {
					// don't build if we already have a build for this client config

					if (buildTarget.responseCache && buildTarget.responseCache[clientConfig._key]) {
						logger.verbose(
							'Build for', buildTarget.describe(), 'with client config',
							clientConfig._key, 'already loaded, skipping.'
						);
						return callback();
					}

					// build, so that responseCache becomes populated

					buildToHttpResponse(buildTarget, clientConfig, null, callback);
				},
				callback
			);
		},
...
WebApp.prototype.loadBuilds = function (cb) {
	...
	function loadBuilds(buildTarget, callback) {
		...
		function loadBuildForConfig(clientConfig, callback) {
			var buildPath = createBuildPath(that.buildPath, buildTarget.routes[0], clientConfig._key);

			var headersPath = pathJoin(buildPath, 'headers.json');
			var dataPath = pathJoin(buildPath, 'data.bin');
			var metaPath = pathJoin(buildPath, 'meta.json');
				...
				async.series([
					function (callback) {
						fs.readFile(headersPath, function (error, headers) {
							...
							response.headers = JSON.parse(headers.toString());
							callback();
						});
					},
					function (callback) {
					...
					function (callback) {
						fs.readFile(metaPath, function (error, meta) {
							if (error) {
								if (error.code === 'ENOENT') {
							...
							response.meta = JSON.parse(meta.toString());
							callback();
						});
					}
				], function (error) {
...
WebApp.prototype.storeBuilds = function (cb) {
	...
	function storeBuilds(buildTarget, callback) {
		...
		function storeBuildForConfigKey(clientConfigKey, callback) {
			var response = responses[clientConfigKey];  // { headers, data, meta }
			var buildPath = createBuildPath(that.buildPath, buildTarget.routes[0], clientConfigKey);
			...
			async.series([
				...
				function (callback) {
					...
					fs.writeFile(pathJoin(buildPath, 'meta.json'), JSON.stringify(response.meta), callback);
				}
			], callback);
		}
...
WebApp.prototype.exposeOnClientHost = function () {
	...
	if (that.firewall) {
		...
		this.registerRequestHook(function (req) {
			if (!that.firewall(req.connection, req)) {
	...
	function setupHandlers(buildTarget) {
		...
		buildTarget.routes.forEach(function (route) {
			...
			httpServer.addRoute(route, handler, 'callback');
		});
	}
	...
	that.buildTargets.indexPages.forEach(function (buildTarget) {
		var details = buildTarget.routes.map(function (route) {
			return httpServer.getRouteUrl(route);
		});

		logger.notice('Exposed application "' + that.name + '" at:', details.join(', '));
	});
};
