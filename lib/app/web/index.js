...
exports.defaultBuilders = {
	page: webpage.build.bind(webpage)
};

// TODO: defaultBuilders can just be an array of strings (builder names)
// TODO: every builder should register itself! on mithril.core.app

if (mithril.assets) {
	exports.defaultBuilders.assets = mithril.assets.build.bind(mithril.assets);
}
...
function WebApp(name, options) {
	...
	// set up default builders

	for (var key in exports.defaultBuilders) {
		this.builders[key] = exports.defaultBuilders[key];
	}
}
...
WebApp.prototype.addPage = function (name, options) {
	this.pages[name] = new webpage.Page(this, name, options);
};
...
WebApp.prototype.expose = function (cb) {
	...
	async.forEachSeries(
		...
		function (page, callback) {
			...
			var rootPath = '/app/' + app.name;
			var pagePaths = [];

			if (page.options.isAppIndex) {
				// app/game

				pagePaths.push(rootPath);
			} else {
				var ext = context.ext;
				if (ext && ext.length > 0) {
					// app/game/landing.html,htm,css,js,...

					for (var i = 0; i < ext.length; i++) {
						pagePaths.push(rootPath + '/' + page.name + '.' + ext[i]);
					}
				} else {
					// app/game/landing

					pagePaths.push(rootPath + '/' + page.name);
				}
			}
