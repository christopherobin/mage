...
function buildToHttpResponse(buildTarget, clientConfig, req, cb) {
	...
	var desc = buildTarget.describe();

	logger.debug('Building:', desc, 'for app:', app.name, 'using client config:', clientConfig);
	...
	buildTarget.build(clientConfig, req, function (error, data, meta) {
		...
		var response = new Response(meta);
		response.setLastModified(new Date());   // disclaimer: some platforms really care about this
		response.setContentType(buildTarget.context.mimetype);

		response.setBaseBuild(data);

		buildTarget.responseCache = buildTarget.responseCache || {};
		...
		var buffers = [];
		var nread = 0;
		var sizeBefore = data.length;

		gzip.on('error', function (error) {
			gzip.removeAllListeners('end');
			gzip.removeAllListeners('error');

			logger.alert('Gzip compression failed (continuing without):', error);
		...
		gzip.on('end', function () {
			if (nread < sizeBefore) {
				// store the gzipped build

				response.setGzippedBuild(Buffer.concat(buffers, nread));

				logger.verbose('Build target', desc, 'compressed from', sizeBefore, 'to', nread, 'bytes.');
			} else {
				logger.verbose('Build target', desc, 'compressed from', sizeBefore, 'to', nread, 'bytes (discarded).');
			}
...
function createCachedRequestHandler(buildTarget, responses) {
	...
	return function (req, path, params, cb) {
		...
		if (!response) {
			logger.warning('No appropriate build found for client config', clientConfig._key);
			return cb(404);
		}

		// check hash

		if (params.hash && response.getHash() === params.hash) {
			logger.debug('HTTP 200: Sending "usecache"');
		...
		if (!response.isModifiedSince(req.headers['if-modified-since'])) {
			logger.debug('HTTP 304: Sending not modified, since', req.headers['if-modified-since']);

			return cb(304, null, {
				'last-modified': response.builds.base.headers['last-modified']
			});
		}

		var build = response.getBestBuild(req);

		if (req.method === 'HEAD') {
			logger.debug('HTTP 200: Sending headers to HEAD request');

			return cb(200, undefined, build.headers);
		}

		logger.debug('HTTP 200: Sending response');

		return cb(200, build.data, build.headers);
	};
}
...
function createRealTimeBuildRequestHandler(buildTarget) {
	...
	return function realTimeBuildRequestHandler(req, path, params, cb) {
		...
		return buildToHttpResponse(buildTarget, clientConfig, req, function (error, response) {
			if (error) {
			...
			// select a build for this request
			...
			cb(200, build.data, build.headers);
		});
	};
}
...
WebApp.prototype.getIndexPageVariants = function (pageName) {
	...
	return this.clientConfigs.map(function (clientConfig) {
		...
		if (!response) {
			...
			digest = parseInt(Date.now() / 1000, 10);
		} else {
			digest = response.getHash();
		}
...
WebApp.prototype.makeBuilds = function (cb) {
	...
	async.eachSeries(
		...
		function (buildTarget, callback) {
			var cache = buildTarget.responseCache || {};
...
WebApp.prototype.loadBuilds = function (cb) {
	...
	function loadBuilds(buildTarget, callback) {
		...
		function loadBuildForConfig(clientConfig, callback) {
			...
			Response.load(buildPath, function (error, response) {
				if (error) {
					if (error.code === 'ENOENT') {
						return callback();
					}
