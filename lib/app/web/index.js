...
function createCachedRequestHandler(buildTarget, responses) {
	...
	return function (req, path, params, cb) {
		...
		var hookResponse = app.checkRequestHooks(req, path, params, 'route');
		if (hookResponse) {
...
function WebApp(name, options) {
	...
	// THIS IS NOW DEPREECATED AND SHOULD NOT BE USED

	this.firewall = null;

	// Array of request hooks to be executed on each request we recieve
...
/**
 * Function whcih registers a hook function which will be executed on each WebApp request.
 * 
 ...
 * @param {string} requestType - Type of request, currently will only be either command or route.
