...
function prebuildHttpResponses(buildTarget, cb) {
	...
	var indexPages = [];
	if (buildTarget.options.requiredBy) {
		indexPages = Object.keys(buildTarget.options.requiredBy);
	}
	...
	function buildForConfig(clientConfig, callback) {
		...
		async.forEachSeries(indexPages, function (indexPage, forEachCB) {
			if (indexPage) {
				clientConfig.pageName = indexPage;
			} else {
			...
			serveBuildTarget(buildTarget, clientConfig, function (error, headers, data, hash) {
				...
				var responseKey = clientConfig._key + (indexPage ? '-' + indexPage : '');
				...
				if (buildTarget.pageName || buildTarget.componentName) {
					// store the digest of the page/clientConfig
					var digests = app.digests[buildTarget.pageName || buildTarget.componentName] || {};
...
function createCachedRequestHandler(app, buildTarget, responses) {
	return function (req, path, params, cb) {
		...
		var responseKey = clientConfig._key + (clientConfig.pageName ? '-' + clientConfig.pageName : '');
...
WebApp.prototype.registerComponent = function (name, path, requiredBy, options) {
	...
	componentOptions.requiredBy = {};
	componentOptions.requiredBy[requiredBy.pageName] = requiredBy;
	...
	async.forEachSeries(
		...
		function () {
			...
			app.buildTargets.indexPages.forEach(function (buildTarget) {
				...
				buildTarget.routes.forEach(function (route) {
					details.push(httpServer.getRouteUrl(route));
				});
