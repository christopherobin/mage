...
function WebApp(name, options) {
	...
	that.densities.forEach(function (density) {
		that.languages.forEach(function (language) {
			that.screens.forEach(function (screen) {
				...
				logger.mithril_debug('Added new supported client config for app "' + name + '":', config);
	...
	this.buildTargets = { indexPages: [], mithrilPages: [], manifest: null };
	this.pages = {};
	this.digests = {};
	this.manifest = null;
	...
	this.assetMap = new mithril.assets.AssetMap(this);
...
WebApp.prototype.addPage = function (name, path, options) {
	...
	this.pages[name] = page;
	this.buildTargets.mithrilPages.push(page);
...
WebApp.prototype.getPageVariants = function (pageName) {
	...
	for (var i = 0, len = this.clientConfigs.length; i < len; i++) {
		...
		variants.push({
			...
			digest: digest || mithril.core.time
		});
	}
...
WebApp.prototype.createManifest = function (assetMap, options) {
	...
	if (this.delivery.useManifest) {
		...
		logger.mithril_debug('HTML5 Application cache disabled, skipping build.');
	}
...
function serveBuildTarget(buildTarget, clientConfig, cb) {
	buildTarget.build(clientConfig, function (error, data, meta) {
		...
		if (!buildTarget.app.delivery.compress) {
			logger.mithril_debug('Build target', buildTarget.describe(), 'size:', data.length, 'bytes.');
		...
		logger.mithril_debug('Build target', buildTarget.describe(), 'size before gzip compression:', data.length, 'bytes.');
		...
		gzip.on('end', function () {
			...
			logger.mithril_debug('Build target', buildTarget.describe(), 'size after gzip compression:', compressed.length, 'bytes.');
...
function prebuildHttpResponses(buildTarget, cb) {
	...
	// note: the buildTarget is the html page, mithril-page or the manifest
...
WebApp.prototype.expose = function (cb) {
	...
	// we create mithril pages last, since they can contain asset maps (which may require indexpages to be digested)

	buildTargets = buildTargets.concat(this.buildTargets.mithrilPages);
	...
	async.forEachSeries(
		...
		function (buildTarget, callback) {
			...
			logger.debug('Exposing build target:', buildTarget.describe());
			...
			if (app.delivery.serverCache) {
				...
				prebuildHttpResponses(buildTarget, function (error, responses) {
					...
					buildTarget.routes.forEach(function (route) {
						logger.mithril_debug('Registering route:', route);
