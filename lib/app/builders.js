...
// CustomBuilder class, that allows developers to register any builder name, and have each key run
// a given external build function
...
CustomBuilder.prototype.build = function (page, clientConfig, req, contextName, key, cb) {
	var fn = this.keys[key];

	if (fn) {
		fn(page, clientConfig, req, contextName, key, cb);
	} else {
