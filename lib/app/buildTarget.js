...
function BuildTarget(app, name, builderName, context, routes, options, allowPostProcessors) {
	this.app = app;
	this.key = name;	// name is used as the initial key to pass into the builder
	this.name = name;
...
BuildTarget.prototype.build = function (language, cb) {
	...
	fnBuild(this, language, this.context.name, this.key, function (error, data, meta) {
		if (error) {
			return cb(error);
		}
