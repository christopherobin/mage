...
BuildTarget.prototype.registerComponent = function (name, path) {
	if (this.components.indexOf(name) !== -1) {
		throw new Error('Component ' + name + ' was already registered on indexPage ' + this.name);
	}

	this.components.push(name);

	this.app.registerComponent(name, path, this);
};
