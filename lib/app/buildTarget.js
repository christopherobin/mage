...
BuildTarget.prototype.describe = function () {
	if (!this.routes.length) {
		throw new Error('Unable to describe buildTarget: no routes.');
	}
...
BuildTarget.prototype.build = function (clientConfig, req, cb) {
	// TODO: perhaps instead of using context.name, we should build by passing around context

	var builder = this.getBuilder();
	var that = this;

	if (typeof req === 'function' && !cb) {
		cb = req;
		req = null;
	}

	builder.build(this, clientConfig, req, this.context.name, this.key, function (error, data, meta) {
		if (error) {
			return cb(error);
		}
