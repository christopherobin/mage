	...
	for (var gameModule in commandList) {
		...
		for (var i = 0, len = cmds.length; i < len; i++) {
			...
			if (cmd) {
				...
				if (requirements) {
					cmd.requirements.push(requirements); 
				}
...
function checkRequirement(state, tags, requirement, cb) {
...
function checkRequirements(state, tags, requirements, cb) {
	...
	if (len === 1) {
		...
		return checkRequirement(state, tags, requirements[0], cb);
	}
	...
	async.forEachSeries(
		...
		function (requirement, callback) {
			checkRequirement(state, tags, requirement, function (error, failure) {
				if (error) {
					return cb(error);
				}
...
exports.execute = function (state, tags, cmdName, p, cb) {
	...
	checkRequirements(state, tags, cmd.requirements, function (error) {
		if (error) {
			cb(error);
		} else {
			// execute the command
