...
var commands = {
	getFullState: require(app.paths.usercommands + '/getFullState.js')
};
...
exports.execute = function(state, playerId, msg)
{
	...
	{
		app.warn(errors.USERCOMMAND_EXPECTED, state.msgClient);
		return;
	}

	if (!(msg.command in commands))
	{
		app.warn(errors.USERCOMMAND_UNKNOWN, state.msgClient);
		return;
	}
