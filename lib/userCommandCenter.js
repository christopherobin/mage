...
var commands = {
	getFullState: require(mithril.paths.usercommands + '/getFullState.js')
};
...
exports.execute = function(state, playerId, msg)
{
	...
	{
		mithril.warn(errors.USERCOMMAND_EXPECTED, state.msgClient);
		return;
	}

	if (!(msg.command in commands))
	{
		mithril.warn(errors.USERCOMMAND_UNKNOWN, state.msgClient);
		return;
	}
