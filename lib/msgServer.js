...
exports.start = function (httpServer) {
	...
	io.sockets.on('connection', function (client) {
		...
		function decodeAndHandleMessage(msg) {
			// decode the message
		...
		client.on('message', function (msg) {
			...
			async.whilst(
				function () {
					// if there is a hook, split the hook type from the data
					// else, leave this while loop
					
					var m = msg.match(/^hook\((.+?)\)(.+)$/);
					if (!m) {
						return false;
					}
				...
				function (callback) {
					...
					if (hook) {
						hook(msg, function (error, result) {
							if (error) {
								callback(error);
							} else {
				...
				function (error) {
					if (error) {
						sessionlessError(client, msg, error);
					} else {
