...
function startServing(cb) {
	...
	mithril.core.logger.error.data('sampler.bind', init.bind).log(error);
	return cb(error);
}
	...
	for (var name in init.intervals) {
		if (init.intervals.hasOwnProperty(name)) {
			if (typeof init.intervals[name] !== 'number') {
				...
				mithril.core.logger.error.data('sampler.intervals', init.intervals).log(error);
