...
function unixSocketListen(server, file, protocol, cb) {
	server.listen(file, function (error) {
		...
		mage.core.logger.notice('Sampler running at ' + protocol + '://' + file);
		return cb();
	});
}
...
function hostPortListen(server, port, host, protocol, cb) {
	server.listen(port, host, function (error) {
		...
		mage.core.logger.notice('Sampler running at ' + protocol + '://' + address.address + ':' + address.port);
		return cb();
	});
}
...
function startServing(cb) {
	...
	if (!init.bind) {
		mage.core.logger.warning('Sampler is collecting data, but no configuration was found for serving.');
		return cb();
	}
	...
	mage.core.logger.error.data('sampler.bind', init.bind).log(error);
	return cb(error);
}
...
exports.setup = function (cb) {
	mage.core.config.setTopLevelDefault('sampler', {
	...
	for (var name in init.intervals) {
		if (init.intervals.hasOwnProperty(name)) {
			if (typeof init.intervals[name] !== 'number') {
				...
				mage.core.logger.error.data('sampler.intervals', init.intervals).log(error);
