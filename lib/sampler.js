...
exports.setup = function () {
	...
	panopticon.on('collect', function () {
		...
		sampleCache.forEach(function (entry) {
			panopticon.sampler(entry.name, entry.number);
		});
...
function mithrilSamplers () {
	...
	function getCpuUsage() { // I need a sysadmin to check this.
		var time_total = fs.readFileSync('/proc/stat').toString().split('\n');
		//console.log(time_total)
		var elems = fs.readFileSync('/proc/' + process.pid + '/stat').toString().split(' ');
		var utime = parseInt(elems[13]);
		var stime = parseInt(elems[14]);

		var jiffies = dataString.split('\n').filter(function (elem) {
 			return elem.match(/cpu /)
 		})[0].split(' ').reduce(function (sum, elem) {
			return (parseInt(elem, 10) || 0) + sum
		}, 0);
...
*
	...
	exports.sample('cpu.usage', getCpuUsage)
}

exports.sample = function (name, number) {
	setCache.push({
		name: name,
		sample: number
	});
