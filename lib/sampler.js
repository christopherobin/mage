...
function unixSocketListen(server, file, protocol, cb) {
	server.listen(file, function (error) {
		...
		mithril.core.logger.notice('Server running at ' + protocol + '://' + file);
		return cb();
	});
}
...
function hostPortListen(server, port, host, protocol, cb) {
	server.listen(port, host, function (error, address) {
		...
		mithril.core.logger.notice('Server running at ' + protocol + '://' + address.address + ':' + address.port);
		return cb();
	});
}
...
function startServing(cb) {
	...
	// It is not the job of the sampler to prepare a path. This function process a url.
	function urlToPath(url) {
		...
		path.shift(); // Remove the useless '' (zeroth) element.
	...
	mithril.once('shutdown', server.close);
	...
	if (init.bind.hasOwnProperty('file')) {
		...
		unixSocketListen(server, init.bindfile, init.protocol, cb);
	} else if (init.bind.hasOwnProperty('port')) {
		mithril.core.logger.debug('Sampler about to listen on port', init.bind.port);
		hostPortListen(server, init.bind.port, init.bind.host, init.protocol, cb);
	}
}
...
exports.setup = function (cb) {
	...
	// which panopticon is which. An emitter is provided if it is needed elsewhere.
	panoptica.forEach(function (panopticon) {
