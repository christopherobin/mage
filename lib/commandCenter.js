	...
	for (var gameModule in commandList) {
		...
		for (var i = 0, len = cmds.length; i < len; i++) {
			...
			if (cmd) {
				...
				mithril.core.logger.error('Trying to expose command "' + gameModule + splitter + cmdName + '" twice on "' + packageName + '" command center. Ignoring.');
				continue;
			}
...
CommandCenter.prototype.execute = function (state, tags, cmdName, p, cb) {
	...
	checkRequirements(state, tags, cmd.requirements, function (error, failure) {
		...
		if (failure) {
			console.log('Auth failed:', failure);

			state.userError('authFailed', cb);
		} else {
			// execute the command
