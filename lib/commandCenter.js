...
var commandResponseCacheTTL = 3 * 60;				// cache lifetime of the command response
...
function tryLoadCachedResponse(session, queryId, cb) {
	...
	session.get('cmdresponse', function (error, value) {
		...
		mithril.core.logger.debug('Loaded command response from cache.');
...
*/
	...
	session.set('cmdresponse', commandResponseCacheTTL, value, function (error) {
		// error parameter is irrelevant, cache is best effort
		if (error) {
			mithril.core.logger.debug('Caching command response failed.');
		} else {
			mithril.core.logger.debug('Caching command response successful.');
		}
