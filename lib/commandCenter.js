...
function getMemcachedClient() {
	...
	if (mcClient === false) {
		logger.mithril_debug('Response cache disabled (missing configuration)');
		return null;
	}
...
function createResponseCacheKeys(session) {
	if (!session) {
		logger.mithril_debug('No session, so could not create response cache keys');
	...
	if (!sessionKey) {
		logger.mithril_debug('No registered session key, so could not create response cache keys');
...
CommandCenter.prototype.expose = function (requirements, commandList) {
	...
	for (var gameModule in commandList) {
		...
		for (var i = 0, len = cmds.length; i < len; i++) {
			...
			logger.mithril_debug('Exposing command "' + gameModule + '.' + cmdName + '" at "' + cmdPath + '"');
	...
	logger.notice(count + ' commands exposed.')
            ...
            .data(Object.keys(commandList), 'exposedCommands');
};
...
CommandCenter.prototype._tryLoadCachedResponse = function (session, queryId, cb) {
	...
	logger.mithril_debug('Trying to load response cache');

	if (!queryId) {
		logger.mithril_debug('No response cache applied (queryId missing)');
		return cb();
	}

	if (!this.responseCacheTTL || this.responseCacheTTL < 1) {
		logger.mithril_debug('Response cache disabled (non-positive TTL set up)');
		return cb();
	}

	var client = getMemcachedClient();
	if (!client) {
		logger.mithril_debug('No memcached client available');
		return cb();
	}
	...
	client.get(keys.info, function (error, info) {
		if (error) {
			logger.mithril_debug('Error while requesting response cache meta-info:', error);
			return cb();
		}

		if (typeof info !== 'string') {
			logger.mithril_debug('No response cache meta-info found:', keys.info);
			return cb();
		}
		...
		if (cachedQueryId !== queryId) {
			...
			logger.mithril_debug('Found a response cache, but query ID did not match:', cachedQueryId, queryId);
			return cb();
		}
		...
		client.get(keys.data, function (error, response) {
			...
			if (typeof response !== 'string') {
				logger.mithril_debug('Response cache was desired, but no data found:', keys.data);
				return cb();
			}
			...
			logger.mithril_debug('Loaded command response from cache.');
...
CommandCenter.prototype._cacheResponse = function (session, queryId, options, response, cb) {
	...
	client.set(keys.data, response, ttl, function (error) {
		...
		logger.mithril_debug('Caching command response successful:', keys.data);
		...
		client.set(keys.info, info, ttl, function (error) {
			...
			logger.mithril_debug('Caching command response meta-info successful:', keys.info);
...
CommandCenter.prototype._executeCommand = function (cmd, session, cb) {
	...
	logger.mithril_debug('Executing command "' + cmdInfo.execPath + '"');
	...
	paramList.push(function () {
		...
		state.close(function (closeError, response) {
			...
			if (startTime) {
				logger.time('User command', cmd.name, 'execution took', (Date.now() - startTime), 'msec.');
			}
...
CommandCenter.prototype.executeCommands = function (commandNames, headerData, paramsData, files, queryId, transportInfo, transportCb) {
    ...
    var commandStr = commandNames.join(',')
	...
	try {
		...
		logger.error(e)
                .details('Parse error in commands details')
                .data(state, 'state');
	...
	processCommandHeader(state, headerData, paramsData, function (error, tags) {
		if (error) {
            ...
            logger.error(commandStr, 'Unauthorized access detected')
                    .data(state, 'state');
		...
		if (failure) {
			logger.error(commandStr, 'User command request requirements failed')
                    .data(state, 'state')
		...
		that._tryLoadCachedResponse(session, queryId, function (error, options, response) {
			if (!error && options && response) {
                ...
                logger.warning(commandStr, 'Re-sending command information')
                        .data(state, 'state');
			...
			async.mapSeries(
				...
				function (error, results) {
					if (error) {
						...
						logger.error(commandStr, 'Unreachable error reached, in commandCenter.executeCommand callback')
                                .data(state, 'state');
