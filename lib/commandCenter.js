	...
	for (var gameModule in commandList) {
		...
		for (var i = 0, len = cmds.length; i < len; i++) {
			...
			try {
				...
				return mithril.fatalError('Loading user command', cmdPath, 'failed:', err, err.stack);
			}
...
CommandCenter.prototype.executeCommands = function (commandNames, headerData, paramsData, files, queryId, transportInfo, transportCb) {
	...
	try {
		...
		logger.error('Parse error in command details').data('postdata', paramsData);
	...
	processCommandHeader(state, headerData, paramsData, function (error, tags) {
		...
		if (failure) {
			logger.error('User command request requirements failed').data('failure', failure);
		...
		that._tryLoadCachedResponse(session, queryId, function (error, options, response) {
			if (!error && options && response) {
                ...
                logger.warning('Re-sending command response').data('session', session ? session.getFullKey() : null);
					...
					if (error) {
						...
						postProcessCommandResponse(results, function (options, content) {
							...
							that._cacheResponse(session, queryId, options, content, function () {
								...
								logger.info('Executed and cached user commands', commandNames).data({ commandNames: commandNames, queryId: queryId });
