...
function createResponseCacheKey(session) {
	if (!session) {
		return null;
	}
	...
	if (mcPrefix) {
		return mcPrefix + '/cmdresponse/' + sessionKey;
	}

	return 'cmdresponse/' + sessionKey;
}
...
CommandCenter.prototype._tryLoadCachedResponse = function (session, queryId, cb) {
	...
	if (!queryId || !this.responseCacheTTL) {
		return cb();
	}
	...
	var key = createResponseCacheKey(session);
	if (!key) {
		return cb();
	}
...
*/
	...
	client.set(key, value, this.responseCacheTTL, function (error) {
		// error parameter is irrelevant, cache is best effort
		if (error) {
			logger.error('Caching command response failed:', error);
		} else {
			logger.debug('Caching command response successful:', key);
		}
