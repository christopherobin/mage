...
CommandCenter.prototype._tryLoadCachedResponse = function (state, queryId, cb) {
	...
	state.archivist.get('ucResponseMeta', { session: sessionKey }, { optional: true, mediaTypes: ['text/plain'], encodings: ['utf8'] }, function (error, info) {
		if (error) {
			return cb(error);
		}

		if (!info || typeof info !== 'string') {
			logger.error('Response meta information was not valid:', info);
			return cb();
		}
		...
		state.archivist.get('ucResponse', { session: sessionKey }, { optional: true }, function (error, response) {
			if (error) {
				return cb(error);
			}
...
CommandCenter.prototype._cacheResponse = function (state, queryId, options, response) {
	...
	var expirationTime = parseInt(Date.now() / 1000, 10) + this.responseCacheTTL;
...
CommandCenter.prototype.executeCommands = function (commandNames, headerData, paramsData, files, queryId, transportInfo, transportCb) {
	...
	processCommandHeader(state, headerData, paramsData, function (error, tags) {
		...
		that._tryLoadCachedResponse(state, queryId, function (error, options, response) {
				...
				function (error, results) {
					if (error) {
						...
						postProcessCommandResponse(results, function (options, content) {
							...
							that._cacheResponse(state, queryId, options, content);
