...
OpenSocial.prototype.authenticateUser = function(httpMethod, url, params, authHeader)
{
	...
	if (!oauth.isValidSignature(httpMethod, url, params, authHeader)) return false;
...
OpenSocial.prototype.oauth =
{
	...
	isValidSignature: function(httpMethod, url, params, authHeader)
	{
		...
		var genSignature = this.generateSignature(httpMethod, url, params, params.oauth_token_secret);
...
OpenSocial.prototype.rest = function(httpMethod, user, path, getData, postData)
{
	...
	var oauthKey = this.consumerSecret + '&' + user.oauth_token_secret;
	var genSignature = crypto.createHmac('sha1', oauthKey).update(baseString).digest('base64');
